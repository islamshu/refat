(function () {
  ...
  async function DEPRECATED_handleFeaturedProductPrice() {
    ...
    !t ||
      t('[data-pf-type="Shopify.FeaturedProduct"]').each((e, n) => {
        t(n)
          ...
          .on('change', (r) => {
            const o = t(r.target),
              ...
              u = c.data('image');
            let f = r.target;
            for (; f && f.nodeName !== 'BODY' && !f.querySelector('.pf-c'); )
              f = f.parentNode;
            l
              ? (s.addClass('pf-p-hs'),
                parseInt(l) !== parseInt(d)
                  ? s.find('.pf-p-prr').removeClass('hide').html(l)
                  : s.find('.pf-p-prr').addClass('hide'),
                s.find('.pf-p-prs').html(d))
              : (s.find('.pf-p-prr').addClass('hide'),
                s.find('.pf-p-prs').html(d)),
              u &&
                f.querySelector('.pf-feature-product__image img') &&
                f
                  .querySelector('.pf-feature-product__image img')
                  .setAttribute('src', u);
          });
      });
  }
  ...
  var LIST_LAYOUT = ((t) => (
      (t[(t.GRID = 0)] = 'GRID'), (t[(t.SLIDE = 1)] = 'SLIDE'), t
    ))(LIST_LAYOUT || {}),
    HOVER_ACTION = ((t) => (
      (t[(t.NONE = 0)] = 'NONE'),
      (t[(t.MAGNIFIER = 1)] = 'MAGNIFIER'),
      (t[(t.HOVER = 2)] = 'HOVER'),
      t
    ))(HOVER_ACTION || {}),
    ON_HOVER = ((t) => (
      (t[(t.NEXT_IMAGE = 0)] = 'NEXT_IMAGE'),
      (t[(t.LAST_IMAGE = 1)] = 'LAST_IMAGE'),
      (t[(t.RANDOM_IMAGE = 2)] = 'RANDOM_IMAGE'),
      (t[(t.ALL_IMAGE = 3)] = 'ALL_IMAGE'),
      t
    ))(ON_HOVER || {}),
    CLICK_ACTION = ((t) => (
      (t[(t.NONE = 0)] = 'NONE'),
      (t[(t.LINK_TO_PRODUCT = 1)] = 'LINK_TO_PRODUCT'),
      (t[(t.SHOW_FULLSCREEN = 2)] = 'SHOW_FULLSCREEN'),
      t
    ))(CLICK_ACTION || {});
  const DEVICES_MAP = {
    ...
    TIME_FORMATS_METAFIELD = { '%I:%M %p': 'KK:mm a', '%H:%M': 'HH:mm' },
    TYPE_METAFIELD = {
      COLOR_FIELD: 'color',
      DATE_FIELD: 'date',
  ...
  `,
    ...
    __pagefly_product_store__ = (window.__pagefly_product_store__ =
      new PageFlyHelperStore()),
    __pagefly_helper_store__$1 = (window.__pagefly_helper_store__ =
      new PageFlyHelperStore()),
    MapStyles = {
  ...
  async function getHttpRequest(t) {
    return new Promise((e, n) => {
      ...
      a.open('GET', t, !0),
        (a.onload = () =>
          a.status === 200 ? e(a.response) : n(Error(a.statusText))),
        (a.onerror = (r) => n(Error(`Network Error: ${r}`))),
        a.send();
    });
  }
  const initZoomImage = (t) => {
      ...
      (r.className = 'zoom-image'),
		...
		top: 0;
	`),
        !!t.querySelector('div.zoom-image') ||
          t.insertBefore(r, t.firstElementChild.nextSibling),
        t.addEventListener('mouseenter', function () {
          (e.style.display = 'none'),
            (r.style.backgroundSize = '150%'),
            (r.style.width = '100%'),
            (r.style.height = '100%');
        }),
        ...
        t.addEventListener('mouseleave', function () {
          t.querySelector('div.zoom-image').style.display !== 'none' &&
    ...
    initZoomImage2 = (t) => {
      ...
      n.addEventListener('mouseenter', function () {
        (e.style.opacity = 0),
          (n.style.backgroundImage = `url("${e.currentSrc}")`),
          (n.style.backgroundRepeat = 'no-repeat'),
          (n.style.backgroundSize = '200%');
      }),
        n.addEventListener('mousemove', function (a) {
          ...
          a.offsetX
            ? (r = a.offsetX)
            : (r = (d = a.touches) == null ? void 0 : d[0].pageX),
            a.offsetY
              ? (o = a.offsetY)
              : (r = (l = a.touches) == null ? void 0 : l[0].pageX);
    ...
    checkARCompatible = (t) => {
      ...
      if (t === 'iOS') {
        let e = !1;
        return (
          document.createElement('a').relList.supports('ar') && (e = !0), e
        );
      }
      return !1;
    };
  function getPageTypeFromPFPageSetting() {
    var t;
    return (
      pfPageSetting.pageType ||
      ((t = window.__pagefly_setting__) == null ? void 0 : t.pageType)
    );
  }
  const TIMER_OUT = 1500,
    timeoutPromise = (t = TIMER_OUT) =>
      new Promise((e) => {
        setTimeout(e, t);
      }),
    isTrackingActive = () => {
      var t, e, n;
      return (
        (((t = window.__pagefly_analytics_settings__) == null
          ? void 0
          : t.acceptNewTracking) ||
          ((e = window.__pagefly_analytics_settings__) == null
            ? void 0
            : e.acceptTracking)) &&
        !window.location.href.includes('preview?id=') &&
        (typeof ((n = window.Shopify.customerPrivacy) == null
          ? void 0
          : n.userCanBeTracked) == 'function'
      ...
      if (s && s.length > e) {
        ...
        for (let f = 0; f < d.length && !(l === e || !s.length); f++) {
          ...
          if (s[0] === v) {
            ...
            !v.match(/^&[a-z0-9]+;$/i) &&
              (v.length > 1 || v.match(/[a-z0-9]/i) || v.match(r)) &&
              ![
                '',
                ' ',
                '`',
              ...
              (l++, n && l === e && (h = h.replace(y, `${y}&hellip;`)));
          }
          c.push(h);
        }
        (c = c.join(' ').replace(/ </g, '<').replace(/> /g, '>')),
          o && (c = c.replace(new RegExp(` (${a})`, 'g'), '$1'));
        const u = c.match(/<[^>]*>/g);
        if (u) {
          const f = [];
          u.forEach((h) => {
            if (
              h.match(/\/>$/) ||
              h.match(/<br\s*(((\w|[.-])+="[^"]*"\s*|\s+(\w)*))*\/?>/)
            )
              return;
            let v = h.match(/^<([a-z0-9\-_]+)/i);
            if (v) return f.push(v[1].toLowerCase());
            if (
              ((v = h.match(/^<\/([a-z0-9\-_]+)>/i)),
              v && v[1].toLowerCase() === f[f.length - 1])
            )
              return f.pop();
          }),
            f.length &&
  ...
  function hideElementIfNotFound(t, e) {
    ...
    if (a === n)
      for (let r = 0; r < n; r++)
        (t[r].style.visibility = t[r].style.visibility || 'visible'),
          (t[r].style.height = e[r].style.height || 'unset'),
          (t[r].style.overflow = t[r].style.overflow || 'auto');
  }
  ...
  function getOriginalSrc(t) {
    if (!t.includes('https://cdn.shopify.com') || !t.includes('&resized'))
      return t;
  ...
  function getPFHelperVersion(t) {
    try {
      const e =
          /^https:\/\/(cdn|apps|wip|rc)\.pagefly\.io\/assets\/pagefly\/assets\/[^\/]+\/helper\.js.*$/,
        n =
          /^https:\/\/(cdn|apps|wip|rc)\.pagefly\.io\/pagefly\/[^\/]+\/core\/helper\.js.*$/;
      if (e.test(t)) {
        const a = t.split('/');
        return a[a.length - 2];
      } else if (n.test(t)) {
  ...
  function comparePFHelperVersion(t, e) {
    try {
      ...
      return n[0] == a[0]
        ? n[1] == a[1]
          ? parseInt(n[2]) - parseInt(a[2])
          : parseInt(n[1]) - parseInt(a[1])
        : parseInt(n[0]) - parseInt(a[0]);
    } catch (n) {
  ...
  const getFrontEndSettings = (t) => {
    ...
    return (((a = window.__pagefly_setting__) == null
      ? void 0
      : a.elementData) || {})[e];
  };
  window.addEventListener('message', (t) => {
    try {
      const e = JSON.parse(t.data),
        { key: n, value: a, action: r } = e;
      if (n === 'PAGEFLY_MESSAGE')
        switch (r) {
          case 'redirect':
            return a.match(/javascript:/i)
              ? void 0
              : (window.location.href = a);
        }
    } catch {}
  });
  const pageflyLivePageRedirect = (t) => {
    t.match(/javascript:/i) ||
      (console.info('PageFly:: redirect to:', t),
  ...
  function cartAddFromForm(t) {
    ...
    return (
      ...
      (e.body = t),
      fetchJSON(
        (a = (n = window.Shopify) == null ? void 0 : n.routes) != null && a.root
  ...
  async function createOrderItem(t, e) {
    try {
      const n = await fetch(`${t}/api/public/order-item/create`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(e),
      });
      if (!n.ok) throw new Error('Bad response :<');
      const a = await n.json();
      if (!a.success) throw new Error(a.message);
    } catch (n) {
      console.error(n instanceof Error ? n.message : n);
    }
  }
  async function onCreateOrderItem(t) {
    ...
    try {
      let f = o;
      if (!e || !n) return;
      const h = await getCart();
      if (!h) return;
      const { items: v } = h,
        y = v.find((I) => I.id === a && !!I.properties[ATC_PF_ANALYTICS_ID]);
      y &&
        (r[`properties[${ATC_PF_ANALYTICS_ID}]`] = f =
          y.properties[ATC_PF_ANALYTICS_ID]);
      const { pf_page_name: m, pf_page_subject_id: S } = n,
        E = (l = window.__pagefly_setting__) == null ? void 0 : l.baseURL,
        k = (u = window.__pagefly_setting__) == null ? void 0 : u.shopDomain,
        w = { _id: f, shopDomain: k, properties: {} };
      if (i) {
        const { featured_image: I, title: M } = i,
          x = {
            pageType: s,
            pageId: c,
            pageName: m,
        ...
        Object.assign(w.properties, x);
      }
      if (d && d instanceof Array) {
        const I = [];
        d.forEach((M) => {
          const { id: x, title: q, image: O } = M,
            T = O == null ? void 0 : O.src;
          I.push({
            collectionId: x,
            collectionTitle: q,
            collectionImageSrc: T,
          });
        }),
          Object.assign(w.properties, { collectionsOfProduct: I });
      }
      await createOrderItem(E, w);
    } catch (f) {
      console.log(f instanceof Error ? f.message : f);
    }
  }
  const GROUPTHOUGHT_THEME = ['pipeline', 'story'],
    handleGroupThoughtThemes = async () => {
      var t, e, n, a;
      window.theme &&
        ((e = (t = window.theme) == null ? void 0 : t.info) == null
          ? void 0
          : e.name) &&
        GROUPTHOUGHT_THEME.includes(
          (a = (n = window.theme) == null ? void 0 : n.info) == null
            ? void 0
            : a.name
        ) &&
        handleGroupThoughtThemePartner().catch(console.error);
    },
    handleGroupThoughtThemePartner = async () => {
      ...
      document.dispatchEvent(
        ...
        (e = document.querySelector('[data-drawer="drawer-cart"]')) == null ||
  ...
  const handleThemeEditions = () => {
    ...
    handleThemeEmpire = () => {
      fetch('/cart.json')
        .then((t) => t.json())
        .then((t) => {
          const e = new CustomEvent('cartcount:update', { detail: t });
          window.dispatchEvent(e);
        });
    },
    handleThemePacific = () => {
      window.Shopify.getCart((t) => {
        window.$('.cart-item-count').html(t.item_count),
    ...
    handleThemeStartup = () => {
      window.Shopify.getCart((t) => {
        document.dispatchEvent(
          new CustomEvent('cart:count', { detail: { count: t.item_count } })
        );
      });
    },
    handleThemeLaunch = () => {
      window.Shopify.getCart((t) => {
        document
          .querySelectorAll('.cart-link .cart-count')
          .forEach((e) => (e.innerHTML = t.item_count)),
          document
            .querySelectorAll('.header-cart-count')
    ...
    handleRedPlugDesignThemes = async () => {
      var t, e;
      window.themeInfo &&
        ((t = window.themeInfo) == null ? void 0 : t.name) &&
        RED_PLUG_DESIGN_THEME.includes(
          (e = window.themeInfo) == null ? void 0 : e.name
        ) &&
        handleRedPlugDesignTheme().catch(console.error);
    },
  ...
  function handleRoarTheme(t) {
    var n;
    const { lastATCResult: e } = __pagefly_helper_store__$1;
    (n = window.Fastor) != null && n.setupCartPopup
      ? handleThemeFastor(e)
      : /BeYours/i.test(t) && handleThemeBeYours(e);
  }
  ...
  const handleThemeOfStudioZash = () => {
    const { lastATCResult: t } = __pagefly_helper_store__$1,
  ...
  function uuid() {
    (!BUFFER || IDX2 + 16 > SIZE) &&
      ((BUFFER = crypto.getRandomValues(new Uint8Array(SIZE))), (IDX2 = 0));
    let t = 0,
      e,
      n = '';
    for (; t < 16; t++)
  ...
  window.uniqueId = { uid, uuid, validateUUID };
  function formcheck(t) {
    console.log('validating...', t);
    const e =
        window._pagefly_form_fields_selector ||
        '[name][required]:not([hidden]):not([type="file"]):not([type="hidden"])',
  ...
  function toFormData(t) {
    let e = new FormData();
    return (
      typeof e._asNative == 'function' && (e = e._asNative().fd),
  ...
  const handleTimberTheme = async () => {
    ...
    handleBlumTheme = (t) => {
      typeof window.SHTHelper.forceUpdateCartStatus == 'function' &&
        window.SHTHelper.forceUpdateCartStatus(t);
    };
  function ajaxCartUpdate(t) {
    var e, n, a;
    try {
      __pagefly_helper_store__$1.cart &&
        ((__pagefly_helper_store__$1.lastATCResult = t),
        __pagefly_helper_store__$1.update(t));
      const { lastATCResult: r, autoCartUpdate: o } =
        __pagefly_helper_store__$1;
      if (
        o &&
        r &&
        ((n = (e = window.Shopify) == null ? void 0 : e.theme) == null
          ? void 0
          : n.name)
      ) {
        const i =
          window.__pagefly_theme_atc_check__ || window.Shopify.theme.name;
        handlePixelUnionThemes(i),
          handleGroupThoughtThemes(),
          handleRedPlugDesignThemes(),
          handleRoarTheme(i),
          handleStudioZashTheme(i),
          handleBlumTheme(t),
          window.timber &&
            ((a = window.ajaxCart) == null ? void 0 : a.cartUpdateCallback) &&
            handleTimberTheme().catch(console.error);
      }
    } catch (r) {
      console.warn(r);
    }
  ...
  function customDataForTheme() {
    var e, n;
    const t =
      window.__pagefly_theme_atc_check__ ||
      ((n =
    ...
    switch (!0) {
      ...
      case /BeYours/i.test(t):
        return { sections: 'cart-icon-bubble,mini-cart' };
      default:
        return {};
    }
  }
  async function handleShopifyProductATC() {
    ...
    document
      ...
      .forEach((r, o) => {
        ...
        pageflyProducts[i] &&
          ((pageflyProducts[i].comparePrices =
            pageflyProducts[i].comparePrices || []),
          pageflyProducts[i].comparePrices.push(r));
      });
    ...
    t.forEach((r) => {
      const o = r.closest('form'),
        ...
        { iconPos: s = 'left' } = i || {},
        c = r.querySelector('[data-pf-type="Icon"]');
      r.addEventListener('click', async (d) => {
        ...
        const x =
          E.pfCurrentVariant &&
          E.pfCurrentVariant[getElemIdByClassname(I) || OUTSIDE_BOX_KEY];
        if (
          (x && x.id
            ? (M = x.id)
            : l.getAttribute('data-current-variant')
            ? (M = l.getAttribute('data-current-variant'))
            : (M =
                ((D = E.selected_or_first_available_variant) == null
                  ? void 0
                  : D.id) ||
                ((F = E == null ? void 0 : E.variants[0]) == null
                  ? void 0
                  : F.id)),
          !M)
        )
          return console.warn('Variant have no product!');
        if (o) {
          ...
          if (
            (N = (B = window.shopstorm) == null ? void 0 : B.apps) != null &&
            N.productCustomizer
          )
            try {
              if (
                o.querySelector('.product-customizer-options') &&
                o.querySelector('input[type="file"]')
              ) {
                window.shopstorm.apps.productCustomizer.addOptionPricingItemToCart(
                  y
                );
          ...
          if (
            o.querySelector('[data-pf-type="InfiniteProductOption"]') &&
            !(window.validate_options && (await window.validate_options(y)))
          )
            return;
          const W = o.querySelector('[data-pf-type="InfiniteOptionsShopPad"]');
          if (
            (W && W.querySelector('[data-uploadery-valid="false"]')) ||
            ('__pf_formcheck_callback' in window &&
              typeof window.__pf_formcheck_callback == 'function' &&
              !window.__pf_formcheck_callback(o))
          )
            return;
          if (!o.checkValidity()) {
            o.reportValidity();
          ...
          if (V.length > 0) {
            ...
            (H.style.color = '#dc3545'),
              (H.textContent =
                'Please fill all the required fields(*) before Add To Cart!'),
              l.insertAfter(H),
              setTimeout(() => H.remove(), 3e3);
            return;
          }
        }
        const q = w
            ? w.querySelector('[data-pf-type="ProductQuantity"] input')
            : E.quantityArr &&
              E.quantityArr.get(OUTSIDE_BOX_KEY) &&
              E.quantityArr.get(OUTSIDE_BOX_KEY)[0].querySelector('input'),
          O = (q && q.value) || 1;
        let T,
          L = {};
        (L = customDataForTheme()),
          window.__getPFCustomATCData &&
            typeof window.__getPFCustomATCData == 'function' &&
            (L = window.__getPFCustomATCData(y));
        const A = { quantity: O, id: M, ...L },
          C = window.pfPageInfo,
          p = uuid();
        if (isTrackingActive()) {
          const R = await getCart(),
            { items: Y } = R,
            W = Y.find(
              (V) => V.id === M && !!V.properties[ATC_PF_ANALYTICS_ID]
            );
          W
            ? (A[`properties[${ATC_PF_ANALYTICS_ID}]`] =
                W.properties[ATC_PF_ANALYTICS_ID])
            : (A[`properties[${ATC_PF_ANALYTICS_ID}]`] = p);
        }
        if (o) {
          (T = new FormData(o)),
            typeof T._asNative == 'function' && (T = T._asNative().fd);
          const R = o.querySelector('[name="id"]'),
            Y = R && R.value;
          Number(Y) || T.append('id', M),
            Object.entries(L).forEach(([W, V]) => {
              T.append(W, V);
            }),
            C &&
              a &&
              A[`properties[${ATC_PF_ANALYTICS_ID}]`] &&
              T.append(
                `properties[${ATC_PF_ANALYTICS_ID}]`,
                A[`properties[${ATC_PF_ANALYTICS_ID}]`]
              );
        } else T = toFormData(A);
        const g = l.textContent,
          _ = l.children;
        l.dataset.pfType === 'ProductATC'
          ? Array.from(_).forEach((R) => (R.style.display = 'none'))
          : l.dataset.pfType === 'ProductATC2' && (l.textContent = '');
        const P = document.createElement('span');
        if (
          ((P.textContent = m),
          l.append(P),
          c && (s === 'left' ? r.prepend(c) : r.append(c)),
          ...
          if (typeof window < 'u') {
            ...
            window.gtag(
              ...
              Object.assign(
                {
                  pf_add_to_cart: sessionStorage.getItem(R) ? 0 : 1,
                  send_to: 'pagefly',
                  pf_product_id: M,
        ...
        return cartAddFromForm(T)
          .then(async (R) => {
            console.log('PF ATC Result: ', R);
            const { status: Y, description: W } = R;
            if (Y >= 422) l.parentElement.append(n), (n.textContent = `* ${W}`);
              ...
              const V = {
                  ...
                  pageType: f,
                  pageId: u,
                  collectionsOfProduct: k,
                },
                H = new Promise(async (X) => {
                  await onCreateOrderItem(V), X('Created order item');
                });
              switch ((await Promise.race([H, timeoutPromise()]), h)) {
                default:
                case 'same':
                  ajaxCartUpdate(R);
                  break;
                case 'cart':
                  console.log('checkout...'), pageflyLivePageRedirect('/cart');
                  break;
                case 'checkout':
                  console.log('checkout...');
                  const X = checkLanguage();
                  pageflyLivePageRedirect(
                    `/checkout${X ? `?locale=${X}` : ''}`
                  );
                  break;
                case 'link':
            ...
            if (
              (setTimeout(
                () => {
                  P.remove(),
                    l.dataset.pfType === 'ProductATC'
                      ? Array.from(_).forEach(
                          (V) => (V.style.display = 'inline-block')
                        )
                      : l.dataset.pfType === 'ProductATC2' &&
                        (l.textContent = g),
                    c && (s === 'left' ? r.prepend(c) : r.append(c)),
  ...
  async function DEPRECATED_addToCartHelper() {
    const t = window.jQuery;
    if (!t) return;
    t('a[data-action="AddToCart"]').length &&
      getCart().then((n) => {
      ...
      t(document).click((n) => {
        ...
        if (n.target.getAttribute('data-action') === 'AddToCart') {
          ...
          const c = s.data('text'),
            ...
            f = s.data('variation');
          let h;
          (h = s.find('span').data('variant')),
            (f || !h) && (h = s.closest('form').find('select').val());
          const v = { quantity: u, id: h };
          s.addClass('is-loading');
          const y = s.children().eq(0);
          y.addClass('pfa pfa-spinner pfa-spin'),
            y.text(''),
            t.ajax({
              type: 'POST',
              url:
                (r = (a = window.Shopify) == null ? void 0 : a.routes) !=
                  null && r.root
                  ? `${
                      (i = (o = window.Shopify) == null ? void 0 : o.routes) ==
                      null
                        ? void 0
                        : i.root
                    }cart/add.js`
              ...
              error: (m) => {
                if (m.status === 422) {
                  const S = JSON.parse(m.responseText);
                  s.html(`<p>${S.description}</p>`);
                }
              },
              success: (m) => {
                y.removeClass('pfa pfa-spinner pfa-spin'),
    ...
    if (t && (n == null ? void 0 : n.available)) {
      ...
      if (!t.querySelector('span')) t.innerHTML = o;
        ...
        const i = new DOMParser().parseFromString(
          ...
          s =
            ((a = i == null ? void 0 : i.firstElementChild) == null
              ? void 0
              : a.querySelector('[data-pf-type="Text"]')) ||
            (i == null ? void 0 : i.firstElementChild),
      ...
      t.querySelector('span')
        ? (t.querySelector('span').innerHTML = o)
  ...
  async function storeSomeProductElements() {
    ...
    storeElsInProductByType(t, 'price'),
      storeElsInProductByType(e, 'comparePrice'),
      storeElsInProductByType(n, 'atc'),
      storeElsInProductByType(a, 'media'),
      storeElsInProductByType(r, 'media2');
  }
  function storeElemInProductByType(t, e) {
    ...
    if (e === 'atc') {
      const { atcContents: i = new Map() } = a,
        s = getElemIdByClassname(t.className);
      if (!i.get(s)) {
        const { innerHTML: c } = t;
  ...
  async function handleShopifyProductBadge() {
    document
      .querySelectorAll('[data-pf-type="ProductBadge"]')
      .forEach((e, n) => {
        storeElemInProductByType(e, 'badge');
        const a = e.getAttribute('data-product-id'),
          r = pageflyProducts[a],
          o = getFrontEndSettings(e);
        isValidCondition(a, o) && e.removeAttribute('hidden'),
          updateProductBadgeByVariant(
            [e],
            r == null ? void 0 : r.selected_or_first_available_variant
          ),
          updateProductBadgeByVariant(
            [e],
            r == null ? void 0 : r.selected_or_first_available_variant
  ...
  function updateProductBadgeByVariant(t, e) {
    ...
    function r() {
      const { price: o, compare_at_price: i } = e;
      let s = 0;
      const c = Number.parseFloat(o || 0),
        d = Number.parseFloat(i || 0);
      d > c && d !== 0 && (s = 100 - Math.round((c / d) * 100));
      const l = t[0].querySelectorAll("[data-badge-type='discount']");
      if ((l.length && s === 0 && (n = !0), !!l))
        for (const u of l) u.textContent = s;
    }
    return n;
  }
  async function handleProductCollectionDescription() {
    const t = document.querySelectorAll(
        '[data-pf-type="ProductDescription"][data-product-type="content"]:not([data-loaded-des="true"])'
      ),
      e = document.querySelectorAll(
        '[data-pf-type="CollectionDescription"][data-collection-type="description"]:not([data-loaded-des="true"])'
      ),
      n = document.querySelectorAll(
        '[data-pf-type="ArticleExcerpt"]:not([data-loaded-des="true"])'
      ),
      a = (r) => {
        const o = r.getAttribute('data-compact');
        r.getAttribute('data-loaded-des') ||
          r.setAttribute('data-loaded-des', 'true');
        const s =
            /<meta charset="utf-8"[^>]*>|(<style>((.|\s)*?)<\/style>)|(<script((.|\s)*?)<\/script>)/g,
        ...
        if (o) {
          ...
          if (l.length < c.length) {
            ...
            (f = (f && f.trim()) || 'Less'),
              d !== 'false' &&
                (l = `${l} <a href="#" class="pf-read-more">${u
                  .replace(/</g, '&lt;')
                  .replace(/>/g, '&gt;')}</a>`),
              (r.innerHTML = l),
              d !== 'false' &&
                (r.onmousedown = function (h) {
                  const y = h.target.closest('.pf-read-more');
                  y &&
                    (y.onclick = function (m) {
                      m.preventDefault(),
                        r.getAttribute('data-collapse') === 'true'
                          ? ((r.innerHTML = c.concat(
                              ` <a href="#" class="pf-read-more">${f
                                .replace(/</g, '&lt;')
                                .replace(/>/g, '&gt;')}</a>`
                            )),
                            r.setAttribute('data-collapse', 'false'))
                          : ((r.innerHTML = l),
                            r.setAttribute('data-collapse', 'true'));
                    });
                });
    ...
    Array.from(t).forEach((r) => a(r)),
      Array.from(e).forEach((r) => a(r)),
      Array.from(n).forEach((r) => a(r));
  }
  const videoTypes = {
    ...
    loadExternalVideo = (t, e, n) => {
      ...
      if (r) r.setAttribute('src', a), r.classList.remove('pf-hidden');
        ...
        (o.src = a),
          (o.style.border = 'none'),
          o.classList.add('featured-media'),
          t.prepend(o);
      }
    },
    loadVideo = (t, e) => {
      ...
      if (n)
        n.setAttribute('src', e),
          ...
          n.classList.remove('pf-hidden');
      else {
        const a = document.createElement('video');
        (a.src = e),
          a.setAttribute('controls', ''),
          a.classList.add('featured-media'),
          t.prepend(a);
      }
    },
    loadModelViewer = (t, e, n, a) => {
      ...
      if (r)
        r.setAttribute('poster', n),
          r.setAttribute('src', e),
          r.setAttribute('alt', a),
          r.classList.remove('pf-hidden');
      else {
        const o = document.createElement('model-viewer');
        (o.loading = 'eager'),
          o.setAttribute('camera-controls', ''),
          o.setAttribute('poster', n),
    ...
    getVideoSrcOfHighestQuality = (t) => {
      ...
      for (let n = 1; n < t.length; n++)
        t[n].format === t[0].format && t[n].height > e.height && (e = t[n]);
      return e.url;
    },
    U = class {
      constructor(e, n, a = 0) {
        ...
        b(this, '$hint');
        b(this, 'isZoom', !1);
        b(this, 'transformState', { scale: 1, transX: 0, transY: 0 });
        b(this, 'pointState', {
          startX0: null,
          startY0: null,
          endX0: null,
        ...
        this.getDevice(),
          ...
          n.appendChild(this.$GalleryEl);
      }
      load() {
        this.loadCSS(),
          this.fillHtmlIntoContainer(),
          this.getElement(),
          this.initEvent(),
          (this.loaded = !0);
      }
      getDevice() {
        (this.isTouchDevice = ['mobile', 'tablet'].includes(getDevice())),
          this.isTouchDevice ? (this.limit = 5) : (this.limit = 10);
      }
      fillHtmlIntoContainer() {
        this.$GalleryEl.classList.add(
          'pf-gallery-container',
          'pf-gallery-hide'
        );
        const e = this.media
          .map((n, a) => U.html.item(n.preview_image.src, a, n.media_type))
          .join('');
        this.$GalleryEl.innerHTML = U.html.container(
          this.media[0].preview_image.src,
          e,
          this.media.length
        );
      }
      getElement() {
        const { selector: e } = U;
        (this.$master = this.$GalleryEl.querySelector(e.master)),
          (this.$masterImg = this.$master.querySelector('img')),
          (this.$List = this.$GalleryEl.querySelector(e.list)),
          (this.$Items = this.$List.querySelectorAll(e.item)),
          (this.$hint = this.$GalleryEl.querySelector(e.hint));
      }
      initEvent() {
        this.isTouchDevice
          ? (this.$masterImg.addEventListener(
              'touchstart',
          ...
          this.$masterImg.addEventListener(
            'load',
            this.onLoadedMasterImageHandler.bind(this)
          ),
      ...
      handleMouseDown(e) {
        if (
          ((this.pointState.startX0 = e.pageX),
          (this.pointState.startY0 = e.pageY),
          (this.pointState.endX0 = e.pageX),
          (this.pointState.endY0 = e.pageY),
          this.isZoom)
        ) {
      ...
      moveZoomedImageOnDesktopHandler(e) {
        e.preventDefault(),
          this.updateStyleOnMovingZoomedImage({
            pageX: e.pageX,
            pageY: e.pageY,
          });
      }
      ...
      touchMoveHandler(e) {
        ...
        e.touches.length === 1 &&
          ...
          e.touches.length === 2 &&
            ((this.twoTouchesMoveTimeStamp = e.timeStamp),
            this.updateStyleOnPinchingZoomingImage(
      ...
      updateStyleOnMovingZoomedImage(e) {
        ...
        Math.abs(o) > Math.abs(n.x) && (o = o > 0 ? n.x : -n.x),
          Math.abs(i) > Math.abs(n.y) && (i = i > 0 ? n.y : -n.y),
          (this.$masterImg.style.transform = `scale(${this.transformState.scale}) translate(${o}px, ${i}px)`),
          (this.transformState.transX = o),
          (this.transformState.transY = i),
          (this.pointState.startX0 = e.pageX),
          (this.pointState.startY0 = e.pageY);
      }
      updateStyleOnPinchingZoomingImage(e, n) {
        const a = this.getDistanceBetweenTwoPoints({
            point0: { x: this.pointState.startX0, y: this.pointState.startY0 },
            point1: { x: this.pointState.startX1, y: this.pointState.startY1 },
          }),
          r = this.getDistanceBetweenTwoPoints({
            point0: { x: e.pageX, y: e.pageY },
            point1: { x: n.pageX, y: n.pageY },
          }),
          o = Math.abs(r / a);
        let i = isNaN(o)
            ? this.transformState.scale
            : o * this.transformState.scale,
          s = this.transformState.transX,
          c = this.transformState.transY;
        i > 8 && (i = 8),
          i < 1 && ((i = i < 0.25 ? 0.25 : i), (s = 0), (c = 0)),
          (this.$masterImg.style.transform = `scale(${i}) translate(${s}px, ${c}px)`),
          (this.transformState = { scale: i, transX: s, transY: c }),
          (this.pointState = {
            ...this.pointState,
            startX0: e.pageX,
            startY0: e.pageY,
      ...
      getDistanceBetweenTwoPoints(e) {
        const { point0: n, point1: a } = e,
          r = n.x - a.x,
          o = n.y - a.y;
        return Math.sqrt(Math.pow(r, 2) + Math.pow(o, 2));
      }
      getTranslateRange() {
        const { offsetWidth: e, offsetHeight: n } = this.$master,
          { width: a, height: r } = this.$masterImg;
        return {
          x: Math.floor(
            Math.abs(a * this.transformState.scale - e) /
              (this.transformState.scale * 2)
          ),
          y: Math.floor(
            Math.abs(r * this.transformState.scale - n) /
              (this.transformState.scale * 2)
          ),
        };
      }
      turnOnZoom() {
        const { desktopScale: e } = U;
        (this.isZoom = !0),
          (this.$master.style.width = '100%'),
          (this.$master.style.height = '100%'),
          (this.$masterImg.style.cursor = 'move'),
          (this.$masterImg.style.transform = `scale(${e}) translate(0px, 0px)`),
          (this.transformState = { scale: e, transX: 0, transY: 0 });
      }
      turnOffZoom() {
        this.$master.style.removeProperty('width'),
          this.$master.style.removeProperty('height'),
          (this.$masterImg.style.cursor = 'zoom-in'),
          (this.$masterImg.style.transform = 'none'),
          (this.transformState = { scale: 1, transX: 0, transY: 0 }),
          (this.isZoom = !1);
      }
      toggleZoom() {
      ...
      show(e = -1) {
        this.loaded || this.load(),
          this.$GalleryEl.classList.remove('pf-gallery-hide'),
          document.querySelector('body').classList.add('pf-gallery-open'),
          e !== -1 && (this.setImage(e), this.scrollItemToView(e));
      }
      hide() {
        document.querySelector('body').classList.remove('pf-gallery-open'),
          this.$GalleryEl.classList.add('pf-gallery-hide'),
          this.turnOffZoom();
      }
      handleClickEvent(e) {
        const n = e.target;
        n.closest(U.selector.exit)
          ? this.hide()
          : n.closest(U.selector.left)
          ? this.setImage(this.index - 1)
          : n.closest(U.selector.right)
          ? this.setImage(this.index + 1)
          : n.closest(U.selector.item) && this.handleClickItem(n);
      }
      handleSelectMedia(e) {
      ...
      setImage(e = 0) {
        ...
        for (let a = 0; a < this.$master.children.length - 1; a++)
          this.$master.children[a].style.display = 'none';
        this.turnOffZoom(),
          this.media[e].media_type === 'image'
            ? ((this.$masterImg.src = this.media[e].preview_image.src),
              (this.$hint.style.display = 'block'))
            : ((this.$masterImg.style.display = 'none'),
              (this.$hint.style.display = 'none')),
          this.handleSelectMedia(this.media[e]),
          this.$Items[this.index].removeAttribute('data-active'),
          this.scrollItemToView(e),
          this.$Items[e].setAttribute('data-active', ''),
          (this.index = e);
      }
      handleClickItem(e) {
        e = e.closest(U.selector.item);
      ...
      scrollItemToView(e) {
        ...
        let i = null;
        if (
          (r && ((i = a.offsetLeft - a.offsetWidth * 0.75), e === 0 && (i = 0)),
          o)
        ) {
          const s =
            a.offsetLeft + a.offsetWidth - (n.scrollLeft + n.offsetWidth);
          i = n.scrollLeft + s + a.offsetWidth * 0.5;
        }
        if (
          (i !== null && this.$List.scroll({ left: i, behavior: 'smooth' }),
          ...
          s = setInterval(() => {
            a.offsetLeft &&
              (clearInterval(s),
              this.$List.scroll({
                left: a.offsetLeft - a.offsetWidth * 0.75,
                behavior: 'smooth',
              }));
          }, 100);
        }
      ...
      handleSwipe() {
        ...
        const o = (s) => {
            this.transformState.scale <= 1 &&
              ...
              this.transformState.scale <= 1 &&
                s.changedTouches &&
                (a = { x: s.changedTouches[0].clientX, time: s.timeStamp });
          },
          i = (s) => {
            var c;
            if (this.transformState.scale <= 1 && s.timeStamp - r < 500)
              return !1;
            if (
              this.transformState.scale <= 1 &&
              ((c = s.changedTouches) == null ? void 0 : c.length) === 1 &&
              s.touches.length === 0
            ) {
              const d = s.changedTouches[0].clientX - a.x;
              s.timeStamp - a.time < 500 &&
                Math.abs(d) > n &&
                (d > n && this.setImage(this.index - 1),
                d < -n && this.setImage(this.index + 1));
            }
          };
      ...
      loadCSS() {
        ...
        (n.rel = 'stylesheet'),
          (n.href = `${e.baseURL}/static/assets/imageGallery.css?v=${e.pageflyVersion}`),
          document.head.appendChild(n);
      }
    };
  let MediaGallery = U;
  b(MediaGallery, 'desktopScale', 2.5),
    b(MediaGallery, 'selector', {
    ...
    b(MediaGallery, 'classes', { hide: 'pf-gallery-hide' }),
    b(MediaGallery, 'html', {
      container: (e, n, a) => `
			...
			<div class="pf-image-gallery">
				${
          a > 1
            ? `<span class="pf-gallery-arrow pf-left">${leftIcon}</span>
  ...
  function getIndexOfUserDefaultSource(t, e) {
    ...
    const n = t.getAttribute('data-product-id'),
      a = pageflyProducts[n],
      { media: r = [] } = a || {},
      o = getFrontEndSettings(t),
      i = (o == null ? void 0 : o.imageSource) || 'featured',
      c = new URLSearchParams(window.location.search).get('variant'),
      d = c
        ? (f = a == null ? void 0 : a.variants) == null
          ? void 0
          : f.findIndex((h) => h.id.toString() === c)
        : -1,
      l = d > 0 ? d : 0,
      u = a == null ? void 0 : a.variants[l];
    if (
      (i === 'variant' || d >= 0) &&
      u &&
      (u == null ? void 0 : u.featured_media)
    ) {
      const h = r.findIndex((v) => u.featured_media.id === v.id);
      return h || 0;
    }
    return 0;
  }
  function getProductMediaId2(t) {
    ...
    return `${
      getElemIdByClassname(e == null ? void 0 : e.className) +
      (e == null ? void 0 : e.getAttribute('data-product-id'))
    }_${e == null ? void 0 : e.getAttribute('data-media-id')}`;
  }
  ...
  (function (t, e) {
    ...
    return (function (t) {
      ...
      return (
        ...
        (n.c = e),
        (n.d = function (a, r, o) {
          n.o(a, r) || Object.defineProperty(a, r, { enumerable: !0, get: o });
        }),
        (n.r = function (a) {
          typeof Symbol < 'u' &&
            Symbol.toStringTag &&
        ...
        (n.t = function (a, r) {
          if (
            (1 & r && (a = n(a)),
            8 & r || (4 & r && typeof a == 'object' && a && a.__esModule))
          )
            return a;
      ...
      function (t, e, n) {
        ...
        t.exports = function (c, d, l) {
          ...
          function I(T) {
            var L = u,
              A = f;
            return (u = f = void 0), (S = T), (v = c.apply(A, L));
          }
          function M(T) {
            var L = T - m;
            return m === void 0 || L >= d || L < 0 || (k && T - S >= h);
          }
          function x() {
            var T,
              L,
            ...
            y = setTimeout(
              x,
              ((L = d - ((T = A) - m)), k ? s(L, h - (T - S)) : L)
            );
          }
          function q(T) {
            return (y = void 0), w && u ? I(T) : ((u = f = void 0), v);
          }
          function O() {
            var T,
              L = r(),
              A = M(L);
            if (((u = arguments), (f = this), (m = L), A)) {
              if (y === void 0)
                return (S = T = m), (y = setTimeout(x, d)), E ? I(T) : v;
              if (k) return clearTimeout(y), (y = setTimeout(x, d)), I(m);
            }
            return y === void 0 && (y = setTimeout(x, d)), v;
          }
          return (
            (d = o(d) || 0),
            a(l) &&
              ((E = !!l.leading),
              (h = (k = 'maxWait' in l) ? i(o(l.maxWait) || 0, d) : h),
      ...
      function (t, e, n) {
        ...
        t.exports = function (o, i, s) {
          ...
          return (
            ...
            a(o, i, { leading: c, maxWait: i, trailing: d })
          );
        };
      },
      function (t, e, n) {
        ...
        t.exports = {
          polyfill: function () {
            ...
            if (
              ...
              a.__forceSmoothScrollPolyfill__ === !0
            ) {
              var o,
                i = a.HTMLElement || a.Element,
                s = {
                ...
                d =
                  ...
                  RegExp('MSIE |Trident/|Edge/').test(o) ? 1 : 0);
              (a.scroll = a.scrollTo =
                function () {
                  arguments[0] !== void 0 &&
                    (u(arguments[0]) !== !0
                    ...
                    if (arguments[0] !== void 0)
                      if (u(arguments[0]) !== !0) {
                        ...
                        y.call(
                          this,
                          this,
                          m === void 0 ? this.scrollLeft : ~~m,
                          S === void 0 ? this.scrollTop : ~~S
                        );
                      } else {
                        if (
                  ...
                  if (u(arguments[0]) !== !0) {
                    ...
                    m !== r.body
                      ? (y.call(
                          this,
                          m,
                          m.scrollLeft + E.left - S.left,
                          m.scrollTop + E.top - S.top
                        ),
                        a.getComputedStyle(m).position !== 'fixed' &&
                          a.scrollBy({
                            left: S.left,
                            top: S.top,
                            behavior: 'smooth',
            ...
            function l(m, S) {
              (this.scrollLeft = m), (this.scrollTop = S);
            }
            function u(m) {
              if (
                m === null ||
            ...
            function f(m, S) {
              return S === 'Y'
                ? m.clientHeight + d < m.scrollHeight
                : S === 'X'
                ? m.clientWidth + d < m.scrollWidth
                : void 0;
            }
            function h(m, S) {
              var E = a.getComputedStyle(m, null)['overflow' + S];
            ...
            function y(m, S, E) {
              ...
              m === r.body
                ? ((k = a),
                  (w = a.scrollX || a.pageXOffset),
                  (I = a.scrollY || a.pageYOffset),
                  (M = s.scroll))
                : ((k = m), (w = m.scrollLeft), (I = m.scrollTop), (M = l)),
                (function q(O) {
                  var T,
                    L,
                    A,
                    C = (c() - O.startTime) / 468;
                  (T = 0.5 * (1 - Math.cos(Math.PI * (C = C > 1 ? 1 : C)))),
                    (L = O.startX + (O.x - O.startX) * T),
                    (A = O.startY + (O.y - O.startY) * T),
                    O.method.call(O.scrollable, L, A),
                    (L === O.x && A === O.y) ||
                      a.requestAnimationFrame(q.bind(a, O));
                })({
                  scrollable: k,
                  method: M,
                  startTime: x,
      ...
      function (t, e) {
        ...
        function o(f, h) {
          ...
          var v,
            ...
            S = [],
            E = [],
            k = f.querySelectorAll(a);
          for (
            h.includeContainer &&
              r.call(f, a) &&
              (k = Array.prototype.slice.apply(k)).unshift(f),
              v = 0;
            v < k.length;
            v++
          )
            i((y = k[v])) &&
              ((m = d(y)) === 0
                ? S.push(y)
                : E.push({ documentOrder: v, tabIndex: m, node: y }));
          return E.sort(l)
            .map(function (w) {
              return w.node;
            })
              ...
              !(function (y) {
                ...
                var m = (function (S) {
                  for (var E = 0; E < S.length; E++)
                    if (S[E].checked) return S[E];
                })(
                  y.ownerDocument.querySelectorAll(
                    'input[type="radio"][name="' + y.name + '"]'
                  )
                );
                return !m || m === y;
              })(h)) ||
        ...
        function l(f, h) {
          return f.tabIndex === h.tabIndex
            ? f.documentOrder - h.documentOrder
      ...
      function (t, e, n) {
        n.r(e);
        var a = function (T) {
            var L = T.Element.prototype;
            typeof L.matches != 'function' &&
              (L.matches =
                L.msMatchesSelector ||
              ...
              typeof L.closest != 'function' &&
                (L.closest = function (A) {
                  for (var C = this; C && C.nodeType === 1; ) {
        ...
        function h(T, L) {
          return T ? f((L || document).querySelectorAll(T)) : [];
        }
        function v(T, L) {
        ...
        function S(T, L, A) {
          return (T = Math.max(L, (T = Math.min(A, T))));
        }
        var E = (function () {
          var T = !1;
          try {
        ...
        function w(T) {
          for (var L = 1; L < arguments.length; L++) {
            ...
            L % 2
              ? k(Object(A), !0).forEach(function (C) {
                  I(T, C, A[C]);
                })
              : Object.getOwnPropertyDescriptors
              ? Object.defineProperties(T, Object.getOwnPropertyDescriptors(A))
              : k(Object(A)).forEach(function (C) {
              ...
              (A = [
                {
                  ...
                  value: function (p) {
                    return (
                      ['first', 'middle', 'last', 'prev', 'next'].indexOf(p) >=
                        0 || parseInt(p, 10) >= 1
                    );
                  },
                },
                {
                  key: 'getButtonTarget',
                  value: function (p) {
                    var g = p ? p.getAttribute('data-snap-slider-goto') : '';
                    if (!g) return {};
                    var _ = g.split(':').map(function (R) {
                        return R.trim();
                      }),
                      P = _.pop(),
                      D = _.pop();
                    if (!D) {
                      var F = p.closest('[data-snap-slider-nav]'),
                        B = p.closest('[data-snap-slider]');
                      F && (D = F.getAttribute('data-snap-slider-nav')),
                        B && (D = B.getAttribute('data-snap-slider'));
                    }
                    if (!D) {
                      var N = p.closest('data-snap-slider');
                      N && (D = N.getAttribute('data-snap-slider'));
                    }
                    return { sliderID: D, index: P };
                  },
                },
                {
                  key: 'handleGoto',
                  value: function (p) {
                    var g = p.target.closest('[data-snap-slider-goto]'),
                      _ = C.getButtonTarget(g),
                      P = _.sliderID,
                      D = _.index,
                ...
                { key: 'notFound', value: function (p) {} },
                {
                  key: 'get',
                  value: function (p) {
                    return window._SnapSliders[p];
                  },
                },
                { key: 'debug', value: function (p) {} },
              ]),
              (L = [
                {
                  key: 'init',
                  value: function (p) {
                    ...
                    var _ = v(this.options.container).shift();
                    if (_) {
                      for (var P in ((this.container = _),
                      (this.options.buttons =
                        g.buttons ||
                        this.container.getAttribute(
                          'data-snap-slider-buttons'
                        )),
                      (this.options.prev =
                        g.prev ||
                      ...
                      (this.id = this.getMaybeSetID(_, this.options.id)),
                      (this.slides = this.getMaybeSetSlides(
                        _,
                        this.options.slides
                      )),
                      (this.align = this.getMaybeSetAlign(
                        _,
                        this.options.align
                      )),
                      (this.current = this.getMaybeSetStart(
                        _,
                        this.options.start
                      )),
                      (this.loop = this.getMaybeSetLoop(_, this.options.loop)),
                      (this.transition = null),
                      (this.scrolling = !1),
                      this.options.on))
                        y(this.options.on, P) && this.on(P, this.options.on[P]);
                      var D,
                        F =
                          ((D = this.options),
                          ['buttons', 'prev', 'next'].reduce(function (B, N) {
                            return (B[N] = D[N]), B;
                          }, {}));
                      this.addGotoButtons(w(w({}, F), {}, { container: _ })),
                        this.addNav(
                          '[data-snap-slider-nav="'.concat(this.id, '"]'),
                          F
                        ),
                        this.options.nav && this.addNav(this.options.nav, F),
                        this.update();
                    }
                  },
                },
                {
                  ...
                  value: function (p, g) {
                    return (
                      (g = g || p.getAttribute('data-snap-slider') || p.id) ||
                        ((g = 'slider-'.concat(q)), (q += 1)),
                      p.setAttribute('data-snap-slider', g),
                      g
                    );
                  },
                },
                {
                  ...
                  value: function (p, g) {
                    (g =
                      g && typeof g == 'string'
                        ? g
                        : p.getAttribute('data-snap-slider-slides')),
                      p.setAttribute('data-snap-slider-slides', g || '');
                    var _ = g ? v(g, p) : f(p.children);
                    return (
                ...
                },
                {
                  ...
                  value: function (p, g) {
                    return (
                      (g = g || p.getAttribute('data-snap-slider-align') || ''),
                      p.setAttribute('data-snap-slider-align', g),
                      g
                    );
                  },
                },
                {
                  ...
                  value: function (p, g) {
                    return (
                      C.isValidIndex(g) ||
                        (g = p.getAttribute('data-snap-slider-start') || 1),
                      p.setAttribute('data-snap-slider-start', g),
                      g
                    );
                  },
                },
                  ...
                  value: function (p, g) {
                    return (
                      (g =
                        typeof g == 'boolean'
                          ? g
                          : p.getAttribute('data-snap-slider-loop') === 'true'),
                      p.setAttribute('data-snap-slider-loop', g),
                      g
                    );
                  },
                },
                {
                  ...
                  value: function (p) {
                    var g,
                      ...
                      F =
                        ((g = p),
                        (_ = window.getComputedStyle(g)) && y(_, D)
                          ? _[D]
                          : ''),
                      B = 'data-snap-slider-align';
                    return F && 0 > F.indexOf('none')
                      ? F
                      : ((P = p) && (P = P.closest('['.concat(B, ']')))
                          ? P.getAttribute(B)
                          : '') || 'start';
                  },
                },
                {
                  key: 'getSlide',
                  value: function (p) {
                    return (p = this.getIndexNumber(p)), this.slides[p - 1];
                  },
                },
                {
                  key: 'getCurrentSlide',
                    ...
                    return (
                      (g =
                        p === 'first'
                          ...
                          : parseInt(p, 10) || -1),
                      this.loop
                        ? (g < 1 && (g = this.slides.length),
                          g > this.slides.length && (g = 1))
                        : (g < 1 || g > this.slides.length) && (g = -1),
                      g || 1
                    );
                  },
                },
                      ...
                      _.indexOf('center') >= 0
                        ? ((P =
                          ...
                          (D =
                            ...
                            g.offsetWidth / 2))
                        : _.indexOf('end') >= 0 &&
                          ((P = p.offsetTop - g.offsetHeight + p.offsetHeight),
                          (D = p.offsetLeft - g.offsetWidth + p.offsetWidth)),
                      {
                        top: (P = S(P, 0, g.scrollHeight)),
                        left: (D = S(D, 0, g.scrollWidth)),
                      }
                    );
                  },
                },
                {
                  ...
                  value: function (p) {
                    ...
                    var P = this.getIndexNumber(p);
                    if (!g.force && P === this.current) return !1;
                    var D = this.getSlide(P);
                    if (!D) return !1;
                    var F = this.getScrollOffset(D),
                      B = F.top,
                      N = F.left;
                    return (
                      g.immediate
                        ? typeof this.container.scroll == 'function' &&
                          this.container.scroll({ top: B, left: N })
                ...
                },
                {
                  ...
                  value: function (p, g) {
                    var _ = this;
                    (p = v(p)).forEach(function (P) {
                      P.setAttribute(
                        'data-snap-slider-goto',
                        _.buildGoto(
                          P.closest(
                            '[data-snap-slider], [data-snap-slider-nav]'
                          ),
                          g
                        )
                      );
                ...
                },
                {
                  ...
                  value: function (p) {
                    ...
                    this.transition = {
                      from: this.current,
                      to: p,
                      diff: Math.abs(p - this.current),
                    };
                    var _ = this.transition.to;
                    this.checkTransition && clearTimeout(this.checkTransition),
                      (this.checkTransition = setTimeout(function () {
                        g.transition.to === _ && g.stopTransition();
                      }, 1e3));
                  },
                },
                {
                ...
                },
                {
                  ...
                  value: function (p) {
                    return (
                      !!(p = v(p).shift()) &&
                      ((p.getAttribute('data-snap-slider-goto') || '').match(
                        /\bprev$/
                      ) ||
                        p.textContent.toLowerCase().match(this.terms.prev) ||
                        p.className.toLowerCase().match(this.terms.prev))
                    );
                  },
                },
                {
                  ...
                  value: function (p) {
                    return (
                      ...
                      ((p.getAttribute('data-snap-slider-goto') || '').match(
                        ...
                        p.textContent.toLowerCase().match(this.terms.next) ||
                        p.className.toLowerCase().match(this.terms.next))
                    );
                  },
                },
                {
                  ...
                  value: function (p) {
                    return (
                      !C.isRelative(p) &&
                      this.getIndexNumber(p) === this.current
                    );
                  },
                },
                {
                  ...
                  value: function (p) {
                    ...
                    var P = v(
                        (_ = w(
                          { container: '', buttons: p, prev: '', next: '' },
                          _
                        )).buttons,
                        _.container
                      ),
                      D = v(_.prev, _.container),
                      F = v(_.next, _.container);
                    D.forEach(function (N) {
                      return (
                        N.hasAttribute('data-snap-slider-goto') ||
                        g.setGoto(D, 'prev')
                      );
                    }),
                    ...
                    return (
                      P.forEach(function (N) {
                        if (N.hasAttribute('data-snap-slider-goto'))
                          return null;
                        if (g.isPrevButton(N)) return g.setGoto(N, 'prev');
                        if (g.isNextButton(N)) return g.setGoto(N, 'next');
                        var R =
                          parseInt(
                            N.textContent.replace(/.*\b(\d+)\b.*/, '$1'),
                            10
                          ) || B;
                        return (B = R + 1), g.setGoto(N, R);
                      }),
                      this.updateButtons(),
                      !0
                    );
                ...
                },
                {
                  ...
                  value: function () {
                    ...
                    return h('[data-snap-slider-nav]').filter(function (g) {
                      return g.getAttribute('data-snap-slider-nav') === p.id;
                    });
                  },
                },
                {
                  ...
                  value: function () {
                    ...
                    return h('[data-snap-slider-goto]').filter(function (g) {
                      return C.getButtonTarget(g).sliderID === p.id;
                    });
                  },
                },
                {
                  ...
                  value: function () {
                    var p = this;
                    this.current &&
                      this.getButtons().forEach(function (g) {
                        var _ = C.getButtonTarget(g).index;
                        p.isCurrent(_)
                          ...
                          !p.loop &&
                            C.isRelative(_) &&
                            ((_ === 'prev' && p.current === 1) ||
                            (_ === 'next' && p.current === p.slides.length)
                ...
                },
                {
                  ...
                  value: function () {
                    var p = this;
                    this.slides.forEach(function (g, _) {
                      _ === p.current - 1
                        ? (g.classList.add('is-current'),
                          g.removeAttribute('aria-hidden'),
                          h('[data-snap-slider-tabindex]', g).forEach(function (
                            P
                          ) {
                        ...
                        : (g.classList.remove('is-current'),
                          ...
                          s()(g).forEach(function (P) {
                            P.setAttribute('tabindex', '-1'),
                              P.setAttribute('data-snap-slider-tabindex', '');
                          }));
                    });
                  },
                },
                  ...
                  value: function (p) {
                    ...
                    return (
                      ...
                      (P.forEach(function (D) {
                        D.setAttribute('data-snap-slider-nav', g.id);
                        var F =
                            D.getAttribute('data-snap-slider-buttons') ||
                            _.buttons ||
                            'button',
                          B = _.prev || D.getAttribute('data-snap-slider-prev'),
                          N = _.next || D.getAttribute('data-snap-slider-next');
                        g.addGotoButtons({
                          container: D,
                          buttons: F,
                          prev: B,
                ...
                },
                {
                  ...
                  value: function () {
                    var p = this;
                    return this.slides.reduce(function (g, _, P) {
                      P += 1;
                      var D = p.getScrollOffset(_),
                        F = {
                          index: P,
                          slide: _,
                          diff: {
                            top: Math.abs(p.container.scrollTop - D.top),
                            left: Math.abs(p.container.scrollLeft - D.left),
                          },
                        };
                      return g
                        ? F.diff.left <= g.diff.left && F.diff.top <= g.diff.top
                          ? F
                          : g
                        : F;
                    }, !1);
                ...
                },
                {
                  ...
                  value: function () {
                    ...
                    (this.scrollListener = u()(function (g) {
                      ...
                      p.transition ||
                        _.index === p.current ||
                        ((p.current = _.index), p.fireEvent('change', g)),
                        p.scrolling ||
                          ((p.scrolling = !0), p.fireEvent('scroll.start', g)),
                        p.fireEvent('scroll', g);
                    }, 250)),
                      (this.scrollEndListener = d()(function (g) {
                        (p.scrolling = !1),
                          p.fireEvent('scroll.end', g),
                          p.stopTransition();
                      }, 100)),
                      (this.arrowKeyListener = u()(function (g) {
                        if (!g.defaultPrevented) {
                          var _ =
                              ['Up', 'ArrowUp', 'Left', 'ArrowLeft'].indexOf(
                                g.key
                              ) >= 0,
                            P =
                              [
                          ...
                          (_ || P) &&
                            (p.goto(P ? 'next' : 'prev', null, g),
                            g.preventDefault());
                        }
                      }, 250)),
                      (this.focusListener = function (g) {
                        var _, P;
                        (p.scrolling && !p.transition) ||
                          (p.slides.forEach(function (D, F) {
                            D.contains(g.target) && ((_ = D), (P = F + 1));
                          }, null),
                          _ && p.goto(P, null, g));
                      }),
                      (this.resizeObserver = {
                ...
                },
                {
                  key: 'destroy',
                  value: function () {
                    for (var p in (this.stopTransition(),
                    ...
                    this.callbacks))
                      y(this.callbacks, p) && (this.callbacks[p] = []);
                ...
                },
                {
                  ...
                  value: function () {
                    ...
                    delete g.on,
                      delete p.container,
                      delete p.id,
                      this.init(this.container, w(w({}, g), p));
                  },
                },
                {
                  key: 'resizeCallback',
                ...
                },
                {
                  ...
                  value: function (p, g) {
                    ...
                    if (
                      y(this.callbacks, p) &&
                      ((P = w({ focus: !0, ignoreCallbacks: !1 }, P)),
                      p === 'load' && this.hasLoaded(),
                      p === 'change' &&
                        (this.updateButtons(), this.updateSlides()),
                      P.focus && this.handleFocus(p, g),
                      !P.ignoreCallbacks)
                    ) {
                      g = g || {};
                      var D = [p];
                      y(this.callbacks, ''.concat(p, '.').concat(g.type)) &&
                        D.push(''.concat(p, '.').concat(g.type)),
                        D.forEach(function (F) {
                          _.callbacks[F].forEach(function (B) {
                            typeof B == 'function' && B(_, g);
                          });
                        });
                    }
                  },
                },
                {
                  ...
                  value: function (p, g) {
                    if (this.transition) {
                      if (g && p === 'change') {
                        var _ = g.target.closest('[data-snap-slider-goto]'),
                          P = C.getButtonTarget(_).index;
                        if (C.isRelative(P)) return;
                      }
                      this.transition.diff > 1 &&
                        p === 'scroll.end' &&
                        ((document.activeElement &&
                          ...
                          this.getCurrentSlide().focus({ preventScroll: !0 }));
                    }
                  },
                },
                {
        ...
        (window._SnapSliders = []),
          ...
          (x = function () {
            ...
            a(window),
              o.a.polyfill(),
              h('[data-snap-slider]').forEach(function (A) {
                return new O(A);
              }),
              (T = O.handleGoto),
              (L = document.querySelector('body')) &&
                L.addEventListener('click', function (A) {
                  for (
                    var C = L.querySelectorAll('[data-snap-slider-goto]'),
                      p = A.target,
                      g = 0,
                      _ = C.length;
                    g < _;
                    ...
                    for (var P = p, D = C[g]; P && P !== L; ) {
                      if (P === D) return T.call(D, A);
                      P = P.parentNode;
                    }
                });
          }),
    ...
    window.SnapSlider &&
      e.forEach((n) => {
        ...
        const {
          ...
          d = n.querySelector('.pf-slider');
        !i && updateSliderHeight(d, a, t, !0),
          new window.SnapSlider(d, {
            id: d.getAttribute('data-id'),
            slides: '.pf-slide',
            loop: r,
            on: {
              load: (l) => {
                initVisibleObservers(l),
                  resizeSlideObserver$1(d, a, t),
                  renderPaginationButtonsLiveView(l, a, t),
                  handleNavigationButtons$1(l, a, t),
                  handlePaginationButtons(l, a, t),
                  handleSlidesScrollStop(l, a, t),
                  o && handleAutoPlay$1(l, a, t);
              },
              'scroll.end': (l) => {
                !i && updateSliderHeight(d, a, t),
                  s !== 'none' && updateNavigationButtonsState(l, a, t),
                  c !== 'none' && updatePaginationButtonsState(l, a, t);
              },
            },
          });
      });
  ...
  function getMaxHeightOfActiveSlides(t) {
    const n = Array.from(t.querySelectorAll('.pf-slide.is-visible')).map((r) =>
      getTotalHeightOfActiveSlides(r, !0)
    );
    return Math.max(...n);
  }
  function updateSliderHeight(t, e, n, a = !1) {
    if (a) {
      const i = Array.from(t.querySelectorAll('.pf-slide'))
          .map((c) => getTotalHeightOfActiveSlides(c))
          .slice(0, e == null ? void 0 : e.slidesToShow[n]),
        s = Math.max(...i);
      t.style.height = `${s}px`;
  ...
  function shouldUpdateHeight(t) {
    ...
    for (let o = 0; o < (e == null ? void 0 : e.length); o++)
      if (
        a * o + OBSERVATIONAL_ERROR_VALUE >= r &&
        r >= a * o - OBSERVATIONAL_ERROR_VALUE
      )
        return !0;
    return !1;
  }
  function handleSlidesScrollStop(t, e, n) {
    ...
    for (
      let r = 0;
      r < a.length;
      r += Number(e == null ? void 0 : e.slidesToScroll[n])
    )
      a[r].style.scrollSnapStop = 'always';
  }
  function getTotalPaginationButtons(t, e, n) {
    const { slidesToShow: a, slidesToScroll: r } = e;
  ...
  const getNextSlideIndex = (t, e, n) => {
    ...
    return (
      i > o && (i = o),
      i == o && (i = o - (Number(r[n]) - 1)),
      i < o && i > o - Number(a[n]) && (i = o - Number(a[n]) + 1),
      i
    );
  };
  function handleNavigationButtons$1(t, e, n) {
    const {
      ...
      d = t.slides.length;
    (e == null ? void 0 : e.navStyle) !== 'none' &&
      updateNavigationButtonsState(t, e, n),
      c.addEventListener('click', function (l) {
        if ((l.stopPropagation(), !l.detail || l.detail === 1))
          if (t.current === d || Number(t.current) + Number(a[n]) - 1 === d)
            o && i !== 'none' && updatePaginationButtonsState(t, e, n, 1),
              o && t.goto(1);
          else {
      ...
      s.addEventListener('click', function (l) {
        if ((l.stopPropagation(), !l.detail || l.detail === 1))
          if (t.current === 1) {
            if (o) {
              const u = d - a[n] + 1;
              i !== 'none' && updatePaginationButtonsState(t, e, n, u),
                t.goto(u);
            }
          } else {
            let u = t.current - r[n];
  ...
  function handlePaginationButtons(t, e, n) {
    ...
    (e == null ? void 0 : e.paginationStyle) !== 'none' &&
      updatePaginationButtonsState(t, e, n),
      r == null ||
        r.forEach((o, i) => {
          o.onclick = function (s) {
            s.stopPropagation(),
              t.goto(i * (e == null ? void 0 : e.slidesToScroll[n]) + 1),
              r.forEach((c) => c.classList.remove('active')),
  ...
  function updatePaginationButtonsWithNumberOfVariantsRemainingState(t) {
    ...
    const a = n == null ? void 0 : n.querySelectorAll('.pagination-button'),
      ...
      i =
        n == null ? void 0 : n.querySelector('.first-media-pagination-button'),
      ...
      d =
        t.current === 1
          ? -1
          : t.current === e
          ? numberOfVisiblePaginationItemInNewLayout
          : Array.from(a).findIndex((m) => m.classList.contains('active')),
      l = c + d,
      u = t.current - l - 1 + d,
      f = (m) => {
        a.forEach((S, E) => {
          S.classList.toggle('active', E === m);
        });
      },
      h = (m, S) => {
        (r.querySelector('.number-media-prev').innerHTML = m),
          (o.querySelector('.number-media-next').innerHTML = S);
      },
      v = (m, S, E) => {
        m.classList.toggle(S, E);
      };
    u > numberOfVisiblePaginationItemInNewLayout - 1
      ? (f(numberOfVisiblePaginationItemInNewLayout - 1),
        t.current !== e
          ? h(
              t.current - numberOfVisiblePaginationItemInNewLayout,
              e - t.current
            )
          : h(e - numberOfVisiblePaginationItemInNewLayout - 1, 1))
      : u < 0
      ? (f(0),
        t.current !== 1
          ? h(
              t.current - 1,
              e - t.current - numberOfVisiblePaginationItemInNewLayout + 1
            )
          : h(1, e - numberOfVisiblePaginationItemInNewLayout - 1))
      : f(u),
      t.current !== 1 && t.current !== e
        ? (r.classList.remove('disabled'),
          o.classList.remove('disabled'),
          i.classList.add('disabled'),
          s.classList.add('disabled'))
        : (a.forEach((m) => m.classList.remove('active')),
          v(r, 'disabled', t.current === 1),
          v(o, 'disabled', t.current === e),
          v(i, 'disabled', t.current === e),
          v(s, 'disabled', t.current === 1),
          v(i, 'active', t.current === 1),
          v(s, 'active', t.current === e));
  }
  function handlePaginationButtonsWithNumberOfVariantsRemaining(t, e, n) {
    ...
    const o = r == null ? void 0 : r.querySelectorAll('.pagination-button'),
      ...
      c =
        r == null ? void 0 : r.querySelector('.first-media-pagination-button'),
      d = r == null ? void 0 : r.querySelector('.last-media-pagination-button'),
      l = getActivePagination(t, e, n);
    l === 0 &&
      (i.classList.add('disabled'),
      (i.querySelector('.number-media-prev').innerHTML = 1),
      c.classList.remove('disabled'),
      c.classList.add('active')),
      l === a - 1 &&
        (s.classList.add('disabled'),
        (s.querySelector('.number-media-next').innerHTML = 1),
  ...
  function updateNavigationButtonsState(t, e, n) {
    ...
    try {
      ...
      if (!(e != null && e.loop)) {
        if (
          ((a = t == null ? void 0 : t.slides) == null ? void 0 : a.length) ===
          ((r = e == null ? void 0 : e.slidesToShow) == null ? void 0 : r[n])
        ) {
          (c.style.visibility = 'hidden'), (s.style.visibility = 'hidden');
          return;
        }
        i >= o - 1
          ? ((c.style.visibility = 'hidden'), (s.style.visibility = ''))
          : i === 0
          ? ((c.style.visibility = ''), (s.style.visibility = 'hidden'))
          : ((c.style.visibility = ''), (s.style.visibility = ''));
      }
    } catch (o) {
  ...
  function initVisibleObservers(t) {
    const { container: e, slides: n } = t,
      a = new IntersectionObserver(
        (r, o) => {
          r.forEach((i) => updateVisibleClasses(i));
        },
    ...
    const { slides: r } = t,
      ...
      u = () => {
        a = setInterval(() => {
          ...
          const { top: v, left: y } = t.getScrollOffset(r[h]);
          t.container.scroll({ top: v, left: y, behavior: 'smooth' });
        }, i);
      };
    u(),
      s &&
        (t.container.addEventListener('mouseover', function () {
      ...
      for (const a of n) {
        ...
        if (!isElementInsideHiddenTabs(a)) {
          const i = a.querySelector('[data-pf-type*="MediaMain"]'),
            ...
            { navStyle: f, paginationStyle: h } = l,
            { navStyle: v, listLayout: y = {} } = u,
            m = y[e] === LIST_LAYOUT.SLIDE;
          c.querySelectorAll('.pf-slide-main-media');
          const S = getIndexOfUserDefaultSource(a);
          S === 0 && o.classList.remove('product-media-loading');
          const E = new window.SnapSlider(c, {
              id: c.getAttribute('data-id'),
              slides: '.pf-slide-main-media',
              start: S,
              on: {
                load: (w) => {
                  handleAddStyleScrollSnapType(w),
                    initVisibleObservers(w),
                    handleSlidesScrollStop(w, l, e),
                    handleHoverMedia$1(w, i, l),
                    handleOpenMediaGallery(w, l, a),
                    handleNavigationButtons(w, l, e, !1, sliderList$1),
                    handlePaginationButtons(w, l, e),
                    ['pagination-style-2', 'pagination-style-3'].includes(h) &&
                      w.slides.length >
                        maximumQuantityToUseOldLayoutPagination &&
                      handlePaginationButtonsWithNumberOfVariantsRemaining(
                        w,
                        l,
                        e
                      ),
                    handleClickMediaMain(w),
                    updateActiveSlide(w),
                    handleScrollToDefaultVariant(w, sliderList$1, S, l);
                },
                'scroll.end': (w) => {
                  updateActiveSlide(w, k),
                    handleChangeMainMedia(w, a),
                    f !== 'none' && updateNavigationButtonsState(w, l, e),
                    h !== 'none' && updatePaginationButtonsState(w, l, e),
                    ['pagination-style-2', 'pagination-style-3'].includes(h) &&
                      w.slides.length >
                        maximumQuantityToUseOldLayoutPagination &&
                      updatePaginationButtonsWithNumberOfVariantsRemainingState(
                        w
                      ),
                    v !== 'none' && m && updateNavigationButtonsState(k, u, e),
                    o.classList.remove('product-media-loading');
                },
              },
            }),
            k = new window.SnapSlider(d, {
              id: d.getAttribute('data-id'),
              slides: '.pf-slide-list-media',
              on: {
                load: (w) => {
                  m && handleAddStyleScrollSnapType(w),
                    m && initVisibleObservers(w),
                    m && handleSlidesScrollStop(w, u, e),
                    m && handleNavigationButtons(E, u, e, !0, w),
                    handleClickOnSliderMode(w, l, u, E, e),
                    updateActiveSlide(w),
                    handleScrollToDefaultVariant(E, w, S, l);
                },
                'scroll.end': (w) => {
                  v !== 'none' && m && updateNavigationButtonsState(w, u, e),
                    o.classList.remove('product-media-loading');
                },
              },
            });
          window.mainSlider2[r] = E;
        }
      }
    ...
    t == null ||
      t.slides.forEach((i, s) => {
        i.addEventListener('mousedown', (c) => {
          var u, f;
          c.stopPropagation(),
            (n == null ? void 0 : n.paginationStyle) !== 'none' &&
              updatePaginationButtonsState(a, n, r, s + 1),
            (u = t.slides.find((h) => h.getAttribute('data-active'))) == null ||
              u.removeAttribute('data-active'),
            t.slides[s].setAttribute('data-active', !0);
          const d = n.navStyle !== 'none',
            l =
              ((f = n.listLayout) == null ? void 0 : f[r]) ===
              LIST_LAYOUT.SLIDE;
          d && l && updateNavigationButtonsState(t, n, r),
            o
              ? updateMainMediaWhenTurnOffSlideshow2(a, t, s, !1)
              : a.goto(s + 1);
        });
      });
  }
  function handleFindModelOrVideo$1(t) {
    return t
      ...
      .querySelector('.pf-media-wrapper');
  }
  ...
  function handleHoverMedia$1(t, e, n) {
    ...
    e.onmouseenter = function () {
      ...
      const d = a.length - 1;
      if (
        n.hoverAction === HOVER_ACTION.HOVER &&
        a.length > 1 &&
        !handleFindModelOrVideo$1(a)
      )
        switch (n.onHover) {
          case ON_HOVER.NEXT_IMAGE:
            for (
              c = i < d ? i + 1 : 0;
              a[c].getAttribute('data-media-type') !== 'image' || s === c;
          ...
          case ON_HOVER.LAST_IMAGE:
            for (
              c = d;
              (a[c].getAttribute('data-media-type') !== 'image' || c === 0) &&
              ((c = c === i ? c - 2 : c - 1), !(c < 0));
          ...
          case ON_HOVER.RANDOM_IMAGE:
            do c = Math.floor(Math.random() * a.length);
            while (a[c].getAttribute('data-media-type') !== 'image' || s === c);
            changeProductMediaOnly$1(a, i, c), (s = c);
            break;
          case ON_HOVER.ALL_IMAGE:
            ...
            for (
              ;
              (c === i || a[c].getAttribute('data-media-type') !== 'image') &&
              ((c = o(c, d)), u++, u !== a.length);
        ...
        n.hoverAction === HOVER_ACTION.MAGNIFIER && handleMagnifierMedia$1(t);
      e.onmouseleave = function () {
        n.hoverAction === HOVER_ACTION.HOVER &&
          (n.onHover === ON_HOVER.ALL_IMAGE
            ? changeProductMediaOnly$1(a, r, i)
            : changeProductMediaOnly$1(a, s, i),
          (moved = !1));
      };
    };
  }
  let firstLoading = !1;
  const mainSlider = (window.mainSlider = {}),
  ...
  async function handleShopifyProductMedia(t) {
    let e;
    t
      ? (e = t.querySelectorAll('[data-pf-type="ProductMedia"]'))
      : (e = document.querySelectorAll('[data-pf-type="ProductMedia"]'));
    for (const n of e) {
      const a = getProductMediaId(n);
      isElementInsideHiddenTabs(n) ||
        (await initSlider(n, (r, o, i, s) => {
          i && handleUpdateImageHeight(n, i),
            handleChangeMainMedia$1(n, r, o, i),
            handleClickMediaItem(n, o, i, s).catch((c) => console.log(c)),
            handleHoverMedia(n, r, o),
            handleOpenMediaGallery$1(n, o),
            (window.mainSlider[a] = o),
            (window.subSlider[a] = i),
            handleDefaultSource(n),
            i && handleShowArrow(n);
        }));
    }
    shouldLoadShopifyXR() && loadShopifyXr();
  ...
  function shouldLoadShopifyXR() {
    ...
    return !t || !e.length || !checkARCompatible(getMobileOS())
      ? !1
      : (e.forEach((a) => {
  ...
  function loadShopifyXr() {
    function t() {
      if (!window.ShopifyXR)
        ...
        document
          .querySelectorAll(
            '.pf-ar-btn-wrapper.pf-ar-compatible [id^="ModelJSON-"]'
          )
          .forEach((n) => {
  ...
  function handleShowArrow(t) {
    ...
    r.querySelectorAll('.splide__track .splide__list .splide__slide').length <=
      n[a] && o.classList.add('pf-hidden');
  }
  function handleUpdateImageHeight(t, e) {
    ...
    if (e.Components.Elements.getSlides().length) {
      const i = e.options.perPage,
        c =
          parseInt(e.Components.Elements.getSlide(0).slide.style.width) -
          (parseInt(a[r]) * (i - 1)) / i;
      (e.options = { ...e.options, fixedWidth: c, fixedHeight: c }),
        c && e.emit('updated');
    }
  }
  function handleDefaultSource(t) {
    ...
    if (o === 'variant' && a && (a == null ? void 0 : a.featured_media)) {
      const l = (s == null ? void 0 : s.Components.Elements.getSlides()).find(
          (f) => {
  ...
  async function initSlider(t, e) {
    if (!window.Splide) {
      ...
      document.head.appendChild(i),
        i.appendChild(document.createTextNode(dataCss)),
        await loadJS(
    ...
    if ((onMounted(n, o), onPaginationMounted(o), o.mount(), r))
      if (r.getAttribute('data-splide')) {
        ...
        r.parentNode.classList.remove('pf-splide-loading'), e(n, o, null, r);
    else e(n, o);
  }
  function handleOpenMediaGallery$1(t, e) {
    (getFrontEndSettings(t) || {}).clickAction === 2 &&
      e.on('click', (a) => {
        if (a.slide.getAttribute('data-media-type') === 'image') {
          const r = t.getAttribute('data-product-id'),
            o = pageflyProducts[r].media;
          window.mediaGallery[r] ||
            (window.mediaGallery[r] = new MediaGallery(o, document.body)),
            window.mediaGallery[r].show(a.index);
        }
      });
  }
  function updateSplideSettings(t, e) {
    isTouchDevice &&
      ...
      t.emit('updated'),
      console.log('splide update', t.options));
  }
    ...
    n.on('move', (r) => {
      if ((isNaN(r) && (r = 0), isTouchDevice)) a && a.go(r);
        ...
        const o =
          e == null
            ? void 0
            : e.parentNode.querySelector('[data-pf-type="ProductMediaList"]');
        if (!o) return;
        const i = Array.from(
          o == null ? void 0 : o.querySelectorAll('[data-pf-type="MediaItem"]')
        );
      ...
      n.on('moved', (r) => {
        ...
        if (
          (n && e && firstLoading && setDynamicHeight(n, e, r),
          (firstLoading = !0),
          ((c = t == null ? void 0 : t.className) == null
            ? void 0
            : c.includes(ACCEPT_UPDATE_VARIANT_BY_IMAGE)) &&
            (!n.root.getAttribute('data-no-move-again') &&
              updateVariantByImage(t, o),
            n.root.removeAttribute('data-no-move-again')),
          i === 'model')
        ) {
          const d = o.slide.getAttribute('data-media-id');
          updateARButton(t, d), showARButton(t);
        } else hideARButton(t);
        if (i === 'model' && !isTouchDevice)
          o.slide.querySelector('.pf-mask').classList.add('pf-hidden');
      ...
      n.on('click', (r) => {
        ...
        if (o === 'model' && isTouchDevice) {
          ...
          n.options.drag
            ? i.classList.add('pf-hidden')
            : i.classList.remove('pf-hidden'),
            updateSplideSettings(n, { drag: !n.options.drag });
          ...
          c.getAttribute('data-play') === 'false'
            ? (c.setAttribute('data-play', 'true'),
              togglePlayExternalVideo('play', s, d))
            : (c.setAttribute('data-play', 'false'),
              togglePlayExternalVideo('pause', s, d));
        } else {
          if (n.options.drag) return;
          updateSplideSettings(n, { drag: !0 });
        }
  ...
  function setDynamicHeight(t, e, n) {
    const a = t.Components.Elements.getSlide(n),
      r =
        (a == null ? void 0 : a.slide) ||
        e.querySelector('.splide__slide.is-active'),
      o = getDevice(),
      i = {
  ...
  function onMounted(t, e) {
    e.on('mounted', () => {
      ...
      isTouchDevice
        ? updateSplideSettings(e, { drag: !0 })
        : n.slide.getAttribute('data-media-type') === 'model' &&
          n.slide.querySelector('.pf-mask').classList.add('pf-hidden'),
        t.parentNode.style.removeProperty('--ratio'),
        t.parentNode.classList.remove('pf-splide-loading');
    });
  }
  function onPaginationMounted(t) {
    t.on('pagination:mounted', (e) => {
      e.items.length === 1 && (e.list.style.visibility = 'hidden'),
        (e.list.querySelectorAll('li') || []).forEach((a) => {
    ...
    !n && a
      ? (() => {
          Array.from(
            a == null
              ? void 0
              : a.querySelectorAll('[data-pf-type="MediaItem"]')
          ).forEach((s, c) => {
      ...
      : (() => {
          !n ||
            ...
            n.on('click', (i) => {
              ...
              (l =
                (d =
                  (c = n == null ? void 0 : n.Components) == null
                    ? void 0
                    : c.Elements) == null
                  ? void 0
                  : d.slides[s]) == null || l.removeAttribute('data-active'),
                i.slide.setAttribute('data-active', 'true'),
                e.go(i.index),
                n.go(i.index);
            }),
            n.on('move', (i) => {
  ...
  function handleFindModelOrVideo(t) {
    return t.Components.Elements.track
      .querySelector('.is-active')
      .querySelector('.pf-media-wrapper');
  }
  function handleMagnifierMedia(t, e) {
    (getFrontEndSettings(t) || {}).hoverAction === 1 &&
      e.Components.Elements.slides.forEach((r) => {
        r.getAttribute('data-media-type') === 'image' && initZoomImage2(r);
      });
  }
  function changeProductMediaOnly(t, e, n) {
    t[e].classList.remove('is-active', 'is-visible'),
      t[n].classList.add('is-active', 'is-visible');
  }
  ...
  function handleHoverMedia(t, e, n) {
    const a = getFrontEndSettings(t) || {},
      r = [];
    if (
      (e.querySelectorAll('.splide__slide').forEach((s, c) => {
        s.getAttribute('data-media-type') === 'image' && r.push(c);
      }),
      isTouchDevice || a.hoverAction === 0)
    )
      return;
    let o = n.index;
    ...
    e.onmouseenter = function () {
      ...
      if (a.hoverAction === 2 && r.length > 1) {
        ...
        a.onHover === 0
          ? ((c =
              r.indexOf(defaultIndexActive) + 1 <= d
                ? r.indexOf(defaultIndexActive) + 1
                : 0),
            changeProductMediaOnly(s, o, r[c]))
          : a.onHover === 1
          ? r.includes(defaultIndexActive) &&
            ((c = r.indexOf(defaultIndexActive)),
            changeProductMediaOnly(s, o, r[c === 0 ? d : c - 1]))
          : a.onHover === 2
          ? ((c = Math.floor(Math.random() * r.length)),
            changeProductMediaOnly(s, o, r[c]))
          : ((c = i(o, d)),
            c === defaultIndexActive && (c = i(c, d)),
            changeProductMediaOnly(s, n.index, r[c]),
            (o = c));
      } else handleMagnifierMedia(t, n);
      e.onmouseleave = function () {
        a.hoverAction === 2 &&
          (a.onHover === 1
            ? changeProductMediaOnly(s, r[c === 0 ? d : c - 1], o)
            : changeProductMediaOnly(s, r[c], n.index));
      };
    };
  }
  const updateImageByVariant = (t, e) => {
      ...
      if (!!r && n) {
        const c = (n == null ? void 0 : n.Components.Elements.getSlides()).find(
            (l) => l.slide.getAttribute('data-media-id') === r.toString()
          ),
          d = c == null ? void 0 : c.index;
        isNaN(d) ||
          ((i = n.root) != null &&
            i.closest(`.${ACCEPT_UPDATE_VARIANT_BY_IMAGE}`) &&
            n.root.setAttribute('data-no-move-again', 'true'),
          n.go(d),
          a && a.go(d));
      }
    },
    updateVariantByImage = (t, e) => {
      ...
      const n = parseInt(t.getAttribute('data-product-id')),
        ...
        s =
          (f =
            (u = i == null ? void 0 : i.pfCurrentVariant) == null
              ? void 0
              : u[o || OUTSIDE_BOX_KEY]) == null
            ? void 0
            : f.id,
        c = (i == null ? void 0 : i.variants) || [],
        d =
          (v =
            (h = e == null ? void 0 : e.slide) == null ? void 0 : h.dataset) ==
          null
            ? void 0
            : v.mediaId,
        l = c.find((y) => {
          var m;
          return ((m = y.featured_media) == null ? void 0 : m.id) === d;
        });
      s !== (l == null ? void 0 : l.id) &&
        l &&
        updateByCurrentVariant(n, l, r, !1, !0, t);
    },
    ...
    showARButton = (t) => {
      ...
      e &&
        e.classList.contains('pf-ar-compatible') &&
        e.classList.remove('pf-ar-btn-hidden');
    },
    hideARButton = (t) => {
      const e = t == null ? void 0 : t.querySelector('.pf-ar-btn-wrapper');
  ...
  function handleChangeMainMedia(t, e) {
    ...
    if (
      (i = e == null ? void 0 : e.className) == null
        ? void 0
        : i.includes(ACCEPT_UPDATE_VARIANT_BY_IMAGE)
    ) {
      const s = {
        slide: {
          dataset: {
      ...
      !t.container.getAttribute('data-no-move-again') &&
        updateVariantByImage(e, s),
        t.container.removeAttribute('data-no-move-again');
    }
    if (r === 'model') {
      const s = a.getAttribute('data-media-id');
      updateARButton(e, s),
        showARButton(e),
        isTouchDevice || a.querySelector('.pf-mask').classList.add('pf-hidden');
    } else hideARButton(e);
    if (r === 'external_video') {
  ...
  function handleClickMediaMain(t) {
    const { slides: e } = t;
    e.forEach((n) => {
      n.registeredClickEvent ||
        ((n.registeredClickEvent = !0),
        n.addEventListener('click', () => {
          const a = n.getAttribute('data-media-type');
          if (a === 'model' && isTouchDevice) {
            const r = n.querySelector('.pf-mask');
            r.classList.contains('pf-hidden')
              ? r.classList.remove('pf-hidden')
              : r.classList.add('pf-hidden');
            ...
            i.getAttribute('data-play') === 'false'
              ? (i.setAttribute('data-play', 'true'),
                togglePlayExternalVideo('play', o, s))
              : (i.setAttribute('data-play', 'false'),
                togglePlayExternalVideo('pause', o, s));
          }
        }));
    });
  }
    ...
    const o = a
      ...
      { disableSlideshow: c } =
        getFrontEndSettings(
          (d = t.container) == null
            ? void 0
            : d.closest('[data-pf-type*="MediaMain"]')
        ) || {};
    (e == null ? void 0 : e.navStyle) !== 'none' &&
      updateNavigationButtonsState(a ? r : t, e, n),
      s == null ||
          ...
          if ((l.stopPropagation(), !l.detail || l.detail === 1))
            if (c && a && r) {
              const { slides: u } = r,
                f = u.length;
              let h =
                u.findIndex((v) => v.getAttribute('data-active')) +
                (e == null ? void 0 : e.slidesToScroll[n]);
              h >= f && (h = f - 1),
                updateMainMediaWhenTurnOffSlideshow2(t, r, h, !0);
            } else {
              ...
              if (t.current < u || !l.detail || l.detail === 1) {
                ...
                f > u && (f = u),
                  (e == null ? void 0 : e.paginationStyle) !== 'none' &&
                    updatePaginationButtonsState(t, e, n, f),
                  t.goto(f);
              }
            }
        }),
        ...
        i.addEventListener('click', (l) => {
          if ((l.stopPropagation(), !l.detail || l.detail === 1)) {
            if (c && a && r) {
              const { slides: u } = r;
              let f =
                u.findIndex((h) => h.getAttribute('data-active')) -
                (e == null ? void 0 : e.slidesToScroll[n]);
              f < 0 && (f = 0),
                updateMainMediaWhenTurnOffSlideshow2(t, r, f, !0);
            } else if (t.current > 1) {
              let u = t.current - (e == null ? void 0 : e.slidesToScroll[n]);
              u < 1 && (u = 1),
                (e == null ? void 0 : e.paginationStyle) !== 'none' &&
                  updatePaginationButtonsState(t, e, n, u),
                t.goto(u);
            }
          }
        });
  ...
  function handleOpenMediaGallery(t, e, n) {
    const { slides: a } = t,
      r = e == null ? void 0 : e.disableSlideshow;
    (e == null ? void 0 : e.clickAction) === CLICK_ACTION.SHOW_FULLSCREEN &&
      a.forEach((o, i) => {
        o.addEventListener('click', () => {
          if (o.getAttribute('data-media-type') === 'image') {
            const s = n.getAttribute('data-product-id'),
              c = pageflyProducts[s].media;
            window.mediaGallery[s] ||
              (window.mediaGallery[s] = new MediaGallery(c, document.body));
            let d = i;
            if (r) {
  ...
  function handleOriginalRatioMainMediaStyleWhenTurnOffSlideshow2(t, e) {
    ...
    if (getCurrentRatio(e.ratio) === 'original') {
      const o = t.container,
        ...
        c =
          (a =
            o == null ? void 0 : o.closest('[data-pf-type="ProductMedia2"]')) ==
          null
            ? void 0
        ...
        l = d == null ? void 0 : d[0];
      l &&
        ((i.style.aspectRatio = l.preview_image.aspect_ratio),
        (i.style.objectFit = 'contain'),
        (i.style.objectPosition = 'center top'),
        (s.style.paddingBottom = `${100 / l.preview_image.aspect_ratio}%`));
    }
  }
  function handleScrollToDefaultVariant(t, e, n, a) {
    a.disableSlideshow
      ? (handleOriginalRatioMainMediaStyleWhenTurnOffSlideshow2(t, a),
        n > 0 && updateMainMediaWhenTurnOffSlideshow2(t, e, n, !0),
        updateActiveSlide(e))
      : t.goto(n + 1);
  }
  function updateActiveSlide(t, e) {
    ...
    n == null ||
      n.forEach((r, o) => {
        r.classList.contains('is-current')
          ? (r.setAttribute('data-active', !0),
            a &&
              (a[o].setAttribute('data-active', !0),
              a[o].classList.contains('is-visible') || e.goto(o + 1)))
          : (r.removeAttribute('data-active'),
            a && a[o].removeAttribute('data-active'));
      });
  }
  ...
  function updateImageByVariant2(t, e) {
    ...
    const n = mainSlider2[t],
      ...
      r = (o = e == null ? void 0 : e.featured_media) == null ? void 0 : o.id;
    if (!!r && n) {
      const l = getFrontEndSettings(
          (i = n.container) == null
            ? void 0
            : i.closest('[data-pf-type*="MediaMain"]')
        ),
        { disableSlideshow: u } = l || {};
      if (u) {
        const f =
            (c =
              (s = n.container) == null
                ? void 0
                : s.closest('[data-pf-type="ProductMedia2"]')) == null
              ? void 0
              : c.getAttribute('data-product-id'),
          v = pageflyProducts[f].media.findIndex((y) => y.id === r);
        updateMainMediaWhenTurnOffSlideshow2(n, a, v, !0);
      } else {
        const { slides: f, current: h } = n,
          v =
            f.findIndex(
              (y) => y.getAttribute('data-media-id') === r.toString()
            ) + 1;
        !isNaN(v) &&
          Number(v) !== Number(h) &&
          ((d = n.container) != null &&
            d.closest(`.${ACCEPT_UPDATE_VARIANT_BY_IMAGE}`) &&
            n.container.setAttribute('data-no-move-again', 'true'),
          n.goto(v));
      }
    }
  }
  function updateMainMediaWhenTurnOffSlideshow2(t, e, n, a = !1) {
    ...
    a &&
      e &&
      (e.slides.forEach((y, m) => {
        const S = y.getAttribute('data-active') === 'true';
        m === n && !S
          ? y.setAttribute('data-active', !0)
          : m !== n && S && y.removeAttribute('data-active');
      }),
      e.goto(n + 1));
    const r = t.container,
      ...
      c =
        (u =
          r == null ? void 0 : r.closest('[data-pf-type="ProductMedia2"]')) ==
        null
          ? void 0
          : u.getAttribute('data-product-id'),
      l = pageflyProducts[c].media[n];
    if (
      (s.setAttribute('data-media-id', l.id),
      s.setAttribute('data-media-type', l.media_type),
      l.media_type === 'image')
    ) {
      const y =
        (f = o.src) != null && f.includes('https')
          ? (h = o.src) == null
            ? void 0
            : h.split('https:')[1]
          : o.src;
      (o.alt = l.alt),
        (o.width = l.width),
        (o.height = l.height),
        (o.srcset = (v = o.srcset) == null ? void 0 : v.replaceAll(y, l.src)),
        (o.src = l.src),
      ...
      switch (
        ...
        case 'model':
          const y = l.sources[0].url,
            m = l.preview_image.src;
          y && loadModelViewer(i, y, m, l.alt);
          break;
        case 'video':
          const S = l.sources[0].url;
          S && loadVideo(i, S);
          break;
        case 'external_video':
          const { host: E, external_id: k } = l;
          E && k && loadExternalVideo(i, E, k);
          break;
      }
  }
    ...
    if (n !== null) {
      const i = n.closest('form').getAttribute('data-productid'),
        s =
          (a = e[i]) == null
            ? void 0
            : a.quantity.find((l) => l.includes(t == null ? void 0 : t.id)),
        c = t && s.split(`${t.id}:`)[1],
        d = (r = window.BISConfig) == null ? void 0 : r.instock_qty_level;
      !(t != null && t.available) || d > c
  ...
  function handlePushOwlBackInStock(t) {
    ...
    const e = document.querySelectorAll(
      '[data-pf-type="PushOwl"].js-pushowl--bis'
    );
    if (e.length > 0) {
      let n = {};
      ...
      if (!t) n = r.variants[0];
        ...
        o
          ? (n = o)
          : (n = r.selected_or_first_available_variant || r.variants[0]);
      }
      console.log(n),
        e.forEach((o) => {
          const i = getFrontEndSettings(o);
          n.available
            ? (console.log('hide bis', o), (o.style.display = 'none'))
            : window.pushowl.isProductVariantSubscribed('back_in_stock', n.id)
            ? i.postDisplay === 'none'
              ? (o.style.display = 'none')
              : (setButtonText(o, i.postSubscriptionText),
                (o.style.display = i.postDisplay))
            : (console.log('show bis'),
              setButtonText(o, i.btnText),
              (o.style.display = i.preDisplay));
        });
    }
  }
  function handlePushOwlPriceDrop(t) {
    const e = document.querySelectorAll(
      '[data-pf-type="PushOwl"].js-pushowl--pd'
    );
    if (e.length > 0) {
      let n = {};
      const a = e[0].getAttribute('data-product-id'),
      ...
      e.forEach((o) => {
        const i = getFrontEndSettings(o);
        window.pushowl.isProductVariantSubscribed('price_drop', n.id)
          ? i.postDisplay === 'none'
            ? (o.style.display = 'none')
            : (setButtonText(o, i.postSubscriptionText),
              (o.style.display = i.postDisplay))
          : (setButtonText(o, i.btnText), (o.style.display = i.preDisplay));
      });
    }
  }
  ...
  function isValid(t) {
    if ((requiredArgs(1, arguments), !isDate(t) && typeof t != 'number'))
      return !1;
    var e = toDate(t);
    ...
    var n = toDate(t),
      ...
      c =
        r.firstWeekContainsDate == null
          ? s
          : toInteger(r.firstWeekContainsDate);
    if (!(c >= 1 && c <= 7))
    ...
    var n = e || {},
      ...
      i =
        n.firstWeekContainsDate == null
          ? o
          : toInteger(n.firstWeekContainsDate),
  ...
  var dayPeriodEnum = {
    ...
    formatters = {
      ...
      b: function (t, e, n) {
        ...
        switch (
          (a === 12
            ? (r = dayPeriodEnum.noon)
            : a === 0
            ? (r = dayPeriodEnum.midnight)
            : (r = a / 12 >= 1 ? 'pm' : 'am'),
          e)
        ) {
          case 'b':
      ...
      B: function (t, e, n) {
        ...
        switch (
          (a >= 17
            ? (r = dayPeriodEnum.evening)
            : a >= 12
            ? (r = dayPeriodEnum.afternoon)
            : a >= 4
            ? (r = dayPeriodEnum.morning)
            : (r = dayPeriodEnum.night),
          e)
        ) {
          case 'B':
          case 'BB':
  ...
  function dateTimeLongFormatter(t, e) {
    ...
    return o
      ...
      .replace('{{time}}', timeLongFormatter(r, e));
  }
    ...
    var h = getTimezoneOffsetInMilliseconds(f),
      ...
      m = a
        ...
        .map(function (S) {
          var E = S[0];
          if (E === 'p' || E === 'P') {
            var k = longFormatters$1[E];
            return k(S, o.formatLong, y);
          }
          return S;
        })
        .join('')
        ...
        .map(function (S) {
          if (S === "''") return "'";
          var E = S[0];
          if (E === "'") return cleanEscapedString(S);
          var k = formatters$1[E];
          if (k)
            return (
              !r.useAdditionalWeekYearTokens &&
                isProtectedWeekYearToken(S) &&
                throwProtectedError(S, e, t),
              !r.useAdditionalDayOfYearTokens &&
                isProtectedDayOfYearToken(S) &&
                throwProtectedError(S, e, t),
              k(v, S, o.localize, y)
            );
          if (E.match(unescapedLatinCharacterRegExp))
            throw new RangeError(
  ...
  ((t) => {
    (t.formatUnit = ({ value: e, unit: n }, a) =>
      a.showUnit ? `${e} ${n}` : `${e}.0`),
      (t.formatRating = ({ value: e, scale_max: n }, a) =>
        a.showMaxScale ? `${e}/${n}` : e);
  })(ShopifyFormatterPrefix || (ShopifyFormatterPrefix = {}));
  const formatDataVariantMetafields = (t) => {
      try {
        const e = [],
          n = textToJsonUseVal(t == null ? void 0 : t[0]) || {},
          { currentType: a, currentKey: r, currentProps: o } = n;
        let i = a;
        const s =
          a == null ? void 0 : a.match(REGEX_CHECK_LAST_WORD_BEFORE_UNDERSCORE);
        return (
          s && (i = s[1]),
          t.forEach((c) => {
    ...
    formatShopifyDateTime = (t, e) => {
      try {
        ...
        let d = '',
          l = '',
          u = '';
        a && (d = s ? n : DATE_FORMATS_METAFIELD[n]),
          r && (l = s ? `at ${o}` : `'at' ${TIME_FORMATS_METAFIELD[o]}`),
          i && (u = s ? 'GMT%z' : "'GMT'xx");
        let f = format(c, `${d} ${l} ${u}`);
        return s && (f = `${d} ${l} ${u}`), f;
      } catch (n) {
        return (
          console.error('===> Error formatting Shopify date and time:', t, n),
          null
        );
      }
    },
    formatShopifyUrl = (t, e) => {
      try {
        const { linkToUrl: n, linkText: a } = e;
    ...
    formatShopifyBoolean = (t, e) => {
      const { trueValText: n, falseValText: a } = e;
      return t ? n || String(t) : a || String(t);
    },
    formatShopifyMoney = (t, e) => {
      var o;
      const { amount: n, currency_code: a } = t || {},
        { showUnit: r } = e || {};
      try {
        return (o = `${Number(n).toLocaleString('en-US')} ${r ? a : ''}`) ==
          null
          ? void 0
          : o.replace(/,/g, '.');
      } catch {
    ...
    formatShopifyList = (t, e, n) => {
      var s, c;
      const { currentType: a, currentProps: r } = n || {},
        { showUnit: o } = r || {};
      let i = '';
      try {
        const d =
          (c = (s = e == null ? void 0 : e.children) == null ? void 0 : s[0]) ==
          null
            ? void 0
            : c.className;
        return (
              ...
              try {
                switch (a) {
                  ...
                  case TYPE_METAFIELD.LIST_RATING_FIELD: {
                    i += `<li class='${d}'>${ShopifyFormatterPrefix.formatRating(
                      l,
                      r
                    )}</li>`;
                    break;
                  ...
                  default: {
                    if (typeof l == 'object') {
                      const { value: u, unit: f, scale_max: h } = l || {},
                        v = u,
                        y = f || h || '';
                      l = `${v}${
                        o ? ` ${UNIT_MAPPING_METAFIELD[y] || y}` : '.0'
                      }`;
                    }
                    i += `<li class='${d}'>${l}</li>`;
                  }
      ...
      return (
        e.forEach((a) => {
          ...
          r instanceof HTMLElement &&
            ((r.style.backgroundColor = a), n.appendChild(r));
        }),
        n.innerHTML
      );
    } catch {
  ...
  function formatMoney(t = '0', e = pfSetting.moneyFormat) {
    ...
    function i(s, c, d, l) {
      if (
        ((c = o(c, 2)), (d = o(d, ',')), (l = o(l, '.')), isNaN(s) || s == null)
      )
        return 0;
      s = (s / 100).toFixed(c);
      const u = s.split('.'),
  ...
  function convertSchemaToHtml(t, e = !1) {
    ...
    if (
      ...
      ((a = t == null ? void 0 : t.children) == null ? void 0 : a.length) > 0
    )
      e
        ? (n += `
          <div class="${e === !0 ? 'rte' : e}">
  ...
  function buildParagraph(t) {
    if (t != null && t.children)
      return `<p>${convertSchemaToHtml(t == null ? void 0 : t.children)}</p>`;
  }
  function buildHeading(t) {
    if (t != null && t.children)
      return `<h${t == null ? void 0 : t.level}>${convertSchemaToHtml(
        t == null ? void 0 : t.children
      )}</h${t == null ? void 0 : t.level}>`;
  }
  function buildList(t) {
    if (t != null && t.children)
      return (t == null ? void 0 : t.listType) === 'ordered'
        ? `<ol>${convertSchemaToHtml(t == null ? void 0 : t.children)}</ol>`
        : `<ul>${convertSchemaToHtml(t == null ? void 0 : t.children)}</ul>`;
  }
  function buildListItem(t) {
    if (t != null && t.children)
      return `<li>${convertSchemaToHtml(t == null ? void 0 : t.children)}</li>`;
  }
  function buildLink(t) {
    return `<a href="${t == null ? void 0 : t.url}" title="${
      t == null ? void 0 : t.title
    }" target="${t == null ? void 0 : t.target}">${convertSchemaToHtml(
      t == null ? void 0 : t.children
    )}</a>`;
  }
  function buildText(t) {
    return t != null && t.bold
      ? `<strong>${t == null ? void 0 : t.value}</strong>`
      : t != null && t.italic
      ? `<em>${t == null ? void 0 : t.value}</em>`
      : t == null
      ? void 0
      : t.value;
  }
  ...
  async function handleShopifyProductVariantSwatches() {
    document
      .querySelectorAll('div[data-pf-type="ProductVariantSwatches"]')
      .forEach((e, n) => {
        var u;
        ...
        e.addEventListener('change', (f) => {
          ...
          if (i)
            if (e.getAttribute('data-display') === 'dropdown') {
              const y = f.target;
              h = d.find((m) => m.id.toString() === y.value);
            } else {
              const y = f.target;
              h = d.find((m) => m.id.toString() === y.value);
            }
            ...
            const y = Array.from(l)
              .map((m) =>
                m.getAttribute('data-swatch-type') === 'dropdown'
                  ? m.querySelector('select').value
                  : m.querySelector('input:checked').value
              )
              .join(' / ');
            (h = d.find((m) => m.title === y)),
              updateByCurrentVariant(s, h, o, !0, !1, f.target);
          }
          h &&
            (updateByCurrentVariant(s, h, o, !0, !1, f.target),
            getPageTypeFromPFPageSetting() === 'product' &&
              window.history.replaceState(
                null,
                null,
                h != null && h.id ? `?variant=${h.id}` : ''
              ),
            handleToggleVariantMetafield(
              { ...h, variantProductId: s, isOutSideBoxProduct: !a },
              r
            ));
        });
      });
  ...
  const handleToggleVariantMetafield = (t, e) => {
      const { variantProductId: n, isOutSideBoxProduct: a } = t || {};
      e.forEach((r) => {
        var m;
        if (!r) return;
        const o = r.querySelector('.variant-metafields'),
          i = r.querySelector('[data-pf-type="VariantMetafieldValue"]');
        let s = (m = i == null ? void 0 : i.childNodes) == null ? void 0 : m[0];
        const c = r.getAttribute('data-product-id');
        if (!(i && (a ? Number(c) === Number(n) : !0))) return;
        s || ((s = document.createElement('div')), i.appendChild(s));
        const u =
            (
              formatDataVariantMetafields(
                textToJsonUseVal(o == null ? void 0 : o.textContent)
              ) || []
            ).find((S) => S.currentVariantId === (t == null ? void 0 : t.id)) ||
            {},
          { currentKey: f } = u,
          h = u == null ? void 0 : u[f],
          v = typeof h == 'string' ? h.trim().replace(/\n/g, '<br>') : h,
          y = !VALUE_NOT_SATISFY_METAFIELD.includes(v);
        (r.style.cssText = y ? '' : 'visibility: hidden; position: absolute;'),
          y && updateBoxValueVariantMetafield(u, i, s, v);
      });
    },
    updateBoxValueVariantMetafield = (t, e, n, a) => {
      const { currentType: r, currentProps: o } = t || {};
      switch (r) {
        ...
        case TYPE_METAFIELD.DATE_FIELD: {
          const i =
            r === TYPE_METAFIELD.DATE_TIME_FIELD
              ? formatShopifyDateTime(a, o)
              : formatShopifyDate(a, o);
          n.innerHTML = i;
          break;
        }
  ...
  function updateOneSwatchesByValue(t, e, n = []) {
    ...
    if (a)
      if (t.getAttribute('data-display') === 'dropdown') {
        const l =
          t == null ? void 0 : t.querySelector('select:not([name="id"])');
        l &&
          l.value !== ((s = e.id) == null ? void 0 : s.toString()) &&
          (l.value = e.id);
      } else {
      ...
      e.options.forEach((d, l) => {
        const u = t.querySelector(
          `[data-unique-id="${l + 1}"].pf-option-swatches`
        );
        if (
          (u == null ? void 0 : u.getAttribute('data-swatch-type')) ===
          'dropdown'
        ) {
          const h =
            u == null
              ? void 0
              : u.querySelector(
                  `select>option[value='${d.replace(/\'/g, "\\'")}']`
                );
          ...
          const h = n[l],
            ...
            y =
              u == null
                ? void 0
                : u.querySelector(`input[id*='${v.replace(/\'/g, "\\'")}']`);
          y && !y.checked && (y.checked = !0);
        }
      });
    const i = t.querySelector("input[type='hidden'][name='id']");
    i.value !== ((c = e.id) == null ? void 0 : c.toString()) &&
      (i.value = e.id);
  }
  function getCurrentSelectedVariantFromSelectDropDown(t, e) {
    ...
    const a = n.join(' / ');
    return (
      e.find((r) => a === r.title) || e.find((r) => a.indexOf(r.title) > -1)
    );
  }
  function updateOneVariantByValue(t, e, n = []) {
    ...
    if ((t.getAttribute('data-combined') === 'true') === !0) {
      ...
      i &&
        i.value !== ((o = e.id) == null ? void 0 : o.toString()) &&
        (i.value = e.id);
    } else
      a === 'dropdown'
        ? n.forEach((i, s) => {
            const c =
              t == null
                ? void 0
                : t.querySelectorAll(`select[data-key="${i}"]`);
            c == null ||
              c.forEach((d, l) => {
        ...
        : a === 'radio' &&
          n.forEach((i, s) => {
            var l;
            const c =
                ((l = t.querySelectorAll('.pf-c[data-unique-id]')[s]) == null
                  ? void 0
                  : l.getAttribute('data-unique-id')) || 0 + s,
              d =
                t == null
                  ? void 0
                  : t.querySelectorAll(`input[name="pf-v-${i}${c}"]`);
            d == null ||
              d.forEach((u, f) => {
                u.value === e[`option${s + 1}`] &&
                  !u.checked &&
                  ((u.checked = !0), u.setAttribute('checked', 'checked'));
              });
          });
  }
  async function handleShopifyProductVariants() {
    ...
    t.forEach((n, a) => {
      ...
      const r = n,
        ...
        u = pageflyProducts[s],
        f =
          ((h = u == null ? void 0 : u.variants) == null
            ? void 0
            : h.length) === 1 &&
          ((v = u == null ? void 0 : u.variants[0]) == null
            ? void 0
            : v.title) === 'Default Title';
      if (!o && u) {
        const m = u.variantEls || [];
        m.push(n), (u.variantEls = m);
      }
      if (
        ((y = u == null ? void 0 : u.variants) == null ? void 0 : y.length) <
          1 ||
        f
      )
        r.style.display = 'none';
      else if (u && u.options) {
        const { variants: m, options: S } = u,
          E = S.map((k, w) => {
            const I = m.map((M) => M[`option${w + 1}`]);
            return { key: k, value: I.filter((M, x) => I.indexOf(M) === x) };
          });
        if (c !== !0) {
          ...
          if (d === 'dropdown') {
            ...
            const k = n.querySelectorAll('select');
            k.forEach((w) => {
              w.addEventListener('change', () => {
                const I = getCurrentSelectedVariantFromSelectDropDown(k, m);
                updateByCurrentVariant(s, I, i),
                  I &&
                    (handlePushOwlBackInStock(I.id),
                    handleBackInStockBySureSwift(I),
                    handlePushOwlPriceDrop(I.id)),
                  getPageTypeFromPFPageSetting() === 'product' &&
                    window.history.replaceState(
                      null,
                      null,
                      I != null && I.id ? `?variant=${I.id}` : ''
                    );
              });
            });
          } else if (d === 'radio') {
            let k = function () {
              const M = [];
              w.forEach((q, O) => {
                M.push(q.querySelector('input:checked').value);
              });
              const x = M.join(' / ');
              return m.find((q) => q.title === x);
            };
            n.innerHTML =
              n.innerHTML +
              E.map(
            ...
            [...w]
              .map((M, x) => [...M.querySelectorAll('input')])
              .reduce((M, x) => M.concat(x))
              .forEach((M) => {
                M.addEventListener('change', (x) => {
                  x.preventDefault();
                  const q = k();
                  updateByCurrentVariant(s, q, i),
                    q &&
                      (handlePushOwlBackInStock(q.id),
                      handleBackInStockBySureSwift(q),
                      handlePushOwlPriceDrop(q.id)),
                    getPageTypeFromPFPageSetting() === 'product' &&
                      window.history.replaceState(
                        null,
                        null,
                        q != null && q.id ? `?variant=${q.id}` : ''
                      );
                });
              });
          }
        } else
          n.querySelectorAll('select').forEach((w) => {
            w.addEventListener('change', (I) => {
              const M = w.querySelector('option:checked').value,
                x = m.find((O) => O.id.toString() === M);
              updateByCurrentVariant(s, x, i),
                x &&
                  (handlePushOwlBackInStock(x.id),
                  handleBackInStockBySureSwift(x),
                  handlePushOwlPriceDrop(x.id)),
                getPageTypeFromPFPageSetting() === 'product' &&
                  window.history.replaceState(
                    null,
                    null,
                    x != null && x.id ? `?variant=${x.id}` : ''
                  );
            });
          });
      }
  ...
  function getDefaultVariant(t) {
    ...
    if (l != null && l.variants) {
      ...
      if (i) {
        ...
        if (h) return { variant: h, changeMasterImg: !0 };
      }
      if (f)
        return a.length > 1
          ? (a.forEach((h) => {
              var y;
              ((y = getFrontEndSettings(h)) == null
                ? void 0
                : y.imageSource) !== 'default-variant' &&
                h.setAttribute('data-not-default-variant', 'true');
            }),
            { variant: f, changeMasterImage: !0 })
          : r.length > 1
          ? (r.forEach((h) => {
              var y;
              ((y = getFrontEndSettings(h)) == null
            ...
            { variant: f, changeMasterImage: !0 })
          : { variant: f, changeMasterImg: o === 'default-variant' };
    }
    return { variant: null, changeMasterImg: !1 };
  }
  async function handleDefaultVariant() {
    setTimeout(() => {
      document
        .querySelectorAll('div[data-pf-type="ProductBox"]')
        .forEach((e, n) => {
          var s;
          const a = e.className,
            r = e.getAttribute('data-product-id'),
            { variant: o, changeMasterImg: i } = getDefaultVariant(e);
          (s = e.querySelectorAll(
            '[data-pf-type="MainMedia"] [data-splide]'
          )) == null ||
            s.forEach((c) => {
              c != null &&
                c.closest(`.${ACCEPT_UPDATE_VARIANT_BY_IMAGE}`) &&
                (c == null || c.setAttribute('data-no-move-again', 'true'));
            }),
  ...
  function updateOutsideBoxByVariant(t, e, n = !1, a) {
    ...
    if (((t.pfCurrentVariant[OUTSIDE_BOX_KEY] = e), e)) {
      t.variantEls &&
        t.variantEls.forEach((u) => {
          updateOneVariantByValue(u, e, t.options);
        });
      const l = t.swatchesArr && t.swatchesArr.get(OUTSIDE_BOX_KEY);
      l == null || l.forEach((u) => updateOneSwatchesByValue(u, e, t.options)),
        t.priceArr &&
          t.priceArr.get(OUTSIDE_BOX_KEY) &&
          t.priceArr.get(OUTSIDE_BOX_KEY).forEach((u) => {
            typeof e.price == 'number' && (u.innerHTML = formatMoney(e.price));
          }),
        t.comparePriceArr &&
          t.comparePriceArr.get(OUTSIDE_BOX_KEY) &&
          t.comparePriceArr.get(OUTSIDE_BOX_KEY).forEach((u) => {
            u.innerHTML = e.compare_at_price
              ? formatMoney(e.compare_at_price)
              : '';
          }),
        t.quantityArr &&
          t.quantityArr.get(OUTSIDE_BOX_KEY) &&
          t.quantityArr.get(OUTSIDE_BOX_KEY).forEach((u) => {
            const f = u.querySelector('[data-pf-type="QuantityField"]');
            if (!f) return;
            const v = (
              f == null ? void 0 : f.getAttribute('data-variants-continue')
            ).includes(`${e.id}`);
            (a && parseInt(a) <= 0 && e.available) || v
              ? f == null || f.removeAttribute('max')
              : f == null || f.setAttribute('max', a || '999');
          }),
        updateProductMediaOutsideBox(
          (o = t == null ? void 0 : t.mediaArr) == null
            ? void 0
            : o.get(OUTSIDE_BOX_KEY),
          e
        ),
        updateProductMediaOutsideBox2(
          (i = t == null ? void 0 : t.mediaArr) == null
            ? void 0
            : i.get(OUTSIDE_BOX_KEY),
          e
        ),
        updateProductBadgeByVariant(
          (s = t == null ? void 0 : t.badgeArr) == null
            ? void 0
            : s.get(OUTSIDE_BOX_KEY),
          e
        ),
        n ||
          (d =
            (c = t == null ? void 0 : t.imageInstances) == null
              ? void 0
              : c.get(OUTSIDE_BOX_KEY)) == null ||
          d.forEach((u) => u.updateByVariant(e));
    }
    const { atcContents: r } = t;
    t.atcArr &&
      t.atcArr.get(OUTSIDE_BOX_KEY) &&
      t.atcArr
        .get(OUTSIDE_BOX_KEY)
        .filter((l) => !!l)
        .forEach((l) => {
  ...
  function updateByCurrentVariant(t, e, n, a = !0, r = !1, o = null) {
    ...
    let i;
    o
      ? (i = o.closest(`[data-product-id="${t}"][data-pf-type="ProductBox"]`))
      : (i =
          n &&
          document.querySelectorAll(
            `[data-product-id="${t}"].${n.trim().replace(/\s/g, '.')}`
          )[0]);
    const s = pageflyProducts[t];
    if (!s || !s.variants) return;
    const c =
      (u = pageflyProducts[t]) == null
        ? void 0
        : u.quantity.find((m) => m.includes(e == null ? void 0 : e.id));
    handleBackInStockBySureSwift(e);
    const d = e && c.split(`${e.id}:`)[1];
    __pagefly_product_store__.update({ productId: t, variant: e }),
      s.pfCurrentVariant || (s.pfCurrentVariant = {}),
      setTimeout(() => {
        var m, S, E;
        ((m = window.BOLD) == null ? void 0 : m.subscriptions) &&
          ((E = (S = window.BOLD) == null ? void 0 : S.BsubWidget) == null
            ? void 0
            : E.products) &&
          window.BOLD.BsubWidget._renderPrices();
      }, 500);
    const l = i == null ? void 0 : i.querySelectorAll(`.pf-variant-${t}`);
    if (
      (l == null || l.forEach((m) => (m.value = e == null ? void 0 : e.id)), !n)
    )
      updateOutsideBoxByVariant(s, e, !1, d);
    else {
      const m = getElemIdByClassname(n);
      s.pfCurrentVariant[m] = e;
      const { options: S } = s,
        E = () => {
          if (!e) return;
          const x =
            i == null
              ? void 0
              : i.querySelectorAll('[data-product-type="price"]');
          x == null ||
            x.forEach((O) => {
              e &&
                typeof e.price == 'number' &&
                (O.innerHTML = formatMoney(e.price));
            });
          const q =
            i == null
              ? void 0
              : i.querySelectorAll('[data-product-type="compare_at_price"]');
          q == null ||
            q.forEach((O) => {
              e &&
                (O.innerHTML = e.compare_at_price
                  ? formatMoney(e.compare_at_price)
                  : '');
            });
        },
        k = () => {
          ...
          const x =
            i == null
              ? void 0
              : i.querySelectorAll('[data-pf-type="QuantityField"]');
          !e ||
            !((q = pageflyProducts[t]) != null && q.quantity) ||
            !x ||
            x == null ||
            x.forEach((O) => {
              const L = O.getAttribute('data-variants-continue').includes(
                `${e.id}`
              );
              (O && parseInt(d) <= 0 && e.available) || L
                ? O.removeAttribute('max')
                : O.setAttribute('max', d);
            });
        };
      if (a && e && e.id) {
        const x =
            i == null
              ? void 0
              : i.querySelectorAll(
                  `[data-product-id="${t}"][data-pf-type="ProductImage"]`
                ),
          q =
            i == null
              ? void 0
              : i.querySelectorAll(
                  `[data-product-id="${t}"][data-pf-type="ProductMedia"]`
                ),
          O =
            i == null
              ? void 0
              : i.querySelectorAll(
                  `[data-product-id="${t}"][data-pf-type="ProductMedia2"]`
                );
        if (x != null && x.length) {
          const { featured_image: T } = e;
          T &&
            (console.log(s == null ? void 0 : s.imageInstances),
            (h =
              (f = s == null ? void 0 : s.imageInstances) == null
                ? void 0
                : f.get(m)) == null ||
              h.forEach((L) => (L == null ? void 0 : L.updateByVariant(e))));
        } else
          q != null && q.length
            ? q.forEach((T) => {
                const L = getProductMediaId(T);
                !T.getAttribute('data-not-default-variant') &&
                  updateImageByVariant(L, e),
                  T.removeAttribute('data-not-default-variant');
              })
            : O != null &&
              O.length &&
              O.forEach((T) => {
                const L = getProductMediaId2(T);
                !T.getAttribute('data-not-default-variant') &&
                  updateImageByVariant2(L, e),
                  T.removeAttribute('data-not-default-variant');
              });
        E(), k();
      } else r && (E(), k());
      const w =
        (v = s == null ? void 0 : s.badgeArr) == null ? void 0 : v.get(m);
      updateProductBadgeByVariant(w, e);
      const I = [
          ...(i == null
            ? void 0
            : i.querySelectorAll('[data-pf-type="ProductATC"]')),
          ...(i == null
            ? void 0
            : i.querySelectorAll('[data-pf-type="ProductATC2"]')),
        ],
        { atcContents: M } = pageflyProducts[t];
      if (
        (I == null ||
          I.forEach((x) => {
            updateATCTextButtonOnChangeVariant(x, M, e);
          }),
        e)
      ) {
        const x =
          (y = s == null ? void 0 : s.variantArr) == null ? void 0 : y.get(m);
        x == null ||
          x.forEach((O) => {
            updateOneVariantByValue(O, e, S);
          });
        const q =
          i == null
            ? void 0
            : i.querySelector(
                `[data-product-id="${t}"][data-pf-type="ProductVariantSwatches"]`
              );
  ...
  var z;
  const lazyLoad =
    ((z = window.__pagefly_setting) == null ? void 0 : z.imageLazyLoad) || !1;
  ...
  function initOutSideCurrentVariantFromProductImage(t) {
    if (!t.closest('div[data-pf-type="ProductBox"]')) {
      ...
      if (n && !n.pfCurrentVariant && n.variants) {
        ...
        (n.pfCurrentVariant = {}),
          (n.pfCurrentVariant[OUTSIDE_BOX_KEY] = a),
          updateOutsideBoxByVariant(n, a, !0);
      }
    }
  }
  async function handleShopifyProductImage() {
    document
      .querySelectorAll('div[data-pf-type="ProductImage"]')
      .forEach((e) => {
        if (e != null && e.hasAttribute('data-product-id')) {
          const n = new ProductImage(e);
          storeInstanceInProductByType(n, 'image'),
            initOutSideCurrentVariantFromProductImage(e),
            n.run();
        }
      });
  }
  const G = class {
    constructor(e) {
      ...
      b(this, 'handleItemSelect', (e, n = !0) => {
        const { limit: a, listLayout: r } = this.setting,
          o = a[this.device];
        (e = e.closest(G.selector.item)),
          e &&
            ((this.active = this.index),
            (this.index = Object.values(this.$Items).indexOf(e)),
            !(this.active === this.index && o % 2 === 1) &&
              (this.switchImage(this.index, this.active, !0, n),
              r === LIST_LAYOUT.SLIDE && this.handleSlideList(o)));
      });
      (this.$el = e),
        (this.handleItemClick = this.handleItemClick.bind(this)),
        (this.handleItemSelect = this.handleItemSelect.bind(this)),
        (this.triggerClickAction = this.triggerClickAction.bind(this)),
        this.init();
    }
    get boxId() {
      return this._boxId;
    ...
    initValue() {
      ...
      (this.device = getDevice()),
        (this._pId = this.$el.getAttribute('data-product-id')),
        (this.product = pageflyProducts[this.pId]),
        (this.setting = getFrontEndSettings(this.$el) || {}),
        console.log('this.setting', this.setting),
        (this.boxName =
          (n = this.$el.closest(e.box)) == null ? void 0 : n.className),
        (this._boxId = getProductBoxId(this.$el)),
        (this.$master = this.$el.querySelector(e.master)),
        (this.$MasterImg = this.$master.querySelector('img')),
        (this.$List = this.$el.querySelector(e.list)),
        (this.$Paginator = this.$el.querySelectorAll(`${e.pagination}>span`)),
    ...
    initDefault() {
      ...
      const { imageSource: e } = this.setting,
        n =
          ((o = pageflyProducts[this.pId]) == null
            ? void 0
            : o.selected_or_first_available_variant) ||
          ((i = pageflyProducts[this.pId]) == null ? void 0 : i.variants[0]);
      if (!n) return;
      const a =
        (s = n == null ? void 0 : n.featured_image) == null
          ? void 0
          : s.position;
      e === 'variant' && typeof a == 'number'
        ? setTimeout(() => this.handleItemSelect(this.$Items[a - 1], !1), 500)
        : ((c = this.$Items[0]) == null ||
            c.setAttribute('data-active', 'true'),
          (d = this.$Paginator[0]) == null ||
      ...
      this.$Items.forEach((l) => {
        r && l.classList.contains(r) && l.setAttribute('hidden', 'hidden'),
          l.classList.remove(
            'pf-lg-hide',
            'pf-md-hide',
            'pf-sm-hide',
    ...
    triggerClickAction() {
      const { clickAction: e } = this.setting;
    ...
    run() {
      var e, n;
      this.itemLength !== 0 &&
        (['mobile', 'tablet'].includes(this.device)
          ? this.handleSwipe()
          : this.handleHover(),
        this.$master.addEventListener('click', this.triggerClickAction),
        this.$Items.forEach((a) =>
          a.addEventListener('click', this.handleItemClick)
        ),
        (e = this.$el.querySelector(G.selector.nextArrow)) == null ||
          e.addEventListener('click', (a) => {
            this.animating ||
              (this.$Items[this.index + 1] &&
                this.handleItemSelect(this.$Items[this.index + 1]));
          }),
        (n = this.$el.querySelector(G.selector.prevArrow)) == null ||
    ...
    showImageGallery() {
      ...
      if (!this.imageGallery) {
        const n =
          (e = pageflyProducts[this.pId].media) == null
            ? void 0
            : e.filter((a) => (a == null ? void 0 : a.media_type) === 'image');
    ...
    switchImage(e = this.index, n = this.active, a = !0, r = !0) {
      ...
      if (
        (!this.$MasterImg.classList.contains('pf-img-lazy') &&
          (this.$MasterImg.setAttribute('src', o.src),
          this.$MasterImg.setAttribute('srcset', o.srcset)),
        i && this.$MasterImg.setAttribute('data-variant-id', i),
        a)
      ) {
        this.$Items[n].removeAttribute('data-active'),
          this.$Paginator[n].removeAttribute('data-active'),
          this.$Items[e].setAttribute('data-active', 'true'),
          this.$Paginator[e].setAttribute('data-active', 'true');
        const l = this.$master.querySelector('div.zoom-image');
        if (l) {
          const h = o.src;
          (l.style.backgroundImage = `url('${h}')`),
            (l.style.width = `${this.$MasterImg.width}px`),
            (l.style.height = `${this.$MasterImg.height}px`);
        }
        ...
        if (f && r) {
          const {
              featured_image: { variant_ids: h },
            } = f,
            v =
              this.product.pfCurrentVariant &&
              ((d =
                this.product.pfCurrentVariant[this.boxId || OUTSIDE_BOX_KEY]) ==
              null
                ? void 0
                : d.id);
          h.includes(v) ||
            updateByCurrentVariant(this.pId, f, this.boxName, !1, !0);
        }
      }
    }
    handleItemClick(e) {
    ...
    handleSlideList(e) {
      ...
      (this.activeIndex = a ? Math.floor(n) : Math.ceil(n)),
        r < 0
          ? ((o = o - r), (r = 0), (this.activeIndex = this.index))
          : o > this.itemLength - 1 &&
            ((r = r - (o - this.itemLength + 1)),
            (o = this.itemLength - 1),
            (this.activeIndex = (this.index % e) + 1)),
        this.$Items.forEach((i, s) => {
          s < r || s > o
            ? i.hasAttribute('hidden') || this.transform.hiddenList.push(i)
            : i.hasAttribute('hidden') && this.transform.showList.push(i);
        }),
        this.transform.hiddenList.length > 0 &&
          ((this.animating = !0),
          this.transform.endCallback.push(() => {
            this.animating = !1;
          }),
          this.transform.run(a));
    }
    handleHover() {
      const { onHover: a, hoverAction: r } = this.setting;
      let o = this.index,
        i = !1;
      if (r === 0) return;
      if (r === 1) {
      ...
      let s = null,
        c;
      a === 3
        ? this.$MasterImg.addEventListener('mousemove', u)
        : this.$MasterImg.addEventListener('mouseenter', () => {
            s = setTimeout(l, 300);
          }),
        this.$MasterImg.addEventListener('mouseleave', () => {
          typeof s == 'number' && (clearTimeout(s), (s = null)),
            i && (this.switchImage(this.index, this.active, !1), (i = !1));
        });
      const d = () => {
          o < 0
            ? (o = this.itemLength - 1)
            : o > this.itemLength - 1 && (o = 0),
            this.switchImage(o, this.active, !1),
            (i = !0);
        },
        l = () => {
      ...
      function u(f) {
        ...
        c
          ? (Math.abs(f.offsetX - c.x) >= 100 ||
              Math.abs(f.offsetY - c.y) >= 100) &&
            (o++, d(), h())
          : h();
      }
    }
    handleSwipe() {
      const { listLayout: e, limit: n } = this.setting,
        a = n[this.device],
        r = this.$MasterImg,
        o = 60;
      let i = 0,
      ...
      const l = pageflyProducts[this.pId].images,
        ...
        f = (y) => {
          y.stopPropagation(),
            !u.swiping &&
              y.changedTouches &&
              ((i = y.changedTouches[0].clientX),
              (s = y.changedTouches[0].clientY),
              u.setSrc(l[this.index - 1], l[this.index + 1]),
              u.start(),
              (c = !1),
              (d = void 0));
        },
        h = (y) => {
          if ((y.stopPropagation(), y.changedTouches)) {
            c = !0;
            const m = y.changedTouches[0].clientX - i,
              S = y.changedTouches[0].clientY - s;
            if (
              d ||
              (this.index === 0 && m > 0) ||
              (this.index === this.itemLength - 1 && m < 0) ||
              m === 0
            )
              return;
            typeof d > 'u' && Math.abs(m) > 10 && (d = !1),
              typeof d > 'u' && Math.abs(S) > 10 && ((d = !0), u.resetStyle()),
              d === !1 && (y.cancelable && y.preventDefault(), u.move(m));
          }
        },
        v = (y) => {
          if (
            (y.stopPropagation(),
            c || this.triggerClickAction(),
            y.changedTouches)
          ) {
            const m = y.changedTouches[0].clientX - i;
            d === !0 ||
              ...
              : ((this.active = this.index),
                ...
                (this.index =
                  this.index < 0
                    ...
                    : this.index),
                u.end(m > 0),
                this.switchImage(this.index, this.active),
                e === LIST_LAYOUT.SLIDE && this.handleSlideList(a));
          }
          d = void 0;
        };
      r.addEventListener('touchstart', f, { passive: !0 }),
        r.addEventListener('touchmove', h, { passive: !1 }),
  ...
  const j = class {
    constructor(e, n, a, r) {
      ...
      (this.$master = e),
        (this.$MasterImg = n),
        (this.offsetWidth = this.$master.offsetWidth),
        (this.offsetHeight = this.$master.offsetHeight),
        this.setSrc(a, r);
    }
    get sign() {
    ...
    start() {
      (this.swiping = !0),
        (this.isNext = null),
        (this.$master.style.pointerEvents = 'none'),
        (this.offsetWidth = this.$master.offsetWidth || this.offsetWidth),
        (this.offsetHeight = this.$master.offsetHeight || this.offsetHeight),
        (this.$Wrapper = document.createElement('div')),
        (this.$Wrapper.style.cssText = `position: absolute; width: ${this.offsetWidth}px; height: ${this.offsetHeight}px; overflow: hidden; pointer-events: none;`),
        (this.$Mask = document.createElement('div')),
        (this.$Mask2 = document.createElement('div')),
        (this.$Mask.id = 'mask1'),
        (this.$Mask2.id = 'mask2'),
        (this.$Img = this.$MasterImg.cloneNode()),
        (this.$Img2 = this.$MasterImg.cloneNode()),
        (this.$Img2.src = this.nextSrc),
        (this.$Mask.style.cssText = `position: absolute; width: ${this.offsetWidth}px; height: ${this.offsetHeight}px; display: flex; justify-content: center; align-items: center; overflow: hidden;`),
        (this.$Mask2.style.cssText = `position: absolute; width: ${this.offsetWidth}px; height: ${this.offsetHeight}px; display: flex; justify-content: center; align-items: center; right: -${this.offsetWidth}px; overflow: hidden;`),
        this.$Mask.appendChild(this.$Img),
        this.$Mask2.appendChild(this.$Img2),
        this.$Wrapper.appendChild(this.$Mask),
        this.$Wrapper.appendChild(this.$Mask2),
        this.$master.appendChild(this.$Wrapper),
        (this.$MasterImg.style.cssText = 'visibility: hidden');
    }
    setDirect(e) {
      if (e !== this.isNext) {
        (this.isNext = e),
          this.$Mask2.style.removeProperty('right'),
          this.$Mask2.style.removeProperty('left'),
          (this.$Mask2.style.cssText += e
    ...
    moveByEffect(e) {
      ...
      n === 'slide'
        ? ((this.$Img.style.cssText = `transform: translateX(${-e}px);`),
          (this.$Img2.style.cssText = `transform: translateX(${
            -this.sign * (Math.abs(e) - this.offsetWidth)
          }px);`))
        : n !== 'carousel' &&
          ((this.$Img.style.cssText = `transform: translateX(${-e / 2}px);`),
          (this.$Img2.style.cssText = `transform: translateX(${
            -this.sign * (Math.abs(e / 2) - this.offsetWidth / 2)
          }px);`));
    }
    move(e) {
      const n = e < 0;
      this.setDirect(n),
        (this.$Mask.style.cssText += `transform: translateX(${e}px);`),
        (this.$Mask2.style.cssText += `transform: translateX(${e}px);`),
        this.moveByEffect(e);
    }
    get endCSS() {
      let e = `transform: translateX(${
        (-this.sign * this.offsetWidth) / 2
      }px);`;
      switch (this.effect) {
        case 'slide':
    ...
    get resetCSS() {
      let e = `transform: translateX(${this.isNext ? '-' : ''}${
        this.offsetWidth / 2
      }px);`;
      switch (this.effect) {
        case 'slide':
      ...
      (this.$Mask.style.cssText += `transition: all ${j.TIME}s;`),
        (this.$Mask2.style.cssText += `transition: all ${j.TIME}s;`),
        (this.$Img.style.cssText = `transition: all ${j.TIME}s;${this.endCSS}`),
        (this.$Img2.style.cssText = `transition: all ${j.TIME}s; transform: translateX(0);`),
        (this.$Mask.style.cssText += `transform: translateX(${e ? '' : '-'}${
          this.offsetWidth
        }px);`),
        (this.$Mask2.style.cssText += `transform: translateX(${e ? '' : '-'}${
          this.offsetWidth
        }px);`),
        setTimeout(() => {
          this.resetStyle();
    ...
    resetStyle() {
      (this.$MasterImg.style.cssText = ''),
        this.$Wrapper && this.$master.removeChild(this.$Wrapper),
        this.$master.style.removeProperty('pointer-events'),
        (this.$Wrapper = null),
        (this.swiping = !1);
    }
    reset(e = !0) {
      (this.$Mask.style.cssText += `transition: all ${j.TIME}s;`),
        (this.$Mask2.style.cssText += `transition: all ${j.TIME}s;`),
        (this.$Img.style.cssText = `transition: all ${j.TIME}s; transform: translateX(0);`),
        (this.$Img2.style.cssText = `transition: all ${j.TIME}s;${this.resetCSS}`),
        setTimeout(() => {
          (this.$Mask.style.cssText += 'transform: translateX(0);'),
            (this.$Mask2.style.cssText += 'transform: translateX(0);'),
            setTimeout(() => {
              e && this.resetStyle();
            }, j.TIME * 1e3);
        }, 10);
  ...
  let SwipeTransition = j;
  b(SwipeTransition, 'TIME', 0.7);
  class Transform {
    constructor(e, n, a, r = 1) {
      ...
      b(this, 'getElCss', (e, n) => {
        ...
        return (
          this.layout === 'h'
            ? (r += `${n ? 'right' : 'left'}: -${
                (this.gutter + this.width) * (n ? e + 1 : a - e)
              }px;`)
            : (r += `${n ? 'bottom' : 'top'}: -${
                (this.gutter + this.height) * (n ? e + 1 : a - e)
              }px;`),
          r
        );
      });
      b(this, 'getListCss', (e) => {
        ...
        return (
          this.layout === 'h'
            ? (a += `transform: translateX(${e ? '-' : ''}${
                (this.width + this.gutter) * n
              }px); `)
            : (a += `transform: translateY(${e ? '-' : ''}${
                (this.height + this.gutter) * n
              }px); `),
          a
        );
      });
      const o = getDevice();
      (this.$List = e),
        (this.width = a == null ? void 0 : a.offsetWidth),
        (this.height = a == null ? void 0 : a.offsetHeight),
        (this.gutter = Number.parseInt(n == null ? void 0 : n.spacing[o])),
        (this.layout =
          (n == null ? void 0 : n.listPosition) === 0 ||
          (n == null ? void 0 : n.listPosition) === 2
            ? 'h'
            : 'v'),
        (this.time = r);
    }
    reset() {
      (this.hiddenList.length = 0),
        ...
        this.endCallback.forEach((e) => typeof e == 'function' && e()),
        (this.endCallback.length = 0);
    }
    run(e) {
      const n = this.hiddenList.length;
      n > 0 &&
        this.showList.length === n &&
        (this.showList.forEach((a, r) => {
          a.removeAttribute('hidden'), (a.style.cssText = this.getElCss(r, e));
        }),
        (this.$List.style.cssText = this.getListCss(e)),
        setTimeout(() => {
      ...
      const n = window.scrollY,
        ...
        o = () => {
          const i = (Date.now() - r) / e,
            s = easeOutCubic(i);
          window.scrollTo({ top: n + s * a }),
            i < 1 && window.requestAnimationFrame(o);
        };
      o();
    };
  function reloadLaiElement() {
    if (typeof window.SMARTIFYAPPS < 'u' && window.SMARTIFYAPPS.rv.installed)
      return window.SMARTIFYAPPS.rv.scmReviewsRate.actionCreateReviews();
  }
  function reloadAreviewsElement() {
    document.querySelectorAll('.areviews_product_item').length > 0 &&
      typeof window.show_infiniti_areviews == 'function' &&
      window.show_infiniti_areviews();
  }
  ...
  function getPageMap(t, e) {
    const n = [1];
    if (t <= 5) for (let a = 2; a <= t; a++) n.push(a);
    else
      e - 1 > 1 && n.push(e - 1),
        e > 1 && n.push(e),
        e + 1 <= t && n.push(e + 1),
        ...
        c.preventDefault(),
          fetch(`${i}page=${e}`)
            ...
            .then((d) => {
              ...
              let u = '';
              l.querySelectorAll(`${r} .pf-c`).forEach(
                (h) => ((u = u + h.outerHTML), h.outerHTML)
              );
              const f = document.querySelectorAll(`${r}`)[0];
              [].slice
                .call(f.getElementsByClassName('pf-c'))
                .map((h) => h.remove()),
                s.parentElement.insertAdjacentHTML('beforebegin', u);
            }),
          (t.innerHTML = ''),
          renderPageIndexes(t, e);
      });
    }
    t.appendChild(a);
  }
  function renderPageIndexes(t, e = 0) {
    ...
    for (let r = 1; r < a.length; r++)
      a[r - 1] + 1 < a[r] && appendPageIndex(t, '...', e),
        appendPageIndex(t, a[r], e);
  }
  async function handleProductPagination() {
    ...
    Array.from(t).forEach((e) => {
      e.addEventListener('click', (n) => {
        n.preventDefault(),
          e.parentElement.parentElement.classList.add('pf-loading');
        ...
        fetch(a.href)
          ...
          .then((d) => {
            ...
            if (e.classList.contains('pf-cmIldm')) {
              const w = e.getAttribute('data-total'),
                I = a.searchParams.get('page');
              if (((l = !0), w === I))
                s != null && s.includes('ArticleList')
                  ? (e.innerText = 'No more post')
                  : (e.innerText = 'No more product'),
                  e.setAttribute('disabled', 'disabled');
              else {
                const M = parseInt(I) + 1;
                a.searchParams.set('page', M.toString()), (e.href = a.href);
              }
            } else
              [...e.parentElement.children].forEach((I) => {
                I !== e && I.classList.remove('pf-cmIatv');
              }),
            ...
            const u = document.querySelector(o),
              ...
              y = [];
            v.forEach((w) => {
              const I = w.querySelectorAll('script');
              if (I.length) {
                const M = document.createElement('script');
                I.forEach((x) => (M.innerHTML += x.innerHTML)), y.push(M);
              }
            }),
              u.querySelectorAll('script').forEach((w, I) => {
                y[I] && w.replaceWith(y[I]);
              }),
              l && e.parentElement.before(...v);
            const S = Array.from(
                u.querySelectorAll(
                  '[data-pf-type^="ArticleBox"], [data-pf-type^="ProductBox"]'
                )
              ),
              E = Array.from(
                h.querySelectorAll(
            ...
            if (
              (!l && hideElementIfNotFound(S, E),
              E.forEach((w, I) => S[I].replaceWith(w)),
              e.parentElement.parentElement.classList.remove('pf-loading'),
              !e.classList.contains('pf-cmIldm'))
            ) {
              const w =
            ...
            '__pf_loadmore_callback' in window &&
              Array.isArray(window.__pf_loadmore_callback) &&
              window.__pf_loadmore_callback.forEach((w) => {
                typeof w == 'function' && w(c);
              }),
              runPaginationHelper(),
              handleShopifyProductBadge(),
              (k = window.jdgm) != null &&
                k.customizeBadges &&
          ...
          .then(() => {
            var d, l;
            (d = window.$) != null &&
              d.aliReviewsAddRatingCollection &&
              window.$.aliReviewsAddRatingCollection(),
              (l = window.SealSubs) != null &&
                l.refresh &&
                window.SealSubs.refresh(),
              reloadLaiElement(),
              reloadAreviewsElement(),
              typeof window.printReviewRatings == 'function' &&
                window.printReviewRatings(),
              window.__handleSideEffectForOtherApp &&
                typeof window.__handleSideEffectForOtherApp == 'function' &&
                window.__handleSideEffectForOtherApp();
          })
  ...
  async function handleShopifyProductList() {
    renderPagination().catch(console.error),
      handleProductPagination().catch(console.error);
  }
  async function handleShopifyProductQuantity() {
    const t = document.querySelectorAll('div[data-pf-type="ProductQuantity"]');
    await storeElsInProductByType(t, 'quantity'),
      setTimeout(() => {
        function e(n) {
          n.forEach((a) => {
            var c, d;
            ...
            async function o(l) {
              r &&
                n.forEach((u) => {
                  const f =
                      r.querySelector('input') || u.querySelector('input'),
                    h =
                      r.querySelector("[data-quantity-action='decrease']") ||
                      u.querySelector("[data-quantity-action='decrease']");
                  (f.value = l),
                    l === 1
                      ? h == null || h.setAttribute('disabled', 'disabled')
                      : h == null || h.removeAttribute('disabled');
                });
            }
            function i(l) {
              const u = l.target,
              ...
              if (h) {
                const v = h.getAttribute('data-quantity-action'),
                  ...
                  m = parseInt(y.value, 10) || 0;
                let S = v === 'increase' ? m + 1 : m - 1;
            ...
            if (a) {
              const l =
                  (c = a.querySelector('input')) == null ? void 0 : c.value,
                u = a.querySelector("[data-quantity-action='decrease']");
              Number(l) === 1 && u && u.setAttribute('disabled', 'disabled'),
                a.getAttribute('listener') ||
                  (a.addEventListener('click', i),
                  (d = a.querySelector('input')) == null ||
                    d.addEventListener('blur', s),
  ...
  ((DataView && getTag$1(new DataView(new ArrayBuffer(1))) != dataViewTag$1) ||
  ...
  var _baseIsArguments = baseIsArguments$1,
    ...
    isArguments$1 = baseIsArguments(
      ...
      : function (t) {
          return (
            isObjectLike$1(t) &&
            hasOwnProperty$1.call(t, 'callee') &&
  ...
  function isLength$2(t) {
    return (
      typeof t == 'number' && t > -1 && t % 1 == 0 && t <= MAX_SAFE_INTEGER
    );
  }
  ...
  function baseIsTypedArray$1(t) {
    return (
      isObjectLike(t) && isLength(t.length) && !!typedArrayTags[baseGetTag(t)]
    );
  }
  ...
  class Slideshow {
    constructor(e, n = {}) {
      ...
      b(this, 'destroy', () => {
        ...
        this.disconnectResizeObserver && this.disconnectResizeObserver(),
          ...
          (e = this.container) == null ||
            e.removeEventListener('scroll', this.scrollEnd),
          (n = this.nextButton) == null ||
            n.removeEventListener('click', this.handleNextNavigationButons),
          (a = this.prevButton) == null ||
            a.removeEventListener('click', this.handlePrevNavigationButons),
          (r = this.paginationButtons) == null ||
            r.forEach((o, i) =>
      ...
      b(this, 'getElementSettings', () => {
        var a;
        const e =
            (a = this.container.getAttribute('data-slider')) == null
              ? void 0
              : a.replace(/'/g, '"'),
          n = e ? JSON.parse(e) : {};
      ...
      b(this, 'initSlideshow', () => {
        try {
          const { maxHeight: e, isEditor: n = !1 } = this.elementSettings;
          !n && this.renderPaginationButtonsLiveView(),
            (this.paginationButtons = this.container.querySelectorAll(
              ':scope > .pf-slider-nav > button'
            )),
            this.initVisibleObservers(),
            !e && this.updateSliderHeight(!0),
            !e && n && this.resizeSlideObserver(),
            this.initScrollAction(),
            this.handleNavigationButtons(),
            this.handlePaginationButtons(),
            this.container.addEventListener('scroll', this.scrollEnd);
        } catch (e) {
      ...
      b(this, 'scrollEnd', () => {
        ...
        clearTimeout(this.timer),
          (this.timer = setTimeout(() => {
            const r = !this.container.getAttribute(
              'no-update-pagination-and-current-index-again'
            );
            r && this.updateCurrentIndexByScroll(),
              e !== 'none' && this.updateNavigationButtonsState(),
              !n && this.updateSliderHeight(),
              a !== 'none' && r && this.updatePaginationButtonsState(),
      ...
      b(this, 'updateCurrentIndexByScroll', () => {
        const {
          ...
          r = Array.from(this.slides).filter((i) =>
            i.classList.contains('is-visible')
          );
        let o = Array.from(this.slides).findIndex((i) =>
          i.classList.contains('is-visible')
        );
        n[e] && (r.length >= Number(a[e]) + 2 || o > 0) && o++,
          (this.currentIndex = o < 0 ? 0 : o);
      });
      b(this, 'updateNavigationButtonsState', () => {
        const { device: e, slidesToShow: n } = this.elementSettings,
          a = this.getTotalPaginationButtons(),
          r = this.getActivePagination();
      ...
      b(this, 'getTotalPaginationButtons', () => {
        ...
        return this.slides.length <= e[a]
          ? 1
          : Math.ceil((this.slides.length - e[a]) / n[a]) + 1;
      });
      b(this, 'getActivePagination', () => {
        const { slidesToScroll: e, device: n } = this.elementSettings;
      ...
      b(this, 'updateSliderHeight', (e = !1) => {
        if (e) {
          const {
            ...
            i = r[n] ? Number(a[n]) + 1 : a[n],
            c = Array.from(this.slides)
              .slice(0, i)
              .map((l) => this.getTotalHeightOfActiveSlides(l)),
            d = Math.max(...c) || 0;
          this.container.style.height = `${Number.parseInt(o[n]) + d}px`;
        } else {
          const { device: n, spacing: a } = this.elementSettings,
            r = this.getMaxHeightOfActiveSlides();
      ...
      b(this, 'getTotalHeightOfActiveSlides', (e, n = !1) => {
        try {
          ...
          for (let o = 0; o < r.length; o++) {
            const i = r[o],
              s = n
                ? parseInt(getComputedStyle(i).height)
      ...
      b(this, 'getMaxHeightOfActiveSlides', () => {
        let e = Array.from(this.slides).filter((r) =>
          r.classList.contains('is-visible')
        );
        if (!e.length) {
          const { slidesToShow: r, device: o } = this.elementSettings;
      ...
      b(this, 'updatePaginationButtonsState', () => {
        var a, r;
        const e =
          ((a = this.paginationButtons) == null ? void 0 : a.length) - 1;
      ...
      b(this, 'resizeSlideObserver', () => {
        ...
        this.slides.forEach((n) => n.childNodes.forEach((a) => e.observe(a))),
      ...
      b(this, 'initVisibleObservers', async () => {
        const e = new IntersectionObserver(
          ...
          { root: this.container, threshold: 0.01 }
        );
        this.slides.forEach((n) => e.observe(n)),
          (this.disconnectVisibleObserver = () => {
            this.slides.forEach((n) => e.unobserve(n)), e.disconnect();
          });
      });
      b(this, 'initScrollAction', () => {
        const { device: e, slidesToScroll: n } = this.elementSettings;
        this.slides.forEach((a, r) => {
          r % n[e] === 0
            ? (a.style.scrollSnapStop = 'always')
            : (a.style.scrollSnapStop = '');
        });
      });
      b(this, 'handleNavigationButtons', () => {
        ...
        this.nextButton &&
          this.prevButton &&
          (this.elementSettings.navStyle !== 'none' &&
            this.updateNavigationButtonsState(),
          (e = this.nextButton) == null ||
            e.addEventListener('click', this.handleNextNavigationButons, {
              passive: !0,
            }),
          (n = this.prevButton) == null ||
            n.addEventListener('click', this.handlePrevNavigationButons, {
              passive: !0,
            }));
      });
      b(this, 'handleNextNavigationButons', (e) => {
        ...
        const n = this.slides.length,
          { slidesToScroll: a, device: r } = this.elementSettings;
      ...
      b(this, 'handlePrevNavigationButons', (e) => {
        e.stopPropagation();
        const { slidesToScroll: n, device: a } = this.elementSettings;
        if (!e.detail || e.detail === 1) {
      ...
      b(this, 'goToSlide', (e) => {
        const { top: n, left: a } = this.getScrollOffset(e);
        this.container.scroll({ top: n, left: a, behavior: 'smooth' });
      });
      b(this, 'getScrollOffset', (e) => {
        ...
        return (
          (a = o(a, 0, this.container.scrollHeight)),
      ...
      b(this, 'handlePaginationButtons', () => {
        var e;
        this.elementSettings.paginationStyle !== 'none' &&
          this.updatePaginationButtonsState(),
          (e = this.paginationButtons) == null ||
            e.forEach((n, a) => {
      ...
      b(this, 'handleClickPaginationButon', (e, n) => {
        e.stopPropagation();
        const { slidesToScroll: a, device: r } = this.elementSettings;
        if (!e.detail || e.detail === 1) {
          this.paginationButtons.forEach((s) => s.classList.remove('active')),
      ...
      if (!e) return;
      (this.container = e),
        (this.elementSettings = Object.assign(this.getElementSettings(), n));
      const { listLayout: a, device: r } = this.elementSettings;
      isEmpty_1(this.elementSettings) ||
        a[r] !== LIST_LAYOUT.SLIDE ||
        ((this.slides = e.querySelectorAll(':scope > .pf-slide')),
        (this.nextButton = e.querySelector(':scope > .pf-slider-next')),
        (this.prevButton = e.querySelector(':scope > .pf-slider-prev')),
        (this.nextButton.style.zIndex = '10'),
        (this.prevButton.style.zIndex = '10'),
        this.initSlideshow());
    }
  ...
  async function handleGlobalSections() {
    const t = (n) => {
        const a = n.querySelector('[data-pf-type="Section"]'),
        ...
        if (!!s) {
          if (u === 'image') {
            ...
            h && (S.alt = h),
              v && (S.title = v),
              e(m, S),
              l && (m.className = `pf-animated ${l}`),
              Array.from(n.children).length < 2 && n.append(m);
          } else if (u === 'youtube' || u === 'vimeo')
            Array.from(n.children).length < 2 && n.append(o), e(o, { src: s });
          else if (u === 'video') {
            let { muted: f, loop: h, controls: v, autoplay: y } = i.dataset;
            const { src: m } = i.dataset,
              S = document.createElement('video');
            e(S, { style: 'width: 100%; height: calc(100% - 24px)' }),
              S.classList.add('pf-html5-video'),
              Array.from(n.children).length < 2 && n.append(S),
              r.preventDefault(),
              r.stopPropagation(),
              (y = y === 'true'),
              (v = v === 'true'),
              ...
              (f = f === 'true'),
              (n.style.cssText =
                'visibility: visible; width: 650px; height: 450px;'),
              e(S, { src: m }),
              v && e(S, { controls: '' }),
              y && S.play(),
              S.addEventListener('click', (E) => {
                E.preventDefault(),
                  E.stopPropagation(),
                  E.currentTarget.hasAttribute('controls') === !1 &&
            ...
            let f = function () {
                if (v && !h) return;
                console.log('addIframeEvent', o.contentDocument),
                  (v = !0),
                  o.contentDocument.querySelectorAll('form').forEach((k) => {
                    k.addEventListener('submit', function (w) {
                      w.preventDefault(),
                        pageflyPost(
                          k.getAttribute('action'),
                          serializeArray(new FormData(k))
                        );
                    });
                  }),
                  o.contentDocument.querySelectorAll('a').forEach((k) => {
                    k.addEventListener('click', function (w) {
                      w.preventDefault();
                      const I = w.currentTarget;
                      console.log('redirect', I);
                      const M = I.getAttribute('href'),
                        x = I.getAttribute('target');
                      M &&
                        (x === '_blank'
                          ? window.open(M)
            ...
            (o.onload = (m) => {
              if (
                (console.log('iframe load', m, o.contentDocument),
                !o.contentDocument || !h)
              ) {
                e(o, { srcdoc: y }), (h = !0);
                return;
              ...
              Array.from(n.children).length < 2 && n.append(o),
              setTimeout(() => {
                console.log('iframe error', r, o.contentDocument),
                  o.contentDocument || (e(o, { srcdoc: y }), (h = !0));
              }, 1e3);
          }
          (n.style.display = 'flex'),
            (n.style.width = c),
            (n.style.height = `calc(${d} + 24px)`),
            n.showModal(),
            l && (o.className = `pf-animated ${l}`);
        }
      });
    });
  }
  function serializeArray(t) {
    const e = [];
    for (const [n, a] of t) e.push({ name: n, value: a });
    return e;
  }
    ...
    t.forEach((n) => {
      ...
      window.addEventListener('load', () => {
        const r = n.getAttribute('data-to-section'),
          o =
            document.querySelector(`[data-section-id=${r}]`) ||
        ...
        n.addEventListener('click', (r) => {
          ...
          const s = r.currentTarget.getAttribute('data-to-section'),
            c =
              'pf_accordion_offset' in window ? window.pf_accordion_offset : 0;
          if (s) {
            const u =
              (
                document.querySelector(`[data-section-id=${s}]`) ||
  ...
  async function initModal() {
	...
	<polygon style="fill:#FFFFFF" points="224.507,6.997 217.521,0 112.256,105.258 6.998,0 0.005,6.997 105.263,112.254    0.005,217.512 6.998,224.512 112.256,119.24 217.521,224.512 224.507,217.512 119.249,112.254  " data-original="#010002" class="active-path" data-old_color="#010002"></polygon>
        ...
        o.classList.add('pf-close-btn'),
        r.appendChild(o),
        document.head.appendChild(a),
        a.appendChild(document.createTextNode(modalDataCss));
    }
  }
  function handleRemoveModalContent(t) {
    Array.from(t.children)
      .filter((n) => !n.classList.contains('pf-close-btn'))
      .forEach((n) => t.removeChild(n)),
  ...
  async function initParallax() {
    document.querySelectorAll('div[data-parallax="true"]').length > 0 &&
      (await loadJS(
        'https://cdn.pagefly.io/static/assets/jarallax.min.js'
      ).catch(console.error),
      setTimeout(() => {
        document.querySelectorAll('div[data-parallax="true"]').forEach((e) => {
    ...
    document.querySelectorAll('[data-href][data-pf-type]').forEach((e) => {
      ...
      if (
        ...
        (e.getAttribute('data-action') &&
          !['url', 'phone', 'email'].includes(e.getAttribute('data-action')))
      )
        return;
      ...
      (n || r || o) &&
        d.addEventListener('click', () => {
          if (n && typeof window.ga == 'function')
            if (typeof n == 'string')
              window.ga('send', 'event', ...n.split(',')),
                console.info('Send Ga event:: ', a, ...n.split(','));
            else {
              const l = {
                hitType: 'event',
                eventCategory: n[0] || '',
          ...
          o &&
            ...
            (new Function(o)(),
            console.info('Send FB Pixel event:: ', a, o, r));
        });
    });
  };
  async function handleAction() {
    initModal().catch(console.error),
      hideModal().catch(console.error),
      initParallax().catch(console.error),
      openLightbox().catch(console.error),
      openUrl().catch(console.error),
      handleScrollToSection().catch(console.error),
      handleOpenPopup().catch(console.error),
      handleGlobalSections().catch(console.error),
      setupEventTracking().catch(console.error);
  }
  async function handleShopifyProductDynamicCheckout() {
    const t = document.querySelectorAll(
      '[data-pf-type="ProductDynamicCheckout"]'
    );
    t.length > 0 &&
      t.forEach((e) => {
        ...
        (r == null ? void 0 : r.querySelectorAll('[name="id"]').length) > 1 &&
          (o == null || o.remove());
      });
  }
  async function runPaginationHelper() {
    storeSomeProductElements().catch(console.error),
      handleAction().catch(console.error),
      await handleShopifyProductMedia().catch(console.error),
      await handleShopifyProductMedia2().catch(console.error),
      handleDefaultVariant().catch(console.error),
      handleShopifyProductATC().catch(console.error),
      handleShopifyProductVariants().catch(console.error),
      handleShopifyProductImage().catch(console.error),
      handleShopifyProductQuantity().catch(console.error),
      handleProductCollectionDescription().catch(console.error),
      handleShopifyProductVariantSwatches().catch(console.warn),
      handleSPR().catch(console.error),
      window.__openUrl();
  }
  async function handleShopifyProduct() {
    storeSomeProductElements().catch(console.warn),
      await handleShopifyProductMedia().catch(console.warn),
      await handleShopifyProductMedia2().catch(console.warn),
      handleShopifyListElement().catch(console.warn),
      handleDefaultVariant().catch(console.warn),
      handleShopifyProductImage().catch(console.warn),
      handleShopifyProductBadge().catch(console.warn),
      handleShopifyProductVariants().catch(console.warn),
      handleShopifyProductATC().catch(console.warn),
      handleShopifyProductQuantity().catch(console.warn),
      handleProductCollectionDescription().catch(console.warn),
      handleShopifyProductList().catch(console.warn),
      handleDEPRECATEDelements().catch(console.warn),
      handleShopifyProductVariantSwatches().catch(console.warn),
      handleShopifyProductDynamicCheckout().catch(console.warn);
  }
  async function handleAccordion() {
    ...
    function n(o, i, s) {
      const c = new MutationObserver((l, u) => {
        ...
        d = { subtree: !0, attributes: !0 };
      c.observe(s, d);
    }
    function a(o, i, s) {
      o.getAttribute('data-active') === 'true'
    ...
    function r(o) {
      return new Promise((i) => {
        ...
        s.forEach((l) => {
          l.complete
            ? d()
            : (l.addEventListener('load', d), l.addEventListener('error', d));
        });
      });
    }
    t.forEach((o) => {
      ...
      n(o, s, c),
        e(c, async () => {
          a(o, s, c),
            await handleShopifyProductMedia(c).catch(console.error),
            await handleShopifyProductMedia2(c).catch(console.error),
            await r(c),
            a(o, s, c);
        }),
        a(o, s, c),
        ...
        i.append(s);
      function d(u) {
        u.classList.remove('pf-accordion-show'),
          u.classList.remove('pf-accordion-visually-show'),
          u.classList.add('pf-accordion-visually-hidden'),
          s.addEventListener(
            'animationend',
            function (f) {
              u.classList.add('pf-accordion-hide'),
                u.classList.remove('pf-accordion-visually-hidden');
            },
      ...
      o.addEventListener('click', (u) => {
        ...
        const f = u.currentTarget,
          ...
          m = c.clientHeight,
          S = [...h.children].filter((k) => k !== f.parentElement),
          E = findParentBySelector(i.parentElement, '.pf-accordion-wrapper');
        if (
          (E && E.classList.add('pf-accordion-wrapper-100'),
          f.getAttribute('data-active') === 'true')
        )
          l && (s.style.height = `${m}px`),
            f.setAttribute('data-active', 'false'),
            d(s),
            f.children[0] &&
              f.children[0].classList.contains('pfa-minus') &&
              f.children[0].classList.replace('pfa-minus', 'pfa-plus');
        else {
          f.setAttribute('data-active', 'true'),
            s.classList.remove('pf-accordion-visually-hidden'),
            s.classList.remove('pf-accordion-hide'),
            s.classList.add('pf-accordion-visually-show'),
            s.addEventListener(
              'animationend',
              async function (I) {
                s.classList.add('pf-accordion-show'),
                  s.classList.remove('pf-accordion-visually-show');
              },
              { capture: !1, once: !0, passive: !1 }
            );
          const k =
              s.querySelectorAll('[data-pf-type="SlideshowSlide"] > div') || [],
            w = s.querySelector('.pf-slider');
          e(c, async () => {
            await r(c), !l && a(f, s, c);
          }),
            k.length && w.style.height === '0px' && (w.style.height = 'auto'),
            f.children[0] &&
              f.children[0].classList.contains('pfa-plus') &&
              f.children[0].classList.replace('pfa-plus', 'pfa-minus');
        }
        if (
          (S.forEach((k) => {
            const w = k.childNodes[0],
              I = k.childNodes[1];
            y === 'false' &&
              (w.setAttribute('data-active', 'false'),
              I && (d(I), I.classList.remove('pf-accordion-wrapper-100')),
              w.children[0] &&
                w.children[0].classList.contains('pfa-minus') &&
                w.children[0].classList.replace('pfa-minus', 'pfa-plus'));
          }),
          v === 'true')
        ) {
          ...
          const k =
              'pf_accordion_offset' in window ? window.pf_accordion_offset : 0,
            w =
              f.parentElement.getBoundingClientRect().top -
              document.body.getBoundingClientRect().top,
            I = f.parentElement.previousElementSibling;
          let M = 0;
          I &&
            (I == null ? void 0 : I.children[0].getAttribute('data-active')) ===
              'false' &&
            (M =
              I == null
                ? void 0
                : I.children[1].getBoundingClientRect().height),
            smoothScroll(w - k - M, 650);
        }
      });
    });
  }
  async function handleAnchorTabAccordion() {
    const t = (e) => {
      const n = e.split('#')[1],
        a =
          document.querySelector(`button.pf-anchor[href="#${n}"]`) ||
          document.querySelector(`summary.pf-anchor[href="#${n}"]`);
...
`),
  ...
  function loadAccordionStylesIfPageHasVersionLessThan4_10_0() {
    ...
    try {
      const n =
        ((e =
          (t = Array.from(document.querySelectorAll('script'))) == null
            ? void 0
            : t.map((r) => getPFHelperVersion(r.getAttribute('src') || ''))) ==
        null
          ? void 0
          : e.filter((r) => !!r)) || [];
      (n == null
        ...
        : n.find(
            (r) =>
              comparePFHelperVersion(
                r,
                ACCORDION_WITH_STYLED_COMPONENT_VERSION
              ) < 0
          )) && loadOldAccordionStyles();
    } catch (n) {
  ...
  class PFAccordion {
    constructor(e) {
      ...
      b(this, 'scrollToTop', (e) => {
        ...
        window.scroll({ top: n - 50 - e.offsetHeight, behavior: 'smooth' });
      });
      (this.selector = e), (this.SCROLL_DURATION = 300), this.init();
    }
    init() {
      (this.accordionHeaders = Array.from(
        document.querySelectorAll(`[data-pf-type="${this.selector}.Header"]`)
      )),
        this.accordionHeaders.length && this.observeAccordionHeader();
    }
    observeAccordionHeader() {
      if ('IntersectionObserver' in window) {
        ...
        this.accordionHeaders.forEach((e) => {
    ...
    handleHeaderClick(e) {
      e.parentElement.addEventListener('click', (n) => {
        ...
        const a = e.closest(`div[data-pf-type="${this.selector}"]`),
          { openMultiple: r, scrollToTop: o } = a.dataset || {},
          i = e.closest('details'),
        ...
        r === 'false' &&
          ...
          this.toggleAccordion(
            s,
            s.querySelector(`div[data-pf-type="${this.selector}.Header"]`),
            !0
          ),
          o === 'true'
            ? this.toggleAccordion(
            ...
            : this.toggleAccordion(i, e, i.hasAttribute('open'));
      });
    }
  ...
  const handleAccordion3 = async () => {
    ...
    TimeFunctionUtils = {
      alias: { w: 'week', d: 'day', h: 'hour', m: 'minute', s: 'second' },
      secondPer: {
        week: 7 * 24 * 60 * 60,
        day: 24 * 60 * 60,
        hour: 60 * 60,
      ...
      parseTimeFromSecond: function (t, e = []) {
        const n = (r, o) => {
          ...
          a = [];
        return (
          Object.keys(this.secondPer).map((r) => {
      ...
      prepareDateRange(t) {
        const e = new Date(t.start),
          n = new Date(t.end);
        return (
          e.setHours(0, 0, 0, 0), n.setHours(0, 0, 0, 0), { start: e, end: n }
        );
      },
    },
    webStorage = {
      ...
      init() {
        try {
          ...
          console.warn(
            'LocalStorage is not available. Your data will not persist across sessions. Consider enabling third-party cookies for a better experience.'
          ),
            (this.storage = {});
        }
      },
  ...
  function countdown(t) {
    ...
    function m(A) {
      const C = A[0],
        p = C.toUpperCase() + A.slice(1);
      return function (g) {
        g.classList.add(`pf-countdown__${A}`),
          v.on && g.nextElementSibling
            ? (g.nextElementSibling.textContent = d[C].text || p)
            : g.nextElementSibling && (g.nextElementSibling.textContent = '');
      };
    }
    const S = {
      w: m('weeks'),
    ...
    y.forEach((A) => {
      const C = A.parentElement.classList;
    ...
    const E = Object.keys(d)
        .map((A) => {
          const C = TimeFunctionUtils.alias[A];
          return !d[A].on && C;
        })
        .filter((A) => A),
      k = () => {
        ...
        if (a === 'specific') {
          ...
          if (p <= new Date() && g > 0) {
            ...
            switch (o) {
              case 'hour':
                ...
                break;
              case 'day':
                const D = _ % (24 * 3600 * 1e3);
                A = g - D;
                break;
              case 'week':
                const F = _ % (7 * 24 * 3600 * 1e3);
                A = g - F;
                break;
              case 'month':
                const B = _ % (30 * 24 * 3600 * 1e3);
                A = g - B;
                break;
              case 'never':
              default:
          ...
          const p =
            i === 'specific' ? new Date(s).getTime() - C.getTime() : c * 1e3;
          if (a === 'first' && typeof e != 'number') {
            const _ = `pf-${l}-start-countDown`,
              P = webStorage.getItem(_),
              D = P ? new Date(P) : C;
            P || webStorage.setItem(_, D.toString()),
              (e = p - (C.getTime() - D.getTime()));
          }
          e = (typeof e == 'number' ? e : p) - 1e3;
          const g = p - e;
          switch (o) {
            case 'hour':
              const _ = g % 36e5;
      ...
      I = t.querySelector('.pf-countdown__days'),
      M = t.querySelector('.pf-countdown__hours'),
      x = t.querySelector('.pf-countdown__mins'),
    ...
    function L() {
      ...
      A > 0
        ? t.style.display === 'none' &&
          (t.style.display = h ? 'block' : 'inline-block')
        : (u && (t.style.display = 'none'),
          f && f.on && f.url && A !== null && (window.location.href = f.url)),
        typeof e == 'number' && e <= 0 && clearInterval(T);
      const C = TimeFunctionUtils.parseTimeFromSecond(
        Math.floor(A > 0 ? A / 1e3 : 0),
        E
      );
      d.w.on && (w.innerText = `${O(C.length && C.pop())}`),
        d.d.on && (I.innerText = `${O(C.length && C.pop())}`),
        d.h.on && (M.innerText = `${O(C.length && C.pop())}`),
        d.m.on && (x.innerText = `${O(C.length && C.pop())}`),
        d.s.on && (q.innerText = `${O(C.length && C.pop())}`);
    }
  }
  ...
  const defaultOptionObserver = { threshold: 0 };
  ...
  async function loadScript({ src: t, iframeDoc: e = document, ...n }) {
    const a = e.createElement('script');
    Object.assign(a, { src: t, ...n });
    const r = new Promise((o, i) => {
      (a.onload = () => o(a)),
        (a.onerror = (s) => i(new Error(`Failed to load script: ${t}: ${s}`)));
    });
  ...
  function processChunk(t) {
    const { start: e, list: n, callback: a, chunkSize: r, method: o } = t;
    let i = e + r;
  ...
  function executeChunk(t, e, n = 10) {
    ...
    window.requestAnimationFrame && (a = (o) => requestAnimationFrame(o)),
      window.requestIdleCallback && (a = (o) => requestIdleCallback(o)),
      processChunk({ start: 0, list: t, callback: e, chunkSize: n, method: a });
  }
  class GMapLoader {
    constructor({ ...e }) {
      ...
      Object.assign(this, e),
        this.gmapV1Selector && this.handleGmapV1(),
        (this.gmapV2Selector ||
          ((n = this.gmapV2Elements) == null ? void 0 : n.length)) &&
          this.handleGmapV2();
    }
    async handleGmapV1() {
    ...
    async handleGmapV2() {
      (this.gmapV2Selector
        ? Array.from(document.querySelectorAll(this.gmapV2Selector))
        : this.gmapV2Elements
      ).forEach((n) => {
        var i;
        ...
        const r =
          n.contentDocument ||
          ((i = n.contentWindow) == null ? void 0 : i.document);
        if (!r) {
          console.error('Cannot access iframe document.');
          return;
        }
        const o = n.getAttribute('data-api');
        (r.body.style.cssText = 'margin: 0; height: 100%;'),
          loadScript({
            iframeDoc: r,
    ...
    onLoadGoogleMap(e, n, a) {
      ...
      const o = JSON.parse(n.replace(/&quot;/g, '"')),
        ...
        E = {
          center: { lat: Number(u), lng: Number(f) },
          zoom: Number(S),
          styles: MapStyles[h],
          maxZoom: 30,
          minZoom: 0,
        ...
        k = new r.maps.Map(a, E),
        w = {
          map: k,
          position: { lat: Number(u), lng: Number(f) },
          options: { optimized: !0 },
          icon: {
            url: y,
        ...
        I = new r.maps.Marker(w);
      if (i || d) {
        const M = new r.maps.InfoWindow({ content: d || i });
        I.addListener('click', () => M.open(k, I));
      }
    }
  }
  class GMapElementLazyLoader extends BaseElementLazyLoader {
    constructor({ optionObserver: n = defaultOptionObserver, ...a }) {
      super(n);
      ...
      b(
        this,
        'loadGMapAdvance',
        (n) => new GMapLoader({ gmapV2Elements: [n] })
      );
    ...
    init() {
      ...
      executeChunk(n, (a) => {
        this.observerElement(
          this.type === 'advanced' ? this.loadGMapAdvance : this.loadGMapBasic,
          a
        );
      });
    }
  }
  const handleAdvancedGMapStandardLoading = async () =>
      new GMapLoader({
        gmapV1Selector: '[data-pf-type="GMap2"] .gmap2',
        gmapV2Selector:
          '[data-pf-type="GMapAdvancedV2"][data-lazyload="false"]',
      }),
    handleAdvancedGMapLazyLoading = async () =>
      new GMapElementLazyLoader({
        elementSelector:
          '[data-pf-type="GMapAdvancedV2"][data-lazyload="true"]',
        type: 'advanced',
      }),
    handleBasicGMapLazyLoading = async () =>
      new GMapElementLazyLoader({
        elementSelector:
          'iframe[data-pf-type="GMapBasicV2"][data-lazyload="true"]',
        type: 'basic',
      });
  async function handleHTML5Video() {
    document
      .querySelectorAll('[data-pf-type="HTML.Video"] > video')
      .forEach((e) => {
        let n = !1;
        ...
        e.addEventListener(
          ...
          function () {
            e.play(),
              (n = !0),
              e.nextElementSibling && (e.nextElementSibling.style.opacity = 0);
          },
          !1
        ),
          e.addEventListener(
          ...
          (e.parentNode.onclick = function (r) {
            const o = r.target === e;
            if (!a || (a && (!n || !o)))
              return (
                e.paused
  ...
  class HTMLVideoHandler extends BaseElementLazyLoader {
    constructor({ optionsObserver: n, ...a }) {
      super(n);
      b(this, 'container');
      b(this, 'selector');
      Object.assign(this, a), this.init();
    ...
    init() {
      (this.container = document.querySelectorAll(this.selector)),
        this.container.length &&
          executeChunk(this.container, this.handleElement.bind(this));
    }
    handleElement(n) {
      var o;
      const a =
        ((o = n == null ? void 0 : n.tagName) == null
          ? void 0
          : o.toLowerCase()) === 'video'
          ? n
          : n.querySelector('video');
      if (!a) return;
      a.getAttribute('data-lazyloading') === 'lazy'
        ? this.observerElement(
            () => this.loadVideo({ element: n, video: a }),
            n
          )
        : this.loadVideo({ element: n, video: a });
    }
    addEvent({ element: n, type: a, callback: r, options: o = !1 }) {
      n.addEventListener(a, r, o);
    }
    loadVideo({ element: n, video: a }) {
      ...
      const d = (f) => f.classList.add('pf-visibility-hidden'),
        l = (f) => {
          var h;
          return (h = f.classList) == null
            ? void 0
            : h.remove('pf-visibility-hidden');
        },
        u = (f) => {
          a.getAttribute('src') || (this.setProp(a) && this.removeProp(a)),
            r && !r.classList.contains('pf-visibility-hidden')
              ? (a.play(), d(r))
              : !s && (a.paused ? a.play() : a.pause());
        };
      this.addEvent({
        element: a,
        type: 'loadeddata',
        ...
        this.addEvent({ element: n, type: 'click', callback: u }),
        this.addEvent({ element: a, type: 'pause', callback: () => o && l(o) }),
        this.addEvent({ element: a, type: 'play', callback: () => o && d(o) }),
        i && c && (r && d(r), o && d(o), a.play());
    }
  }
  const handleHTMLVideo2 = async () => {
    new HTMLVideoHandler({
      optionsObserver: { threshold: 0.3 },
  ...
  class TwitterLoader extends BaseElementLazyLoader {
    ...
    init() {
      (this.container = [].slice.call(
        document.querySelectorAll(this.selector)
      )),
        (this.container = this.container.filter(
          (n) =>
            (n == null ? void 0 : n.getAttribute('data-lazyload')) === 'true'
        )),
        this.observerElements(
          this.appendScriptWidget.bind(this),
          this.container,
          !0,
          this.appendScriptWidget
        );
    }
    async appendScriptWidget(n, a) {
      try {
        const r = n == null ? void 0 : n.querySelector('a');
        r.classList.add('twitter-timeline'),
          typeof window.twttr < 'u' && window.twttr.widgets
            ? window.twttr.widgets.load(r)
            : !isScriptLoaded(this.scriptUrl) &&
              (await loadScript({
                src: this.scriptUrl,
                charset: 'utf-8',
                defer: !0,
  ...
  class SocialMediaFrameLoader extends BaseElementLazyLoader {
    constructor({ optionObserver: n = defaultOptionObserver, ...a }) {
      ...
      b(this, 'loadMedia', (n) => {
        this.type === 'video'
          ? (this.setProp(n, 'poster', 'data-poster') &&
              this.removeProp(n, 'data-poster'),
            n.querySelectorAll('source[data-src]').forEach((r) => {
              this.setProp(r) && this.removeProp(r);
            }),
            n.load())
          : this.setProp(n) && this.removeProp(n);
      });
      Object.assign(this, a), this.init();
    }
    init() {
      const n =
        ...
        [].slice.call(document.querySelectorAll(this.containerSelector));
      executeChunk(n, (a) => {
  ...
  const handleFBPage2Loader = async () =>
      new SocialMediaFrameLoader({
        containerSelector: '[data-pf-type="FBPageBox2"]',
        elementSelector: 'iframe[data-lazyload="true"]',
      }),
    handleFBLike2Loader = async () =>
      new SocialMediaFrameLoader({
  ...
  async function fetchData(t) {
    ...
    if (e) {
      const o = await getHttpRequest(
          `https://graph.instagram.com/me/media?fields=media_url,media_type,permalink,thumbnail_url&access_token=${e}&limit=${n}`
        ),
        i = JSON.parse(o);
      if (Array.isArray(i == null ? void 0 : i.data))
  ...
  const removeElementToDom = (t) => t.remove(),
    appendHtmlToDom = (t, e, n = !1) => {
      ...
      const a = t.querySelector('&> div'),
        r =
          (d = t.querySelector('div.pf-c img')) == null
            ? void 0
            : d.closest('div.pf-c'),
        o = document.createElement('div');
      (o.className = 'pf-c pf-slide'),
        (o.innerHTML +=
          '<a target="_blank"><video controls preload="none"><source type="video/mp4"/></video></a>'),
        t.querySelectorAll('.pf-c').forEach((u) => {
      ...
      for (let u = 0; u < s; u++) {
        let f = null;
        e[u].media_type === 'VIDEO'
          ? ((f = o.cloneNode(!0)),
            f
              .querySelector('video')
              .setAttribute(n ? 'data-poster' : 'poster', e[u].thumbnail_url),
            f
              .querySelector('source')
              .setAttribute(n ? 'data-src' : 'src', e[u].media_url))
          : ((f = r.cloneNode(!0)),
            (l = f.querySelector('img')) == null ||
              l.setAttribute('src', e[u].media_url)),
          f.querySelector('a').setAttribute('href', e[u].permalink),
          c.append(f);
      }
      a.append(c);
    };
  class HandleInstagram3 extends BaseElementLazyLoader {
    constructor({
      optionObserver: e = { rootMargin: '400px 0px 0px 0px' },
      ...n
    }) {
        ...
        this.observerElement(() => {
          fetchData(r)
            .then((i) => {
              ...
              o === a - 1 &&
                handleShopifyInitSlideshow('[data-pf-type="Insta3"]').catch(
                  console.warn
                );
            })
            .catch(console.error);
  ...
  async function handleInstagram() {
    ...
    async function e(n) {
      ...
      const a = (c = getFrontEndSettings(n)) == null ? void 0 : c.token,
        ...
        o =
          (l = n.querySelector('div.pf-c img')) == null
            ? void 0
            : l.closest('div.pf-c'),
        i = document.createElement('div'),
        s = n.getAttribute('data-pf-type') === 'Instagram';
      if (
        ((i.className = s ? 'pf-c' : 'pf-c pf-slide'),
        (i.innerHTML +=
          '<a target="_blank"><video controls preload="none"><source type="video/mp4"/></video></a>'),
        a)
      ) {
        const f = await getHttpRequest(
            `https://graph.instagram.com/me/media?fields=media_url,media_type,permalink,thumbnail_url&access_token=${a}&limit=${r}`
          ),
          h = JSON.parse(f);
        if (Array.isArray(h == null ? void 0 : h.data)) {
          const v = n.querySelector('&> div');
          n.querySelectorAll('.pf-c').forEach((m) => {
            m.remove();
          });
          for (let m = 0; m < h.data.length; m++) {
            let S = null;
            h.data[m].media_type === 'VIDEO'
              ? ((S = i.cloneNode(!0)),
                S.querySelector('video').setAttribute(
                  'poster',
                  h.data[m].thumbnail_url
                ),
                S.querySelector('source').setAttribute(
                  'src',
                  h.data[m].media_url
                ))
              : ((S = o.cloneNode(!0)),
                (u = S.querySelector('img')) == null ||
                  u.setAttribute('src', h.data[m].media_url)),
              S.querySelector('a').setAttribute('href', h.data[m].permalink),
              v.append(S);
          }
        }
      }
    ...
    await handleShopifyInitSlideshow('[data-pf-type="Instagram2"]').catch(
      console.warn
    );
  }
  /* @preserve
...
*/ (function (t) {
    ...
    return (
      (n.init = function (a, r) {
        ...
        (o.slides = o.track.children),
          [].forEach.call(o.slides, function (l, u) {
        ...
        o.opt.slidesToScroll === 'auto' &&
          ...
          (c || r) && (o.bindArrows(), o.buildDots(), o.bindDrag()),
          o.updateControls(),
          o.emit(a ? 'refresh' : 'loaded');
      }),
      (n.bindDrag = function () {
        var a = this;
        ...
        var r = function () {
            (a.mouseDown = void 0),
              a.ele.classList.remove('drag'),
              a.isDrag && (a.preventClick = !0),
              (a.isDrag = !1);
          },
          o = {
            mouseup: r,
            ...
            mousedown: function (i) {
              const s = i.target;
              ['SELECT', 'INPUT', 'TEXTAREA'].includes(s.tagName)
                ? (a.preventClick = !1)
                : (i.preventDefault(),
                  i.stopPropagation(),
                  (a.mouseDown = i.clientX),
                  a.ele.classList.add('drag'));
            },
            mousemove: a.mouse,
            click: function (i) {
              a.preventClick && (i.preventDefault(), i.stopPropagation()),
                (a.preventClick = !1);
            },
          };
        a.ele.classList.toggle('draggable', a.opt.draggable === !0),
          a.event(a.ele, 'remove', o),
          a.opt.draggable && a.event(a.ele, 'add', o);
      }),
      (n.buildDots = function () {
        ...
        if (
          (typeof a.opt.dots == 'string'
            ? (a.dots = document.querySelector(a.opt.dots))
            : (a.dots = a.opt.dots),
          !!a.dots)
        ) {
          ...
          for (
            var r = 0;
            r < Math.ceil(a.slides.length / a.opt.slidesToShow);
            ++r
          ) {
            var o = document.createElement('button');
            (o.dataset.index = r),
              o.setAttribute('aria-label', 'Page ' + (r + 1)),
              o.setAttribute('role', 'tab'),
              (o.className = 'glider-dot ' + (r ? '' : 'active')),
              a.event(o, 'add', { click: a.scrollItem.bind(a, r, !0) }),
      ...
      (n.bindArrows = function () {
        ...
        if (!a.opt.arrows) {
          Object.keys(a.arrows).forEach(function (r) {
            var o = a.arrows[r];
            a.event(o, 'remove', { click: o._func });
          });
          return;
        }
        ['prev', 'next'].forEach(function (r) {
          ...
          o &&
            ...
            o &&
              ((o._func = o._func || a.scrollItem.bind(a, r)),
              a.event(o, 'remove', { click: o._func }),
              a.event(o, 'add', { click: o._func }),
              (a.arrows[r] = o)));
        });
      }),
      (n.updateControls = function (a) {
        ...
        r.opt.rewind ||
          (r.arrows.prev &&
            (r.arrows.prev.classList.toggle(
              'disabled',
              r.ele.scrollLeft <= 0 || o
            ),
            r.arrows.prev.classList.contains('disabled')
              ? r.arrows.prev.setAttribute('aria-disabled', !0)
              : r.arrows.prev.setAttribute('aria-disabled', !1)),
          r.arrows.next &&
            (r.arrows.next.classList.toggle(
              'disabled',
              Math.ceil(r.ele.scrollLeft + r.containerWidth) >=
                Math.floor(r.trackWidth) || o
            ),
            r.arrows.next.classList.contains('disabled')
              ? r.arrows.next.setAttribute('aria-disabled', !0)
              : r.arrows.next.setAttribute('aria-disabled', !1))),
          (r.slide = Math.round(r.ele.scrollLeft / r.itemWidth)),
          (r.page = Math.round(r.ele.scrollLeft / r.containerWidth));
        var i = r.slide + Math.floor(Math.floor(r.opt.slidesToShow) / 2),
          s = Math.floor(r.opt.slidesToShow) % 2 ? 0 : i + 1;
        Math.floor(r.opt.slidesToShow) === 1 && (s = 0),
          r.ele.scrollLeft + r.containerWidth >= Math.floor(r.trackWidth) &&
            (r.page = r.dots ? r.dots.children.length - 1 : 0),
          [].forEach.call(r.slides, function (c, d) {
            var l = c.classList,
              ...
              i === d || (s && s === d)
                ...
                : (l.remove('center'),
                  l.add(
                    [
                      d < i ? 'left' : 'right',
                      Math.abs(d - (d < i ? i : s || i)),
                    ].join('-')
                  ));
            var m =
              Math.ceil(v) >= Math.floor(f) && Math.floor(y) <= Math.ceil(h);
            l.toggle('visible', m),
              m !== u &&
                r.emit('slide-' + (m ? 'visible' : 'hidden'), { slide: d });
          }),
          r.dots &&
            [].forEach.call(r.dots.children, function (c, d) {
              c.classList.toggle('active', r.page === d);
            }),
          a &&
            r.opt.scrollLock &&
            (clearTimeout(r.scrollLock),
            (r.scrollLock = setTimeout(function () {
              clearTimeout(r.scrollLock),
                Math.abs(r.ele.scrollLeft / r.itemWidth - r.slide) > 0.02 &&
                  (r.mouseDown ||
                    (r.trackWidth > r.containerWidth + r.ele.scrollLeft &&
                      r.scrollItem(r.getCurrentSlide())));
      ...
      (n.scrollItem = function (a, r, o) {
        o && (o.preventDefault(), o.stopPropagation()),
          r != null && r.preventDefault && r.preventDefault();
        var i = this,
          s = a;
        if ((++i.animate_id, r === !0))
          (a = a * i.containerWidth),
            (a = Math.round(a / i.itemWidth) * i.itemWidth);
        else {
          if (typeof a == 'string') {
            var c = a === 'prev';
            if (
              (i.opt.slidesToScroll % 1 || i.opt.slidesToShow % 1
                ? (a = i.getCurrentSlide())
                : (a = i.slide),
              c ? (a -= i.opt.slidesToScroll) : (a += i.opt.slidesToScroll),
              i.opt.rewind)
            ) {
              ...
              a =
                c && !d
                  ? i.slides.length
                  : !c &&
                    Math.ceil(d + i.containerWidth) >= Math.floor(i.trackWidth)
                  ? 0
                  : a;
            }
          }
          (a = Math.max(Math.min(a, i.slides.length), 0)),
            (i.slide = a),
            (a = i.itemWidth * a);
        }
        return (
          i.scrollTo(
            a,
            i.opt.duration * Math.abs(i.ele.scrollLeft - a),
            function () {
              i.updateControls(),
                i.emit('animated', {
                  value: s,
        ...
        if (r) {
          ...
          for (var o = 0; o < r.length; ++o) {
            ...
            if (t.innerWidth >= i.breakpoint)
              return a.breakpoint !== i.breakpoint
                ? ((a.opt = Object.assign({}, a._opt, i.settings)),
                  (a.breakpoint = i.breakpoint),
                  !0)
                : !1;
          }
        }
      ...
      (n.scrollTo = function (a, r, o) {
        var i = this,
          ...
          d = function () {
            ...
            (i.ele.scrollLeft =
              i.ele.scrollLeft +
              (a - i.ele.scrollLeft) * i.opt.easing(0, l, 0, 1, r)),
              l < r && c === i.animate_id
                ? t.requestAnimationFrame(d)
                : ((i.ele.scrollLeft = a), o && o.call(i));
          };
        t.requestAnimationFrame(d);
      }),
      (n.removeItem = function (a) {
        var r = this;
        r.slides.length &&
          (r.track.removeChild(r.slides[a]), r.refresh(!0), r.emit('remove'));
      }),
      ...
      (n.handleMouse = function (a) {
        ...
        r.mouseDown &&
          ((r.isDrag = !0),
          (r.ele.scrollLeft +=
            (r.mouseDown - a.clientX) * (r.opt.dragVelocity || 3.3)),
          (r.mouseDown = a.clientX));
      }),
      (n.round = function (a) {
        var r = this,
      ...
      (n.setOption = function (a, r) {
        var o = this;
        o.breakpoint && !r
          ? o._opt.responsive.forEach(function (i) {
              i.breakpoint === o.breakpoint &&
                (i.settings = Object.assign({}, i.settings, a));
            })
          : (o._opt = Object.assign({}, o._opt, a)),
          (o.breakpoint = 0),
        ...
        var a = this,
          ...
          o = function (i) {
            i.removeAttribute('style'),
              [].forEach.call(i.classList, function (s) {
                /^glider/.test(s) && i.classList.remove(s);
              });
          };
        (r.children[0].outerHTML = r.children[0].innerHTML),
          o(r),
          [].forEach.call(r.getElementsByTagName('*'), o),
          a.ele.parentNode.replaceChild(r, a.ele),
          a.event(t, 'remove', { resize: a.resize }),
          a.emit('destroy');
      }),
      (n.emit = function (a, r) {
        var o = this,
  ...
  async function handleSliderHeight(t) {
    const e = Array.from(
      t.querySelectorAll('.glider-slide.visible [data-pf-type="Slide2"]')
    );
    await e.map((s, c) => s.removeAttribute('style'));
    const n = e.map(
        (s) => (s == null ? void 0 : s.getBoundingClientRect().height) || 0
      ),
      a = Math.max(...n),
    ...
    o && r.setAttribute('style', `width: ${i}px; height: ${o}px`),
      a !== o &&
        a &&
        r &&
        r.setAttribute('style', `width: ${i}px; height: ${a}px`),
      await e.map((s, c) => s.setAttribute('style', 'height: 100%'));
  }
  ...
  const refresh = (t) => {
    ...
    addPreloadStyle = (t, e, n) => {
      ...
      r.setAttribute(
        'style',
        `${o}; display: grid; grid-template-columns: ${i}`
      ),
        e.forEach((s, c) => {
    ...
    removePreloadStyle = (t, e, n) => {
      ...
      r.setAttribute(
        'style',
        o.replace(`display: grid; grid-template-columns: ${i}`, '')
      ),
        e.forEach((s, c) => {
          if (c < n[a]) return;
      ...
      for (let n = 0; n < t.length; n++) {
        ...
        let y = {
          ...
          responsive: [
            ...
            {
              breakpoint: 1025,
              settings: { slidesToShow: s.laptop, slidesToScroll: c.laptop },
            },
            {
              breakpoint: 1200,
              settings: { slidesToShow: s.all, slidesToScroll: c.all },
            },
          ],
        };
        ['mobile', 'tablet'].includes(e) && (y = { ...y, draggable: !1 });
        const m = (window.__pagefly_slideshows__[h] = new window.Glider(
          a.querySelector('.glider'),
          y
        ));
        ...
        const E = setInterval(() => {
          S++,
            ...
            (a.style.opacity = 1),
            !f && o === 'Slider2' && handleSliderHeight(a),
            S > 2 && clearInterval(E);
        }, 500);
        !f &&
          o === 'Slider2' &&
          ...
          a
            ...
            .addEventListener('glider-slide-visible', (k) => {
              handleSliderHeight(a);
            }),
          resizeSlideObserver(
            a,
            debounce$1(() => handleSliderHeight(a), 100)
          ),
          window.addEventListener(
            'resize',
            debounce$1(() => handleSliderHeight(a), 500)
          )),
          l &&
            u > 0 &&
            handleAutoPlay({ dom: a, g: m, autoPlayDelay: u, pauseOnHover: v });
      }
    }
  }
  function handleAutoPlay({ dom: t, g: e, autoPlayDelay: n, pauseOnHover: a }) {
    const r = new AutoplayTimer(function () {
        e.scrollItem('next');
      }, n || 3e3),
      o = () => {
        var c, d;
        (c = t.querySelectorAll('.glider-nav')) == null ||
          c.forEach((l) => {
            l == null ||
          ...
          (d = t.querySelectorAll('.glider-dot')) == null ||
            d.forEach((l) => {
              l == null ||
                l.addEventListener('click', () => {
                  r.destroy();
      ...
      s = () => {
        const c = { rootMargin: '0px', threshold: 0.25 };
        let d = !1;
        new IntersectionObserver(function (u) {
          u.forEach((f) => {
            f.isIntersecting
              ? ((d = !0), r.start(), refresh(e))
              : d &&
                ((d = !1),
                r.destroy(),
                setTimeout(() => {
  ...
  async function handleTab() {
    ...
    t.length > 0 &&
      t.forEach((e) => {
        e.querySelector('[data-pf-type="TabsMenu"]').addEventListener(
          'click',
          (a) => {
            a.stopPropagation();
            const r = a.target,
              o =
                r.closest('[data-pf-type="TabHeader"]') ||
                r.closest('[data-pf-type="TabHeader2"]');
            if (!o) return;
            const i = [...(o == null ? void 0 : o.parentElement.children)],
              s = [
                ...(o == null
                  ? void 0
                  : o.parentElement.nextElementSibling.children),
              ],
              c = i.indexOf(o),
              d = i.filter((l) => l !== i[c]);
            o && o.setAttribute('data-pf-tab-active', 'true'),
              s.forEach((l, u) => {
                if (u === c) {
                  (l.style.display = 'block'),
                    handleShopifyProductMedia(l).catch(console.error),
                    handleShopifyProductMedia2(l).catch(console.error),
                    l.querySelectorAll('[data-magnifier]').forEach((y) => {
                      const m = y.querySelector('.zoom-image');
                      m && m.remove(), initZoomImage(y);
                    });
                  const h = l.querySelectorAll('[data-slider][data-pf-type]'),
                    v = Array.from(h);
                  if (v.length)
                    for (let y = 0; y < v.length; y++)
                      v[y].removeAttribute('style'),
                        handleSlideshowVersion1and2(v);
  ...
  class PFTabs {
    constructor(e, n, a, r) {
      ...
      (this.selectorTab = e || 'Tabs3'),
        (this.selectorTabMenu = n || 'TabsMenu3'),
        (this.selectorTabHeader = a || 'TabHeader3'),
        (this.selectorDropDown = r || 'DropdownButton'),
        this.init();
    }
    init() {
      const e = Array.from(
        document.querySelectorAll(`[data-pf-type="${this.selectorTab}"]`)
      );
      if (!!e.length)
    ...
    run(e) {
      const n = e.querySelector(`[data-pf-type="${this.selectorTabMenu}"]`),
        ...
        o = Array.from(
          n.querySelectorAll(`[data-pf-type="${this.selectorTabHeader}"]`)
        ),
        i = e.querySelector(`[data-pf-type="${this.selectorDropDown}"]`);
      this.handleScrollWhenFirstLoad(n),
        this.handleDisplayNavigation(n, a, r, i),
        this.setupListeners(e, n, a, r, i, o);
    }
    setupListeners(e, n, a, r, o, i) {
      ...
      n == null ||
        n.addEventListener('click', (u) =>
          this.handleClickTabHeader(u, n, i, o)
        ),
        r == null ||
          r.addEventListener('click', () => this.handleClickNextBtn(n)),
        a == null ||
          a.addEventListener('click', () => this.handleClickPrevBtn(n));
      for (let u = 0; u < c.length; u++)
        c[u].addEventListener('change', (f) => {
          let h = f.target.id;
          e.querySelector('[data-pf-tab-active="true"]').setAttribute(
            'data-pf-tab-active',
            'false'
          );
          const v = e.querySelector(`[for="${h}"]`),
            y = this.getActiveTabScrollValue(n, v);
          n == null || n.scrollTo({ ...y, behavior: 'smooth' }),
      ...
      n == null ||
        ...
        n == null ||
          n.addEventListener('keydown', (u) => {
            (u.key === 'ArrowRight' || u.key === 'ArrowLeft') &&
              u.preventDefault();
          }),
        o == null ||
          o.addEventListener('click', (u) => {
        ...
        l.addEventListener('focusout', (u) => {
          (!l.contains(u.target) || !l.contains(u.relatedTarget)) &&
            this.handleDropDownMode(n, o, d);
        });
    }
    handleClickTabHeader(e, n, a, r) {
      e.preventDefault();
      const o = e.target,
        i = document.querySelector(`#${o.getAttribute('for')}`);
      i.checked = !0;
      const s = o.closest(`[data-pf-type="${this.selectorTabHeader}"]`),
        c = r.clientHeight === 0 && r.clientWidth === 0;
      if (!s || (s.getAttribute('data-pf-tab-active') === 'true' && c)) return;
      const d = this.getActiveTabScrollValue(n, s);
      this.removeAllActiveStatus(a),
        s.setAttribute('data-pf-tab-active', 'true'),
        n == null || n.scrollTo({ ...d, behavior: 'smooth' }),
        c ||
          ((r.querySelector('.pf-tab-dropdown-text').innerHTML =
            s == null ? void 0 : s.innerHTML),
          (r.title = s == null ? void 0 : s.innerText));
    }
    handleClickNextBtn(e) {
      const n = this.getScrollValue(e, 'next');
      e == null ||
        e.scrollTo({ top: n.top || 0, left: n.left || 0, behavior: 'smooth' });
    }
    ...
    handleDisplayNavigation(e, n, a, r) {
      if (!(!n || !a) && !(r.clientHeight !== 0 && r.clientWidth !== 0)) {
        if (
          ...
          (a.style.visibility = 'visible'),
          (e == null ? void 0 : e.scrollLeft) === 0 &&
            (e == null ? void 0 : e.scrollTop) === 0)
        ) {
          n.style.visibility = 'hidden';
          return;
        }
        if (
          e.scrollLeft + 1 >= e.scrollWidth - e.clientWidth &&
          e.scrollTop + 1 >= e.scrollHeight - e.clientHeight
        ) {
    ...
    getScrollValue(e, n) {
      let a = {},
        r = Array.from(
          e.querySelectorAll(`[data-pf-type="${this.selectorTabHeader}"]`)
        );
      if (n === 'next')
        for (let o = 0; o < r.length; o++) {
          const i = r[o],
            ...
            m = l + c;
          if (
            (!a.top && m > Math.round(u + h) && (a.top = l + c - h),
            !a.left && y > Math.round(f + v) && (a.left = s + d - v),
            a.left || a.top)
          )
            return a;
        }
      if (n === 'prev') {
        ...
        for (let s = 0; s < r.length; s++) {
          const c = r[s],
            ...
            l = c.offsetTop;
          if (
            (!a.left && d < Math.floor(o) && (a.left = d),
            !a.top && l < Math.floor(i) && (a.top = l),
            a.left || a.top)
          )
            return a;
        }
    ...
    getActiveTabScrollValue(e, n) {
      ...
      return (
        n &&
          (!a.left && r < Math.floor(i)
            ? (a.left = r)
            : !a.left && r + d > i + c && (a.left = r + d - c),
          !a.top && o < Math.floor(u)
            ? (a.top = o)
            : !a.top && o + l > u + s && (a.top = o + l - s),
          a.left || a.top),
        a
      );
    }
    handleDropDownMode(e, n, a) {
      e.classList.toggle('pf-tab-menu-hide'),
        (a == null ? void 0 : a.classList.contains('pf-dropdown-collapse'))
          ? a == null || a.classList.remove('pf-dropdown-collapse')
          : a == null || a.classList.add('pf-dropdown-collapse'),
        window.scrollTo({
  ...
  async function handleAutoPlayYTVideo() {
    const t = document.querySelectorAll(
      '.__pf [title="video_youtube"]:not(.pf-video-youtube-no-api)'
    );
    t.length > 0 &&
      t.forEach((e) => {
        e.style.opacity = '1';
  ...
  async function handleBasicElement() {
    handleAction().catch(console.error),
      handleShopifyListElement().catch(console.error),
      handleSlideshowVersion1and2().catch(console.error),
      handleSlideshowVersion3().catch(console.error),
      handleAccordion().catch(console.error),
      handleAccordion3().catch(console.error),
      handleAnchorTabAccordion().catch(console.error),
      handleCountDown().catch(console.error),
      handleHTMLVideo2().catch(console.error),
      handleHTML5Video().catch(console.error),
      handleAutoPlayYTVideo().catch(console.error),
      handleTab().catch(console.error),
      handleTab3().catch(console.error),
      handleTwitterLoader().catch(console.error),
      handleFBLike2Loader().catch(console.error),
      handleFBPage2Loader().catch(console.error),
      handleInstagram().catch(console.error),
      handleInstagram3().catch(console.error),
      handleAdvancedGMapStandardLoading().catch(console.error),
      handleAdvancedGMapLazyLoading().catch(console.error),
      handleBasicGMapLazyLoading().catch(console.error);
  }
  async function initAliReviews() {
    window.__pagefly_setting__ &&
      document
        .querySelectorAll('[data-pf-type="AliReviews"] [pf-ar-element]')
        .forEach((e) => {
          const n = e == null ? void 0 : e.getAttribute('pf-widget-id'),
            a = document.querySelector(`[data-section-id="${n}"]`);
          a &&
            !a.closest('[pf-ar-element]') &&
            e &&
  ...
  async function initRivyo() {
    const t = document.querySelector(
        '#wc_review_section:not([data-pf-element])'
      ),
    ...
    if (
      ...
      e &&
      ((t.style.display = 'none'),
      !e.querySelector('.wc_review_layout_section'))
    ) {
  ...
  async function initOpinew() {
    const t = document.querySelector(
        '#opinew-reviews-product-page-code:not([data-pf-element])'
      ),
      e = document.querySelector(
        '#opinew-reviews-product-page-code[data-pf-element]'
      );
    if (t && e) {
      t.style.display = 'none';
  ...
  async function initProductReviews() {
    const t = document.querySelector(
        '#shopify-product-reviews:not([data-pf-element])'
      ),
  ...
  async function initWiser() {
    handleWiserElement('new_arrival'),
      handleWiserElement('related_products'),
      handleWiserElement('alsobought'),
      handleWiserElement('recently_products'),
      handleWiserElement('featured_collections'),
      handleWiserElement('recommended_products'),
      handleWiserElement('trending_products'),
      handleWiserElement('top_selling');
  }
  ...
  async function initLimeSpot() {
    let t = 0;
    const e = setInterval(() => {
      handleLimeSpotElement('Popular'),
        handleLimeSpotElement('RecentViews'),
        handleLimeSpotElement('YouMayLike'),
        handleLimeSpotElement('Trending'),
        handleLimeSpotElement('CrossSell'),
        handleLimeSpotElement('BoughtTogether'),
        handleLimeSpotElement('NewArrival'),
        handleLimeSpotElement('Related'),
        handleLimeSpotElement('FeaturedCollection1'),
        handleLimeSpotElement('FeaturedCollection2'),
        handleLimeSpotElement('FeaturedCollection3'),
        handleLimeSpotElement('FeaturedCollection4'),
        ++t >= 10 && clearInterval(e);
    }, 1e3);
  }
  async function initReCharge() {
    ...
    Array.from(t).forEach((e) => {
      ...
      const a = setInterval(() => {
        if (
          (n > 20 && clearInterval(a), n++, !e.querySelector('.rc_container'))
        ) {
  ...
  async function initLooxReview() {
    ...
    if (
      (t == null ? void 0 : t.closest('[data-pf-type=ProductPreview]')) &&
      e
    ) {
      t.style.display = 'none';
      const n = e.parentElement;
  ...
  async function initPowr() {
    !document.querySelector('[data-pf-type="Powr"]') ||
      (await loadJS('https://www.powr.io/powr.js?platform=shopify'),
      window.loadPowr && window.loadPowr());
  }
  async function initInfiniteOptionsShoppad() {
    ...
    if (!t) return;
    const e =
      (o = t.closest('[data-pf-type="ProductBox"]')) == null
        ? void 0
        : o.getAttribute('data-product-id');
    (window.productJSON = window.productJSON || window.__pageflyProducts[e]),
      ((i) => {
        const s = i == null ? void 0 : i.closest('[data-pf-type="ProductBox"]');
        if (!s) return;
        const c =
          s.querySelector('[data-pf-type="ProductATC"]') ||
          s.querySelector('[data-pf-type="ProductATC2"]');
    ...
    const r = setInterval(() => {
      a++,
        (
          t.querySelectorAll(
            '[data-required="true"] input[type="hidden"][name]'
          ) || []
        ).forEach((s) => {
  ...
  async function initInkybay() {
    const t = setInterval(() => {
      var e;
      if (
        ((e = window.customizable_pro_id_arr) == null ? void 0 : e.length) > 0
      ) {
        clearInterval(t);
        const n = window.customizable_pro_id_arr;
        document
          .querySelectorAll('[data-pf-type="InkybayProductPersonalizer"]')
          .forEach((r) => {
            ...
            if (n.includes(i)) {
              ...
              s.setAttribute(
                'style',
                'display: block; position: absolute; z-index: 1; top: 0; left: 0; background: transparent; width: 100%; height: 100%; cursor: pointer'
              ),
                s.addEventListener('click', () => {
                  r.querySelector(
                    `#inkybay_collection_customizeit_btn_${i}`
                  ).click();
                }),
                r.prepend(s);
              ...
              c &&
                (c.classList.add(
                  'inkybay_custom_product_hide_elements',
                  'inkybay_option_product_hide_elements'
                ),
                c.style.setProperty('display', 'none', 'important'));
              const d =
                o.querySelector('[data-pf-type="ProductATC"]') ||
                o.querySelector('[data-pf-type="ProductATC2"]');
              d &&
                (d.classList.add(
                  'inkybay_custom_product_hide_elements',
                  'inkybay_option_product_hide_elements'
  ...
  const initTrackingMoreElement = async () => {
    if (document.querySelector('[data-pf-type="TrackingMore"]')) {
      const t = document.createElement('script'),
        ...
        a = e.getMonth() + 1,
        r = e.getDate() + 1;
      (t.src = `//admin.trackingmore.net/static/js/tm_loadFrame.js?time=${n}${a}${
        r % 7
      }`),
        document
          .querySelector('.tm_main')
          .parentNode.insertBefore(
            t,
            document.querySelector('.tm_main').nextSibling
          );
    }
  };
  async function initGloboBackInStock() {
    ...
    e.className.indexOf('hideBuyNowBtn') > -1
      ? n.then(() => {
          n.then(
            () =>
              (e.querySelector('[data-pf-type="ProductATC2"]').style.display =
                'none')
          );
        })
      : n.then(() => {
  ...
  async function handleThirdPartyElements() {
    initAliReviews().catch(console.error),
      initRivyo().catch(console.error),
      initGrowave().catch(console.error),
      initOpinew().catch(console.error),
      initProductReviews().catch(console.error),
      initWiser().catch(console.error),
      initLimeSpot().catch(console.error),
      initReCharge().catch(console.error),
      initLooxReview().catch(console.error),
      initPushOwl().catch(console.error),
      initPowr().catch(console.error),
      initInfiniteOptionsShoppad().catch(console.error),
      initBIS().catch(console.error),
      initInkybay().catch(console.error),
      initVitalsTrustBadge().catch(console.error),
      initTrackingMoreElement().catch(console.error),
      initGloboBackInStock().catch(console.error);
  }
  ...
  async function handleAnimation() {
    ...
    if (
      !document.querySelector(`link[href*="${PAGEFLY_ANIMATION_CSS}.css"]`) &&
      (e.length || n.length || a.length)
    ) {
      const i = document.createElement('link');
      (i.rel = 'stylesheet'),
        (i.href = `${pfSetting.cdnURL}/static/css/animation.css`),
        (i.media = 'print'),
        (i.onload = () => {
          i.media = 'all';
        }),
    ...
    for (let i = 0; i < a.length; i++) {
      o.push({
        element: a[i],
        position: window.pfCustomGetOffset
          ? window.pfCustomGetOffset(a[i])
          : getOffset(a[i]),
      ...
      if (c) {
        ...
        s.position.top + s.position.height <= t + l &&
          ((d.style.visibility = 'visible'),
          d.setAttribute(
            'class',
            `${d.getAttribute('class') || ''} pf-animated ${c}`
          )),
          (s.element.onanimationend = () => {
            setTimeout(() => {
    ...
    document.addEventListener(
      ...
      (i) => {
        const s = window.scrollY || document.body.scrollTop,
          c = t + s;
        o.forEach(({ element: d, position: l, animation: u }) => {
          ...
          l.top + l.height + f < c &&
            ...
            (d.classList.contains(u) ||
              ...
              d.setAttribute(
                'class',
                `${d.getAttribute('class') || ''} pf-animated ${u}`
              )));
        });
      },
      !0
  ...
  function elementsHelperTrigger() {
    try {
      handleBasicElement().catch(console.error),
        handleShopifyProduct().catch(console.error),
        handleThirdPartyElements().catch(console.error),
        handleAnimation().catch(console.error);
    } catch (t) {
  ...
  class PFPagePrefetcher {
    constructor() {
      ...
      (this.lastTouchTimestamp = 0),
        ...
        (this.cachedURLToPrefetch = new Map()),
        (this.prefetcher = document.createElement('link')),
        (this.prefetcher.id = 'pf-prefetcher'),
        (this.isDataSaverEnabled =
          navigator.connection && navigator.connection.saveData),
        (this.isPrefetchAvailable =
          this.prefetcher &&
          this.prefetcher.relList.supports &&
          this.prefetcher.relList.supports('prefetch')),
        (this.startTime = 0),
        this.init();
    }
    init() {
      (this.prefetcher.rel = 'prefetch'),
        (this.prefetcher.as = 'document'),
    ...
    handleTouchstartEvent(e) {
      ...
      a &&
        ...
        (n.addEventListener(
          'touchcancel',
          this.handleTouchcancelAndTouchEndEvent.bind(this),
          { passive: !0 }
        ),
        n.addEventListener(
          'touchend',
    ...
    handleMouseoverEvent(e) {
      ...
      a &&
        n &&
        (n.addEventListener('mouseout', this.handleMouseoutEvent.bind(this), {
          passive: !0,
        }),
    ...
    handleMouseoutEvent(e) {
      ...
      (!n ||
        e.target.closest('a, [data-href]') !== n.closest('a, [data-href]')) &&
        this.stopPrefetching();
    }
    getAbsoluteUrl(e) {
      try {
    ...
    isPrefetchable(e) {
      ...
      if (
        !(
          (e == null ? void 0 : e.hasAttribute('data-pf-type')) ||
          (e == null ? void 0 : e.closest('[data-pf-type]'))
        ) ||
        ((r = e == null ? void 0 : e.href) == null
          ? void 0
          : r.includes(location.href))
      )
        return !1;
      const n =
        (e == null ? void 0 : e.href) ||
        (e == null ? void 0 : e.getAttribute('data-href')) ||
        '';
      if (!n) return !1;
      const a = this.getAbsoluteUrl(n);
      return this.cachedURLToPrefetch.get(a.href)
        ? !1
        : a.origin === location.origin &&
            ['http:', 'https:'].includes(a.protocol) &&
            !(a.protocol === 'http:' && location.protocol === 'https:') &&
            !(
              a.hash &&
    ...
    prefetch(e) {
      this.prefetcher.setAttribute('href', e),
        this.isPrefetchAvailable || this.prefetchViaXHR(e);
    }
    stopPrefetching() {
    ...
    prefetchViaXHR(e) {
      return new Promise((n, a) => {
        ...
        r.open('GET', e, !0),
          this.cachedURLToPrefetch.set(e, !0),
          (r.onload = () => {
  ...
  const handlePrefetchAssets = async () => {
      var t, e;
      (e = (t = window.PAGEFLY) == null ? void 0 : t.page_optimization) !=
        null &&
        e.assets_prefetching &&
        new PFPagePrefetcher();
    },
    ...
    runHelper = () => {
      Array.from(document.querySelectorAll('script'))
        .map((n) => {
          ...
          return specificHelperCDN.includes(a)
            ? '2.0.0'
            : getPFHelperVersion(a || '');
        })
        .filter((n) => n)
        .find(
          (n) =>
            comparePFHelperVersion(
              n,
              STARTED_VERSION_USING_THEME_APP_EXTENSION
            ) < 0
        ) ||
        (console.log('Run PageFly Theme Extension...'),
        elementsHelperTrigger(),
        handleLazyLoadBackgroundImage().catch(console.error),
        handlePrefetchAssets().catch(console.error));
    };
  runHelper();
    ...
    try {
      (t = window.Shopify) != null &&
        ...
        document.addEventListener('shopify:section:load', function (e) {
          if (
            e.target.closest('[data-shopify-editor-section*=pagefly_section]')
          ) {
            const n = document.querySelectorAll(
                '[data-shopify-editor-section*=pf-]'
              ),
              a = document.querySelectorAll(
                '[data-shopify-editor-section*=pagefly_section]'
              );
            [...n, ...a].forEach((o) => {
              ...
              document
                .querySelectorAll('[data-pf-type=ProductQuantity]')
                .forEach((o) => {
    ...
    Array.from(t).forEach((e) => {
      if (!e.getAttribute('data-pf-activated')) {
        ...
        Array.from(e.attributes).forEach((a) => {
          ...
          n.appendChild(document.createTextNode(e.innerHTML)),
          n.setAttribute('data-pf-activated', 'true'),
  ...
  function handleAthensTheme() {
    fetch('/cart?view=mini-cart')
      ...
      .then((t) => {
        const n = new DOMParser()
          .parseFromString(t, 'text/html')
          .getElementById('HeaderMiniCart');
        $('#HeaderMiniCart').html(n.innerHTML),
          $('.head-slot-cart-link .head-slot-cart-link-quantity').length > 0
            ? $('.head-slot-cart-link .head-slot-cart-link-quantity').html(
                cart.item_count
              )
            : $('.head-slot-cart-link').append(
  ...
  function handleFlowTheme(t) {
    wetheme.updateCartDrawer(t),
      wetheme.toggleRightDrawer('cart', !0, { cart: t });
  }
  ...
  function handleRebrandingTheme() {
      ...
      $.ajax({
        ...
        complete: function (r) {
          $('#cart-dropdown-span').html(
            $('#cart-dropdown-span', r.responseText).html()
          );
        },
      });
    };
    if ($('.cartCountSelector').length) {
      ...
      $(t.cartCountSelector)
        .html(n.replace(/[0-9]+/, cart.item_count))
        .removeClass('hidden'),
        $('.cartTotalSelector').html(
          Shopify.formatMoney(cart.total_price, theme.moneyFormat).replace(
            /((\,00)|(\.00))$/g,
            ''
          )
        );
  ...
  function handleMarkerTheme(t) {
    var e = t.item_count;
    return (
      $('.cart--external--total-items').text(e),
      $.ajax({
        ...
        var a;
        (a = $($.parseHTML(n))),
          $('.cart--root').attr('data-has-items', !0),
          $('.cart--root')
            .find('.cart--form')
            .replaceWith(a.find('.cart--form'));
      }),
      e > 0
        ? ($('.cart--external--total-items').css('display', 'inline-block'),
          $('.cart--external--total-items').parent().show())
        : ($('.cart--external--total-items')
            .not('.header--mobile-cart-count')
            .hide(),
          $('.cart--external--total-items')
            .not('.header--mobile-cart-count')
            .parent()
  ...
  function handleImpulseTheme() {
    document.dispatchEvent(
      new CustomEvent('ajaxProduct:added', {
        detail: { product: res, addToCartBtn: '' },
      })
    );
  }
  function handleFocalTheme(t) {
    document.documentElement.dispatchEvent(
      ...
      (document.querySelector('cart-count').innerText = t.item_count);
  }
  ...
  function handleWarehouseTheme() {
    document.dispatchEvent(
      new CustomEvent('product:added', {
        bubbles: !0,
        detail: { variant: res.variant_id, quantity: 1 },
      })
    );
  }
  function handlePrestigeTheme(t) {
    try {
      document.dispatchEvent(
        new CustomEvent('product:added', {
          bubbles: !0,
          detail: { variant: t.variant_id, quantity: 1 },
        })
      );
    } catch (e) {
  ...
  function handleFurnTheme(t, e) {
    Shopify.onItemAdded(t),
      Shopify.onCartUpdate(e),
      jQuery('#modalAddToCart').modal('toggle'),
      (document.querySelector('.popupimage').src = t.image);
  }
  function handleTurboTheme(t) {
    window.refreshCart && window.refreshCart(t),
      $('#header').is(':visible')
      ...
      window.PXUTheme.media_queries.medium.matches &&
        ($(this).parent().hasClass('active_link'),
        $('body').addClass('blocked-scroll'),
        document
          .getElementById('header')
          .classList.contains('mobile_nav-fixed--false') &&
    ...
    fetch('/?snippets_id=cart')
      ...
      .then((e) => {
        const a = new DOMParser()
          ...
          .querySelector('[data-active="cart"]');
        if (a) {
          const r = a.innerHTML;
          $('[data-active="cart"]').html(r),
            $('.header--cart-link').attr('data-has-items', 'true'),
            $('.cart--external--total-items').text(t.item_count),
            $('.cart--external--total-price').text(
              $('.cart--total--price.money').text()
            ),
  ...
  function handleProvinceTheme(t) {
    ...
    e.classList.add('item-count', 'inline-block', 'text-center'),
      document.querySelector('.cart').appendChild(e),
      (e.innerHTML = t == null ? void 0 : t.item_count);
  }
  ...
  function handleQuarkTheme(t) {
    document.documentElement.dispatchEvent(
      ...
      document
        .querySelectorAll('.header__icon-wrapper[aria-label="Cart"]')[0]
        .click();
  }
  function handleLaunchTheme(t) {
    $('.header-cart-count').text(t.item_count),
  ...
  function handleStockholmTheme(t) {
    fetch('/?snippets_id=cart-notification')
      ...
      .then((e) => {
        ...
        if (
          (n.querySelector('#cart-notification .totals__subtotal-value')
            .innerHTML,
          $('#cart-notification').html(a),
          $('cart-drawer').removeClass('is-empty'),
          $('cart-drawer-items').removeClass('is-empty'),
          $('#cart-icon-bubble .cart-count-bubble').length === 0)
        ) {
          const r = document.createElement('div');
          (r.className = 'cart-count-bubble'),
            (r.innerHTML = `
          ...
          $('.cart-count-bubble span[aria-hidden="true"]').each((r, o) =>
            $(o).text(t.item_count)
          );
  ...
  function handleEmpireTheme(t, e) {
    const n = new CustomEvent('cartcount:update', { detail: e, res: t });
    window.dispatchEvent(n),
      ...
      $('.atc-banner--container [data-atc-banner-product-image]').html(
        `<img src="${t.image}" alt="${t.title}">`
      ),
      $('.atc-banner--container [data-atc-banner-product-title]').html(t.title),
      $('.atc-banner--container [data-atc-banner-product-price-quantity]').html(
        `${t.quantity} \xD7 `
      ),
      $('.atc-banner--container [data-atc-banner-cart-button] span').html(
        e.item_count
      ),
      $('.atc-banner--container [data-atc-banner-product-discounts]').css(
        'display',
        'none'
      ),
  ...
  function handleFreeShopifyTheme(t) {
    $.getJSON(
      ...
      n
        ? (n.classList.remove('is-empty'), n.renderContents(t))
        : $('#cart-icon-bubble')[0].click();
    });
  }
  function handleHandmadeTheme(t, e) {
    $.getJSON(
      ...
      o && (o.innerHTML = e.total_price / 100 + ' ' + e.currency),
        a.renderContents(t);
    });
  }
      ...
      setTimeout(function () {
        $('.loader-container').hide(),
          ...
          cartData.shopping_cart_action == 'widget' &&
            ($('.header-container').hasClass('sticky-header')
  ...
  function handleSpeedflyTheme(t) {
    document.querySelector('mini-cart').generateDom(t),
  ...
  const handleThemeBrooklyn = async () => {
      !ajaxCartInitialized &&
        ...
        (window.ajaxCart.init({
          ...
          moneyFormat:
            window.pfSetting.moneyFormat || '${{amount_with_comma_separator}}',
        }),
        (ajaxCartInitialized = !0)),
        $('body').trigger(
          'ajaxCart.afterCartLoad',
          window.__pagefly_helper_store__.cart
        ),
        $('.cart-link.js-drawer-open-button-right').trigger('click');
    },
    handleThemeDebut = async () => {
      var e;
      const { lastATCResult: t } = __pagefly_helper_store__;
      !window.pfProduct &&
        window.theme.Product.toString().includes('$container') &&
        (window.pfProduct = new window.theme.Product()),
        typeof ((e = window.pfProduct) == null ? void 0 : e._setupCartPopup) ==
          'function' && window.pfProduct._setupCartPopup(t);
    },
    handleThemeVenue = async (t) => {
      const { item_count: e } = t;
  ...
  try {
    setTimeout(function () {
      window.__pagefly_helper_store__ &&
        window.__pagefly_helper_store__.subscribe(function (t) {
          const e =
            window.__pagefly_theme_atc_check__ ||
            ...
            .then((n) => {
              switch (!0) {
                ...
                case /Empire/i.test(e):
                  handleEmpireTheme(t, n);
                  break;
                case /Speedfly/i.test(e):
                  handleSpeedflyTheme(n);
                  break;
                ...
                case /Exclusive/i.test(e):
                  handleThemeExclusive();
                  break;
                case /Origin|Spotlight|Crave|Publisher|Colorblock|Studio|Ride/i.test(
                  e
                ):
                  handleThemeFreeShopify(n);
                  break;
              }
            })
            .catch((n) => console.error(`Failed to update cart ${e}`, n));
        });
    }, 2500);
  } catch (t) {
